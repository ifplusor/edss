...
//
QTAtom_stsz::QTAtom_stsz(QTFile *File,
...
bool QTAtom_stsz::Initialize() {
  ...
  if (((PointerSizedInt) fSampleSizeTable & (PointerSizedInt) 0x3) == 0)
    ...
    fTable = (UInt32 *) (((PointerSizedInt) fSampleSizeTable + 4)
        & ~((PointerSizedInt) 0x3));
...
bool QTAtom_stsz::SampleRangeSize(UInt32 firstSampleNumber,
                                  UInt32 lastSampleNumber,
                                  UInt32 *sizePtr) {
  bool result = false;

  do {
    if (lastSampleNumber < firstSampleNumber) {
      //          s_printf("QTAtom_stsz::SampleRangeSize (lastSampleNumber %" _S32BITARG_ " < firstSampleNumber %" _S32BITARG_ ") \n",lastSampleNumber, firstSampleNumber);
      break;
    }

    if (fCommonSampleSize) {
      //          s_printf("QTAtom_stsz::SampleRangeSize fCommonSampleSize %" _S32BITARG_ " firstSampleNumber %" _S32BITARG_ " lastSampleNumber %" _S32BITARG_ " *sizePtr %" _S32BITARG_ "\n",fCommonSampleSize,firstSampleNumber,lastSampleNumber,*sizePtr);
      if (sizePtr != NULL)
        *sizePtr =
            fCommonSampleSize * (lastSampleNumber - firstSampleNumber + 1);
    ...
    if (firstSampleNumber && lastSampleNumber
        && (lastSampleNumber <= fNumEntries)
        && (firstSampleNumber <= fNumEntries)) {
      if (sizePtr != NULL) {
        *sizePtr = 0;

        for (UInt32 sampleNumber = firstSampleNumber;
             sampleNumber <= lastSampleNumber; sampleNumber++)
...
void QTAtom_stsz::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_stsz::DumpAtom - ..Number of sample size entries: %"
                  _S32BITARG_
                  "\n", fNumEntries));
}
...
void QTAtom_stsz::DumpTable() {
  ...
  s_printf(
      "-- Sample Size table -----------------------------------------------------------\n");
  s_printf("\n");
  s_printf("  Sample Num   SampleSize\n");
  s_printf("  ----------   ----------\n");
  ...
  for (UInt32 CurEntry = 1; CurEntry <= fNumEntries; CurEntry++) {
    //
    // Print out a listing.
    s_printf("  %10" _U32BITARG_ " : %10" _U32BITARG_ "\n",
                CurEntry,
                fTable[CurEntry - 1]);
  }
}
