...
QTFile::ErrorCode QTFile::Open(char const *MoviePath) {
  ...
  QTSS_Error error = QTSS_GetAttrInfoByName(fMovieFD,
                                            "QTSSPosixFileSysModuleOSFileSource",
                                            &attrInfoObject);
  if (QTSS_NoErr == error) {
...
void QTFile::AllocateBuffers(UInt32 inUnitSizeInK,
                             UInt32 inBufferInc,
                             UInt32 inBufferSizeUnits,
  ...
  if (fOSFileSourceFD != NULL) {
    if (!fCacheBuffersSet) {
      ...
      fOSFileSourceFD->AllocateFileCache(inUnitSizeInK,
                                         inBufferSizeUnits,
                                         inBufferInc,
...
bool QTFile::FindTOCEntry(char const *AtomPath,
  ...
  if (LastFoundTOCEntry != NULL) {
    ...
    if (*pCurAtomType == ':') {
      DEEP_DEBUG_PRINT(("QTFile::FindTOCEntry - ..Rooting search at [%03"
                           _U32BITARG_
      ...
      for (CurParent = Atom->Parent; CurParent != NULL;
      ...
      if (Atom != NULL) {
        DEEP_DEBUG_PRINT(("QTFile::FindTOCEntry - ..Starting search at [%03"
                             _U32BITARG_
                             "] '%c%c%c%c'.  Search path is \"%s\"\n",
                                 Atom->TOCID,
  ...
  while (Atom != NULL) { // already initialized by the above
    DEEP_DEBUG_PRINT(("QTFile::FindTOCEntry - ..Comparing against [%03"
                         _U32BITARG_
                         " '%c%c%c%c'\n",
                             Atom->TOCID,
                             (char) ((Atom->AtomType & 0xff000000) >> 24),
...
//
// Read functions.
bool QTFile::Read(UInt64 Offset,
                  char *const Buffer,
                  UInt32 Length,
                  QTFile_FileControlBlock *FCB) {
  // General vars
  CF::Core::MutexLocker ReadMutex(fReadMutex);
  bool rv = false;
  UInt32 gotlen = 0;
...
bool QTFile::GenerateAtomTOC() {
  ...
  while (Read(CurPos, (char *) &atomLength, 4)) {
    ...
    if ((BigAtomLength == 0) && CurParent
        && (CurParent->AtomType == FOUR_CHARS_TO_INT('u', 'd', 't', 'a'))) {
      //
      // Do no harm..
    ...
    } else  // This is a normal atom; get the atom type.
    {
      ...
      if (AtomType == FOUR_CHARS_TO_INT('u', 'u', 'i', 'd')) {
        ...
        UInt8 usertype[sExtendedTypeSize + 1
        ]; //sExtendedTypeSize for the type + 1 for 0 terminator.
        usertype[sExtendedTypeSize] = 0;
        if (!Read(CurPos,
                  (char *) usertype,
                  16)) // read and just throw it away we don't need to store
          return false;
    ...
    if (0 == BigAtomLength) {
      DEEP_DEBUG_PRINT(("QTFile::GenerateAtomTOC - Bail atom is bad. type= '%c%c%c%c'; pos=%"
                           _64BITARG_
                           "u; length=%"
                           _64BITARG_
                           "u; header=%"
                           ...
                           ".\n",
                               ...
                               CurPos
                                   - CurAtomHeaderSize, BigAtomLength, CurAtomHeaderSize));

      return false;
    }

    if (hasBigAtom) {
      DEEP_DEBUG_PRINT(("QTFile::GenerateAtomTOC - Found 64 bit atom '%c%c%c%c'; pos=%"
                           _64BITARG_
                           "u; length=%"
                           _64BITARG_
                           "u; header=%"
      ...
      DEEP_DEBUG_PRINT(("QTFile::GenerateAtomTOC - Found 32 bit atom '%c%c%c%c'; pos=%"
                           _64BITARG_
                           "u; length=%"
                           _64BITARG_
                           "u; header=%"
    ...
    switch (NewTOCEntry->AtomType) {
      ...
      case FOUR_CHARS_TO_INT('u',
                             'd',
                             't',
                             'a'): /* can appear anywhere */ //udta
      ...
      case FOUR_CHARS_TO_INT('h', 'i', 'n', 'f'): //hinf
      {
        ...
        if (NewTOCEntry->AtomDataLength
            > 0) // maybe it should be greater than some number such as header size?
        {
...
lbl_SkipAtom:
    while (CurParent
        && ((LastTOCEntry->AtomDataPos - LastTOCEntry->AtomHeaderSize)
            + (LastTOCEntry->AtomDataLength + LastTOCEntry->AtomHeaderSize))
            >= ((CurParent->AtomDataPos - CurParent->AtomHeaderSize)
                + (CurParent->AtomDataLength + CurParent->AtomHeaderSize))) {
