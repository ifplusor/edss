...
class QTRTPFile {
 ...
 public:
  ...
  void AllocateSharedBuffers(UInt32 inUnitSizeInK,
    ...
    fFile->AllocateBuffers(inUnitSizeInK,
                           inBufferInc,
                           inBufferSizeUnits,
                           inMaxBitRateBuffSizeInBlocks,
                           this->GetBytesPerSecond() * 8);
  }

  void AllocatePrivateBuffers(UInt32 inUnitSizeInK,
  ...
  void SetAllowInvalidHintRefs(bool inAllowInvalidHintRefs) {
  ...
  void SetTrackRTPMetaInfo(UInt32 TrackID,
  ...
  enum {
    ...
    kKeyFramesOnly = 5,
    kKeyFramesPlusOneP =
    6            //Special quality level with Key frames followed by 1 P frame
  };
  ...
  bool SetDropRepeatPackets(bool allowRepeatPackets) {
    (!fHasRTPMetaInfoFieldArray) ? fDropRepeatPackets = allowRepeatPackets :
        fDropRepeatPackets = false;
    return fDropRepeatPackets;
  }
