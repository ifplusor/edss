...
//
QTAtom_tref::QTAtom_tref(QTFile *File,
...
bool QTAtom_tref::Initialize() {
  ...
  if (((PointerSizedInt) fTrackReferenceTable & (PointerSizedInt) 0x3) == 0)
    ...
    fTable = (UInt32 *) (((PointerSizedInt) fTrackReferenceTable + 4)
        & ~((PointerSizedInt) 0x3));
...
void QTAtom_tref::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_tref::DumpAtom - ..Number of track reference entries: %"
                  _U32BITARG_
                  "\n", (UInt32) fNumEntries));
}
