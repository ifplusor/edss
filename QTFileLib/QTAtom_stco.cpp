...
//
QTAtom_stco::QTAtom_stco(QTFile *File,
    ...
    : QTAtom(File, TOCEntry, Debug, DeepDebug),
      fNumEntries(0),
      fOffSetSize(offSetSize),
      fChunkOffsetTable(NULL),
      fTable(NULL) {
}
...
void QTAtom_stco::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_stco::DumpAtom - ..Number of chunk offset entries: %"
                  _S32BITARG_
                  "\n", fNumEntries));
}
...
void QTAtom_stco::DumpTable() {
  ...
  for (UInt32 CurEntry = 1; CurEntry <= fNumEntries; CurEntry++) {
    if (ChunkOffset(CurEntry, &offset))
      s_printf("  %10"   _U32BITARG_   ": %" _64BITARG_ "u\n",
                  CurEntry,
                  offset);
    else
      s_printf("  %10"   _U32BITARG_   ": QTAtom_stco::DumpTable ChunkOffset error\n",
                  CurEntry);
  }
}
