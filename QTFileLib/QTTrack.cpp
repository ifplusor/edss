...
//
QTTrack::QTTrack(QTFile *File,
                 QTFile::AtomTOCEntry *Atom,
                 bool Debug,
                 bool DeepDebug)
    : fDebug(Debug),
      fDeepDebug(DeepDebug),
      fFile(File),
      ...
      fEditListAtom(NULL),
      fDataReferenceAtom(NULL),
      fTimeToSampleAtom(NULL),
      fCompTimeToSampleAtom(NULL),
      fSampleToChunkAtom(NULL),
      fSampleDescriptionAtom(NULL),
      fChunkOffsetAtom(NULL),
      fSampleSizeAtom(NULL),
...
QTTrack::ErrorCode QTTrack::Initialize() {
  ...
  if (fFile->FindTOCEntry(":udta:name", &tempTOCEntry, &fTOCEntry)) {
    ...
    if (fTrackName != NULL)
      fFile->Read(tempTOCEntry->AtomDataPos,
                  fTrackName,
                  (UInt32) tempTOCEntry->AtomDataLength);
  }
  ...
  DEEP_DEBUG_PRINT(("Searching track #%"
                       _U32BITARG_
                       " 'elst' atom.\n", GetTrackID()));
  if (fFile->FindTOCEntry(":edts:elst", &tempTOCEntry, &fTOCEntry)) {
    ...
    fFirstEditMediaTime = (UInt32) (
        (((Float64) (SInt64) GetFirstEditMovieTime()) / fFile->GetTimeScale())
            * GetTimeScale());
  } else {
    fEditListAtom = NULL;
  }
  ...
  if (fFile->FindTOCEntry(":mdia:minf:stbl:ctts", &tempTOCEntry, &fTOCEntry)) {
    fCompTimeToSampleAtom =
        new QTAtom_ctts(fFile, tempTOCEntry, fDebug, fDeepDebug);
  ...
  fSampleDescriptionAtom =
      new QTAtom_stsd(fFile, tempTOCEntry, fDebug, fDeepDebug);
  if (fSampleDescriptionAtom == NULL)
    return errInternalError;
  ...
  if (fFile->FindTOCEntry(":mdia:minf:stbl:stco", &tempTOCEntry, &fTOCEntry)) {
    coFound = true;
    offSetSize = 4;
  } else if (fFile->FindTOCEntry(":mdia:minf:stbl:co64",
                                 &tempTOCEntry,
                                 &fTOCEntry)) {
    coFound = true;
    offSetSize = 8;
  ...
  fChunkOffsetAtom =
      new QTAtom_stco(fFile, tempTOCEntry, offSetSize, fDebug, fDeepDebug);
  if (fChunkOffsetAtom == NULL)
    return errInternalError;
...
// Sample functions
//
bool QTTrack::GetSampleInfo(UInt32 SampleNumber,
                            UInt32 *const Length,
                            UInt64 *const Offset,
                            UInt32 *const SampleDescriptionIndex,
                            QTAtom_stsc_SampleTableControlBlock *STCB) {

  Assert(STCB != NULL);

  //  s_printf("GetSampleInfo QTTrack SampleNumber = %" _S32BITARG_ " \n", SampleNumber);

  if (STCB->fGetSampleInfo_SampleNumber == SampleNumber
      && STCB->fGetSampleInfo_Length > 0) {
    //      s_printf("----- GetSampleInfo Cache Hit QTTrack SampleNumber = %" _S32BITARG_ " \n", SampleNumber);

    if (Length) *Length = STCB->fGetSampleInfo_Length;
    if (Offset) *Offset = STCB->fGetSampleInfo_Offset;
    if (SampleDescriptionIndex)
      *SampleDescriptionIndex = STCB->fGetSampleInfo_SampleDescriptionIndex;
  ...
  if (!SampleNumberToChunkNumber(SampleNumber,
                                 &ChunkNumber,
  ...
  if (ChunkNumber == STCB->fGetSampleInfo_LastChunk
      && (SampleNumber == (STCB->fGetSampleInfo_SampleNumber + 1))) {
...
bool QTTrack::GetSizeOfSamplesInChunk(UInt32 chunkNumber,
                                      UInt32 *const sizePtr,
                                      UInt32 *const firstSampleNumPtr,
                                      UInt32 *const lastSampleNumPtr,
                                      QTAtom_stsc_SampleTableControlBlock *stcbPtr) {
  UInt32 firstSample = 0;
  UInt32 lastSample = 0;
  UInt32 size = 0;

  if (stcbPtr && stcbPtr->fGetSizeOfSamplesInChunk_chunkNumber == chunkNumber) {
    //      s_printf("QTTrack::GetSizeOfSamplesInChunk cache hit %" _S32BITARG_ " \n", chunkNumber);
    if (firstSampleNumPtr != NULL)
      *firstSampleNumPtr = stcbPtr->fGetSizeOfSamplesInChunk_firstSample;
    if (lastSampleNumPtr != NULL)
      *lastSampleNumPtr = stcbPtr->fGetSizeOfSamplesInChunk_lastSample;
    if (sizePtr != NULL) *sizePtr = stcbPtr->fGetSizeOfSamplesInChunk_size;
    return true;
  }

  bool result =
      GetChunkFirstLastSample(chunkNumber, &firstSample, &lastSample, stcbPtr);
...
bool QTTrack::GetSample(UInt32 SampleNumber,
                        char *Buffer,
                        UInt32 *Length,
