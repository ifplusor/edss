...
class QTTrack {
 ...
 public:
  ...
  inline Float64 GetDurationInSeconds() {
    return GetDuration() / (Float64) GetTimeScale();
  }
  ...
  //
  // Sample functions
  bool GetSizeOfSamplesInChunk(UInt32 chunkNumber,
  ...
  inline bool GetChunkFirstLastSample(UInt32 chunkNumber,
                                      UInt32 *firstSample,
                                      UInt32 *lastSample,
                                      QTAtom_stsc_SampleTableControlBlock *STCB) {
    return fSampleToChunkAtom->GetChunkFirstLastSample(chunkNumber,
                                                       firstSample,
                                                       lastSample,
                                                       STCB);
  }

  inline bool SampleToChunkInfo(UInt32 SampleNumber,
                                UInt32 *samplesPerChunk,
                                UInt32 *ChunkNumber,
                                UInt32 *SampleDescriptionIndex,
                                UInt32 *SampleOffsetInChunk,
                                QTAtom_stsc_SampleTableControlBlock *STCB) {
    return fSampleToChunkAtom->SampleToChunkInfo(SampleNumber,
                                                 samplesPerChunk,
                                                 ChunkNumber,
  ...
  inline bool SampleNumberToChunkNumber(UInt32 SampleNumber,
                                        UInt32 *ChunkNumber,
                                        UInt32 *SampleDescriptionIndex,
                                        UInt32 *SampleOffsetInChunk,
                                        QTAtom_stsc_SampleTableControlBlock *STCB) {
    return fSampleToChunkAtom->SampleNumberToChunkNumber(SampleNumber,
                                                         ChunkNumber,
                                                         SampleDescriptionIndex,
                                                         SampleOffsetInChunk,
                                                         STCB);
  ...
  inline bool SampleRangeSize(UInt32 firstSample,
                              UInt32 lastSample,
                              UInt32 *sizePtr = NULL) {
    return fSampleSizeAtom->SampleRangeSize(firstSample, lastSample, sizePtr);
  }

  bool GetSampleInfo(UInt32 SampleNumber,
                     UInt32 *const Length,
                     UInt64 *const Offset,
                     UInt32 *const SampleDescriptionIndex,
                     QTAtom_stsc_SampleTableControlBlock *STCB);

  bool GetSample(UInt32 SampleNumber,
                 char *Buffer,
                 UInt32 *Length,
  ...
  inline bool GetSampleMediaTime(UInt32 SampleNumber, UInt32 *const MediaTime,
    ...
    return fTimeToSampleAtom->SampleNumberToMediaTime(SampleNumber,
                                                      MediaTime,
                                                      STCB);
  }

  inline bool GetSampleNumberFromMediaTime(UInt32 MediaTime,
                                           UInt32 *const SampleNumber,
                                           QTAtom_stts_SampleTableControlBlock *STCB) {
    return fTimeToSampleAtom->MediaTimeToSampleNumber(MediaTime,
                                                      SampleNumber,
                                                      STCB);
  }

  inline void GetPreviousSyncSample(UInt32 SampleNumber,
                                    UInt32 *SyncSampleNumber) {
    if (fSyncSampleAtom != NULL)
      fSyncSampleAtom->PreviousSyncSample(SampleNumber,
                                          SyncSampleNumber);
  ...
  inline void GetNextSyncSample(UInt32 SampleNumber, UInt32 *SyncSampleNumber) {
    if (fSyncSampleAtom != NULL)
      fSyncSampleAtom->NextSyncSample(SampleNumber,
                                      SyncSampleNumber);
  ...
  inline bool IsSyncSample(UInt32 SampleNumber, UInt32 SyncSampleCursor) {
    if (fSyncSampleAtom != NULL)
      return fSyncSampleAtom->IsSyncSample(SampleNumber,
                                           SyncSampleCursor);
    else return true;
  }
  //
  // Read functions.
  inline bool Read(UInt32 SampleDescriptionID,
                   UInt64 Offset,
                   char *const Buffer,
                   UInt32 Length,
                   QTFile_FileControlBlock *FCB = NULL) {
    return fDataReferenceAtom->Read(fSampleDescriptionAtom->SampleDescriptionToDataReference(
        SampleDescriptionID), Offset, Buffer, Length, FCB);
  }

  inline bool GetSampleMediaTimeOffset(UInt32 SampleNumber,
                                       UInt32 *mediaTimeOffset,
                                       QTAtom_ctts_SampleTableControlBlock *STCB) {
    if (fCompTimeToSampleAtom)
      return fCompTimeToSampleAtom->SampleNumberToMediaTimeOffset(SampleNumber,
                                                                  mediaTimeOffset,
                                                                  STCB);
    else
      return false;
  ...
  inline void DumpCompTimeToSampleTable() {
    if (fCompTimeToSampleAtom) fCompTimeToSampleAtom->DumpTable();
    else
      printf("*** no ctts table ****\n");
  }
