...
#define TEMP_PRINT(s)  s_printf( s )
#define TEMP_PRINT_ONE( s, p1 )   s_printf( s, p1 )
#define TEMP_PRINT_TWO( s, p1, p2 )   s_printf( s, p1, p2 )
...
#define DEBUG_PRINT(s) if(fDebug) s_printf s
#define DEEP_DEBUG_PRINT(s) if(fDeepDebug) s_printf s
...
//
QTHintTrack::QTHintTrack(QTFile *File,
                         QTFile::AtomTOCEntry *Atom,
                         bool Debug,
                         bool DeepDebug)
...
#if TESTTIME
  s_printf(" QTHintTrack initialized \n");
  mediaPacketCount = 0;
  hintPacketCount = 0;
...
QTTrack::ErrorCode QTHintTrack::Initialize() {
  ...
  for (pSampleDescription = (sampleDescription + 24);
    ...
    switch (dataType) {
      ...
      case FOUR_CHARS_TO_INT('t',
                             's',
                             'r',
                             'o'): // tsro Timestamp random offset
        ::memcpy(&fTimestampRandomOffset, pSampleDescription + 8, 4);
        fTimestampRandomOffset = ntohl(fTimestampRandomOffset);
        break;

      case FOUR_CHARS_TO_INT('s',
                             'n',
                             'r',
                             'o'): // snro Sequence number random offset
  ...
  fHintTrackReferenceAtom =
      new QTAtom_tref(fFile, hinfTOCEntry, fDebug, fDeepDebug);
  if (fHintTrackReferenceAtom == NULL)
    return errInternalError;
  ...
  ::memset((void *) fTrackRefs,
           0,
           sizeof(QTTrack *) * numTrackRefs); //initialize ptr array with 0.
...
void QTHintTrack::GetSamplePacketHeaderVars(char *samplePacketPtr,
                                            char *maxBuffPtr,
  ...
  bool tlvOK =
      false; // reset tlvSize to 0 if the size value or the tlv flag is invalid
  if (hdrData.hintFlags & 0x4)
    do // Extra Information TLV is present
    {
      ...
      if (tlvEnd >= maxBuffPtr || tlvParser >= maxBuffPtr
          || tlvEnd < samplePacketPtr || tlvParser < samplePacketPtr) {
        WarnV(tlvParser < maxBuffPtr,
              "incorrect tlv data: ignoring"); // error tlv-start past buffer
        WarnV(tlvEnd < maxBuffPtr,
              "incorrect tlv data: ignoring"); // error tlv-end past buffer
        WarnV(tlvEnd >= samplePacketPtr,
              "incorrect tlv data: ignoring"); // error tlv-end before start of sample in buffer
        WarnV(tlvParser >= samplePacketPtr,
              "incorrect tlv data: ignoring"); // error tlv-start before start of sample in buffer
        break;
      }

      tlvOK =
          true; // at this point hdrData.tlvSize is ok. The size is used to calculate the next packet so it better be correct.

      // if there is a TLV, parse out the 1 field we currently know about, the 'rtpo' field
      while (tlvParser + 12
          < tlvEnd) // test for the minimum tlv size (size+type+smallest data size)
      {
        UInt32 fieldSize = ntohl(*(UInt32 *) tlvParser);
        UInt32 theType = ntohl(*(UInt32 *) (tlvParser + 4));
        UInt32 theData = ntohl(*(UInt32 *) (tlvParser
            + 8)); //data can't be smaller 4 and we only know about rtpo data
        ...
        tlvParser +=
            fieldSize;// add the field size to current for the next TLV entry
      }
  ...
  hdrData.relativePacketTransmissionTime =
      ntohl(hdrData.relativePacketTransmissionTime);
...
QTTrack::ErrorCode QTHintTrack::GetSamplePacketPtr(char **samplePacketPtr,
                                                   UInt32 sampleNumber,
                                                   UInt16 packetNumber,
                                                   QTHintTrackRTPHeaderData &hdrData,
  ...
  if (htcb.fLastPacketNumberFetched != 0xFFFF
      && packetNumber == htcb.fLastPacketNumberFetched + 1) {
    ...
    *samplePacketPtr = htcb.fPointerToNextPacket;
    ...
    Assert(htcb.fPointerToNextPacket
               <= (char *) (htcb.fCachedSample + htcb.fCachedSampleLength));

    if (htcb.fPointerToNextPacket
        > (htcb.fCachedSample + htcb.fCachedSampleLength))
      return errInvalidQuickTimeFile;
...
bool QTHintTrack::GetSamplePtr(UInt32 sampleNumber,
                               char **samplePtr,
                               UInt32 *length,
                               QTHintTrack_HintTrackControlBlock *htcb) {
  // General vars
  ...
  if (!this->GetSampleInfo(sampleNumber,
                           &newSampleLength,
                           &sampleOffset,
  ...
  if ((htcb->fCachedSample == NULL)
      || (htcb->fCachedSampleSize < newSampleLength)) {
    //
  ...
  //
  // Read in the sample
  if (!fDataReferenceAtom->Read(sampleDescriptionIndex,
                                sampleOffset,
                                htcb->fCachedSample,
                                htcb->fCachedSampleLength,
                                htcb->fFCB))
    return false;
  //
...
// Packet functions
//
QTTrack::ErrorCode QTHintTrack::GetNumPackets(UInt32 sampleNumber,
                                              UInt16 *numPackets,
                                              QTHintTrack_HintTrackControlBlock *htcb) {
  char *buf;
  UInt32 bufLen;
...
QTTrack::ErrorCode QTHintTrack::GetSampleData(QTHintTrack_HintTrackControlBlock *htcb,
                                              char **buffPtr,
                                              char **ppPacketBufOut,
                                              UInt32 sampleNumber,
                                              UInt16 packetNumber,
  ...
  //  s_printf("GetSampleData sampleNumber = %"   _U32BITARG_   " packetNumber = %"   _U32BITARG_   " buffOutLen = %"   _U32BITARG_   " \n",sampleNumber, packetNumber, buffOutLen);
  // General vars
  ...
  UInt16 samplesPerCompressionBlock =
      0;  // inititialization eliminates a stupid compiler warning :(
  UInt32 sampleDescriptionIndex;
  UInt64 dataOffset;
  char *pBuf = NULL;
  ...
  if (NULL == ppPacketBufOut || NULL == *ppPacketBufOut)
    return errInternalError;
  ...
  DEEP_DEBUG_PRINT(("QTHintTrack::GetPacket - ....Sample entry found (sample#=%"
                       _U32BITARG_
                       "; offset=%"
                       _U32BITARG_
  ...
  if (trackRefIndex == -1) {
    ...
    //      s_printf("hint track sample = %" _S32BITARG_ " \n", mediaSampleNumber);
    //
    ...
    if ((mediaSampleNumber == htcb->fCachedHintTrackSampleNumber)
      ...
      //          s_printf("found cached hint sample = %" _S32BITARG_ " \n", mediaSampleNumber);
    ...
    if ((mediaSampleNumber == htcb->fCachedSampleNumber)
      ...
      //          s_printf("found currently cached sample = %" _S32BITARG_ " \n", mediaSampleNumber);
      if ((char *) (*ppPacketBufOut + readLength - 1) > maxBuffPtr) {
        return errInvalidQuickTimeFile;
      }
    ...
    if (!track->GetSampleInfo(mediaSampleNumber,
                              &sampleLength,
                              &dataOffset,
                              &sampleDescriptionIndex,
                              mediaTrackSTSC_STCBPtr))
      return errInvalidQuickTimeFile;
    ...
    if (!track->Read(sampleDescriptionIndex,
                     dataOffset,
                     *ppPacketBufOut,
                     readLength,
                     htcb->fFCB))
    ...
    *ppPacketBufOut += readLength;  // point to remainder of buffer;
    ...
    if (!track->IsInitialized()) {
      TEMP_PRINT_ONE(
          "QTHintTrack::GetPacket trackRefIndex %li, not initialized,\n",
          (SInt32) trackRefIndex);
      CF::Core::MutexLocker theLocker(fFile->GetMutex());
    ...
    if ((isOneForOne && sampleLength
        == 1)  // special case  (isOneForOne && sampleLength == 1) //  the data is compressed and the sample's byte offset is really a byte offset into a chunk
        || (!isOneForOne)   // compressed data is normally defined by bytesPerCompressionBlock or samplesPerCompressionBlock
        ) {
      //          s_printf("track = %" _S32BITARG_ " sample  = %" _S32BITARG_ " is compressed \n",this, mediaSampleNumber);
      //          s_printf("is compressed bytesPerCompressionBlock = %" _S32BITARG_ " samplesPerCompressionBlock = %d sampleLength = %" _S32BITARG_ "\n", bytesPerCompressionBlock, samplesPerCompressionBlock, sampleLength);
      isCompressed = true;
    }
    ...
    if (isCompressed) {   // Media track sample compressed

      UInt32 compressionBlocksToSkip =
          (UInt32) ((Float64) readOffset / (Float64) bytesPerCompressionBlock);
      mediaSampleNumber += compressionBlocksToSkip * samplesPerCompressionBlock;
      readOffset -= compressionBlocksToSkip
          * bytesPerCompressionBlock; // readoffset should always be 0 after this
      // start gathering chunk info to check sample length against chunk length
      if (!track->SampleToChunkInfo(mediaSampleNumber,
                                    &samplesPerChunk,
                                    &chunkNumber,
                                    NULL,
                                    &sampleOffsetInChunk,
      ...
      dataOffset = (UInt64) chunkOffset
          + (UInt64) ((Float64) sampleOffsetInChunk
              * ((Float64) bytesPerCompressionBlock
                  / (Float64) samplesPerCompressionBlock));

      if (!track->GetSizeOfSamplesInChunk(chunkNumber,
                                          (UInt32 *) &sizeOfSamplesInChunk,
                                          NULL,
                                          NULL,
                                          mediaTrackSTSC_STCBPtr))
        return (errInvalidQuickTimeFile);

      sizeOfSamplesInChunk = (UInt32) ((Float64) sizeOfSamplesInChunk
          * ((Float64) bytesPerCompressionBlock
              / (Float64) samplesPerCompressionBlock));

      endOfSampleInChunk = sizeOfSamplesInChunk + chunkOffset;
      sampleFirstPartLength = endOfSampleInChunk
          - dataOffset; // the first piece length = maxlen - start
      remainingLength = readLength
          - sampleFirstPartLength; // the read - first piece is either <= 0 or the remaining amount.

      if ((remainingLength > 0)
          && (sampleFirstPartLength > 0)) // this packet is split across chunks
      {
        //              s_printf("mediaSampleNumber = %" _S32BITARG_ " is compressed and split across chunks first part = %" _S32BITARG_ " remaining = %" _S32BITARG_ "\n",mediaSampleNumber, readLength, remainingLength);
        readLength = (UInt16) sampleFirstPartLength;
      } else {   // this is still needed. For some movies the compressed split packet calculation doesn't match the simple dataOffset calc below --a problem with sampleOffsetInChunk
//              s_printf("compressed but not split across chunks \n");
//              s_printf("chunkOffset = %" _S32BITARG_ "  ",chunkOffset);
//              s_printf("old dataOffset = %qd ",dataOffset);
        remainingLength = 0;
        dataOffset = chunkOffset + readOffset + (UInt64) (sampleOffsetInChunk
            * ((Float64) bytesPerCompressionBlock
                / (Float64) samplesPerCompressionBlock));
        //              s_printf("new dataOffset = %qd \n", dataOffset);
      }
...
#if TESTTIME
      //              s_printf("Read mediaSampleNumber = %" _S32BITARG_ " dataOffset = %qd  readLength = %" _S32BITARG_ " remaining = %" _S32BITARG_ "\n",mediaSampleNumber, dataOffset, readLength, remainingLength);
      ...
      *ppPacketBufOut += readLength;  // point to remainder of buffer;
      ...
      while (remainingLength
          > 0)  // loop if packet is split across more than just two chunks
      {
        //              s_printf("mediaSampleNumber = %" _S32BITARG_ " remaining read  = %" _S32BITARG_ "\n",mediaSampleNumber, remainingLength);

        readLength =
            (UInt16) remainingLength; // set the read to what is left
        chunkNumber++; // The rest of the sample is in the next N chunks
        if (!track->ChunkOffset(chunkNumber,
                                &chunkOffset)) // Get the Next chunk location
        {
        ...
        dataOffset =
            chunkOffset;    // the location of the data starting at the beginning of the chunk
        if (!track->GetSizeOfSamplesInChunk(chunkNumber,
                                            (UInt32 *) &sizeOfSamplesInChunk,
                                            NULL,
                                            NULL,
        ...
        if (sizeOfSamplesInChunk
            < remainingLength) // read in the whole chunk and keep going
        {
...
#if TESTTIME
        //                  s_printf("Read next parts mediaSampleNumber = %" _S32BITARG_ " dataOffset = %qd  readLength = %" _S32BITARG_ " remaining = %" _S32BITARG_ "\n",mediaSampleNumber, dataOffset, readLength, remainingLength);
        readStart = GetMicroseconds();
    ...
    } else {   // Media track sample not compressed
      dataOffset += readOffset;
  ...
  if ((trackRefIndex == -1) && (mediaSampleNumber < sampleNumber)) {
    ...
    if (htcb->fCachedHintTrackSample == NULL) {
      //          s_printf("create a cache buffer for %" _S32BITARG_ " of size %" _S32BITARG_ "\n",mediaSampleNumber,cacheHintSampleLen);
      ...
      //          s_printf("reallocate a cache buffer for %" _S32BITARG_ " of size %" _S32BITARG_ "\n",mediaSampleNumber,cacheHintSampleLen);
    ...
    if (htcb->fCachedHintTrackSampleNumber != mediaSampleNumber) {
      ...
      if (htcb->fCachedHintTrackSample != NULL) {
        //              s_printf("cache a hint sample sampleNumber %" _S32BITARG_ " readLength = %" _S32BITARG_ "\n",mediaSampleNumber,cacheHintSampleLen);
        ::memcpy(htcb->fCachedHintTrackSample,
                 buffOutPtr,
...
QTTrack::ErrorCode QTHintTrack::GetPacket(UInt32 sampleNumber,
                                          UInt16 packetNumber,
                                          char *buffer,
                                          UInt32 *length,
                                          Float64 *transmitTime,
                                          bool dropBFrames,
                                          bool dropRepeatPackets,
                                          UInt32 ssrc,
                                          QTHintTrack_HintTrackControlBlock *htcb) {
  // Temporary vars
  UInt16 tempInt16;
  ...
  DEEP_DEBUG_PRINT(("QTHintTrack::GetPacket - Building packet #%u in sample %"
                       _U32BITARG_
  ...
  err = this->GetSamplePacketPtr(&pSampleBuffer,
                                 sampleNumber,
                                 packetNumber,
                                 hdrData,
                                 *htcb);
  ...
  *transmitTime = (mediaTime * fMediaHeaderAtom->GetTimeScaleRecip())
      + (hdrData.relativePacketTransmissionTime
          * fMediaHeaderAtom->GetTimeScaleRecip());

#endif

  // remove negative start times so each track's transmit time is now 0 based
  if (hdrData.relativePacketTransmissionTime != 0
      && (fFirstTransmitTime == 0.0)) {
    fFirstTransmitTime = *transmitTime * -1;
  }
  *transmitTime +=
      fFirstTransmitTime; // prevent any negative transmission times by making all the streams 0 based.


  if (hdrData.hintFlags) {   //s_printf( "QTHintTrack::GetPacket hintFlags %lx\n", (SInt32)hdrData.hintFlags );
  }

  if (hdrData.hintFlags & kRepeatPacketMask
      && (dropRepeatPackets)) {   //s_printf( "QTHintTrack::GetPacket repeat packet dropped.\n" );
  ...
  DEEP_DEBUG_PRINT(("QTHintTrack::GetPacket - ..rtpTimestamp=%"
                       _U32BITARG_
                       "; rtpSequenceNumber=%u; transmitTime=%.2f\n", rtpTimestamp, hdrData.rtpSequenceNumber, *transmitTime));
  ...
  for (UInt32 fieldCount = 0; fieldCount < RTPMetaInfoPacket::kNumFields;
       fieldCount++) {
    //
    ...
    switch (fieldCount) {
      case RTPMetaInfoPacket::kPacketPosField: {
        SInt64 curPacketPos =
            CF::Utils::HostToNetworkSInt64(htcb->fCurrentPacketPosition);
        this->WriteMetaInfoField(RTPMetaInfoPacket::kPacketPosField,
      ...
      case RTPMetaInfoPacket::kTransTimeField: {
        SInt64 transmitTimeInMsec =
            CF::Utils::HostToNetworkSInt64((SInt64) (*transmitTime * 1000));
        this->WriteMetaInfoField(RTPMetaInfoPacket::kTransTimeField,
                                 htcb->fRTPMetaInfoFieldArray[fieldCount],
                                 &transmitTimeInMsec,
                                 sizeof(transmitTimeInMsec),
                                 &pPacketOutBuf);
        break;
      }
      ...
      case RTPMetaInfoPacket::kFrameTypeField: {
        ...
        this->WriteMetaInfoField(RTPMetaInfoPacket::kFrameTypeField,
                                 htcb->fRTPMetaInfoFieldArray[fieldCount],
                                 &theFrameType,
                                 sizeof(theFrameType),
                                 &pPacketOutBuf);
      ...
      case RTPMetaInfoPacket::kPacketNumField: {
        SInt64
            curPacketNum = CF::Utils::HostToNetworkSInt64(htcb->fCurrentPacketNumber);
        this->WriteMetaInfoField(RTPMetaInfoPacket::kPacketNumField,
                                 htcb->fRTPMetaInfoFieldArray[fieldCount],
                                 &curPacketNum,
                                 sizeof(curPacketNum),
                                 &pPacketOutBuf);
      ...
      case RTPMetaInfoPacket::kSeqNumField: {
        tempInt16 = htons(hdrData.rtpSequenceNumber);
        this->WriteMetaInfoField(RTPMetaInfoPacket::kSeqNumField,
                                 htcb->fRTPMetaInfoFieldArray[fieldCount],
                                 &tempInt16,
                                 sizeof(tempInt16),
                                 &pPacketOutBuf);
      ...
      case RTPMetaInfoPacket::kMediaDataField: {
        ...
        Assert(htcb->fRTPMetaInfoFieldArray[fieldCount]
                   == RTPMetaInfoPacket::kUncompressed);

        //
        // We don't have the data yet, so just write in the header
        this->WriteMetaInfoField(RTPMetaInfoPacket::kMediaDataField,
                                 htcb->fRTPMetaInfoFieldArray[fieldCount],
                                 NULL,
                                 0,
                                 &pPacketOutBuf);
  ...
  TEMP_PRINT_TWO(
      "QTHintTrack::GetPacket Building packet %li ; hdrData.dataEntryCount %li .\n",
      (SInt32) packetNumber,
      (SInt32) hdrData.dataEntryCount);
  ...
  for (curEntry = 0; curEntry < hdrData.dataEntryCount; curEntry++) {
    ...
    if (*pSampleBuffer == 0x02) {
      ...
      //          s_printf("Found unknown RTP data table type!\n");
      Assert(0);
    }
  ...
  DEEP_DEBUG_PRINT(("QTHintTrack::GetPacket - ..Packet length is %"
                       _U32BITARG_
                       " bytes.\n", packetSize));
  ...
  *length = packetSize;
  ...
  if (htcb->fRTPMetaInfoFieldArray != NULL) {
    ...
    if (htcb->fRTPMetaInfoFieldArray[RTPMetaInfoPacket::kMediaDataField]
        == RTPMetaInfoPacket::kFieldNotUsed)
...
void QTHintTrack::WriteMetaInfoField(RTPMetaInfoPacket::FieldIndex inFieldIndex,
  ...
  if (inFieldID == RTPMetaInfoPacket::kUncompressed) {
    ...
    RTPMetaInfoPacket::FieldName
        theName = htons(RTPMetaInfoPacket::GetFieldNameForIndex(inFieldIndex));
