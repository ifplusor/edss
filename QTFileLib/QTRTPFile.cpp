...
const RTPMetaInfoPacket::FieldID QTRTPFile::kMetaInfoFields[] =
    {RTPMetaInfoPacket::kUncompressed, RTPMetaInfoPacket::kUncompressed,
     RTPMetaInfoPacket::kUncompressed, RTPMetaInfoPacket::kUncompressed,
     RTPMetaInfoPacket::kUncompressed, RTPMetaInfoPacket::kUncompressed};
...
//
CF::Core::Mutex *QTRTPFile::gFileCacheMutex;
CF::Core::Mutex *QTRTPFile::gFileCacheAddMutex;
QTRTPFile::RTPFileCacheEntry *QTRTPFile::gFirstFileCacheEntry = NULL;

void QTRTPFile::Initialize() {
  QTRTPFile::gFileCacheMutex = new CF::Core::Mutex();
...
QTRTPFile::ErrorCode QTRTPFile::new_QTFile(char const *filePath,
  ...
  // General vars
  CF::Core::MutexLocker fileCacheAddMutex(QTRTPFile::gFileCacheAddMutex);
...
void QTRTPFile::delete_QTFile(QTFile *theQTFile) {
  ...
  for (listEntry = QTRTPFile::gFirstFileCacheEntry; listEntry != NULL;
...
void QTRTPFile::AddFileToCache(char const *inFilename,
  ...
  (*newListEntry)->InitMutex = new CF::Core::Mutex();
  ...
  if (QTRTPFile::gFirstFileCacheEntry == NULL) {
    ...
    for (listEntry = lastListEntry = QTRTPFile::gFirstFileCacheEntry;
         listEntry != NULL; listEntry = listEntry->NextEntry) {
      //
...
char *QTRTPFile::GetSDPFile(int *sdpFileLength) {
  ...
  for (curEntry = fFirstTrack;
    ...
    if (curEntry->HintTrack->GetSDPFileLength(&trackSDPLength)
        != QTTrack::errNoError)
      continue;
  ...
  if (haveGlobalSDPAtom) {
    fFile->Read(globalSDPTOCEntry->AtomDataPos + 4,
                pSDPFile,
                (UInt32) (globalSDPTOCEntry->AtomDataLength - 4));
...
QTRTPFile::ErrorCode QTRTPFile::AddTrack(UInt32 trackID, bool useRandomOffset) {
  ...
  {
    CF::Core::MutexLocker locker(fFile->GetMutex());
    //
  ...
  trackEntry->FileSequenceNumberRandomOffset =
      trackEntry->HintTrack->GetRTPSequenceNumberRandomOffset();
  trackEntry->FileTimestampRandomOffset =
      trackEntry->HintTrack->GetRTPTimestampRandomOffset();
...
void QTRTPFile::SetTrackQualityLevel(RTPTrackListEntry *inEntry,
                                     UInt32 inNewQualityLevel) {
...
void QTRTPFile::SetTrackRTPMetaInfo(UInt32 TrackID,
                                    RTPMetaInfoPacket::FieldID *inFieldArray,
                                    bool isVideo) {
  // General vars
  RTPTrackListEntry *trackEntry = NULL;
...
// -------------------------------------
void QTRTPFile::AllocatePrivateBuffers(UInt32 inUnitSizeInK,
  ...
  fFCB->AdjustDataBufferBitRate(inUnitSizeInK,
                                bitRate,
                                inNumBuffSizeUnits,
...
// Packet functions
//
QTRTPFile::ErrorCode QTRTPFile::Seek(Float64 seekToTime,
                                     Float64 maxBackupTime) {
  //fHasRTPMetaInfoFieldArray = true;
  // General vars
  ...
  // sample.
  for (listEntry = fFirstTrack; listEntry != NULL;
       listEntry = listEntry->NextTrack) {
    // General vars
    SInt32 mediaTime;
    ...
    if (!listEntry->HintTrack->GetSampleNumberFromMediaTime(mediaTime,
                                                            &newSampleNumber,
                                                            &listEntry->HTCB->fsttsSTCB))
      continue;   // This track is probably done playing.

    //
    // Find the nearest (moving backwards in time) keyframe.
    listEntry->HintTrack->GetPreviousSyncSample(newSampleNumber,
                                                &newSyncSampleNumber);
    ...
    if (!listEntry->HintTrack->GetSampleMediaTime(newSyncSampleNumber,
                                                  &newSampleMediaTime,
                                                  &listEntry->HTCB->fsttsSTCB))
      return errInvalidQuickTimeFile;

    newSampleMediaTime += listEntry->HintTrack->GetFirstEditMediaTime();
    newSampleTime = (Float64) newSampleMediaTime
        * listEntry->HintTrack->GetTimeScaleRecip();
  ...
  for (listEntry = fFirstTrack; listEntry != NULL;
    ...
    // Compute the media time and get the sample at that time.
    SInt32
        mediaTime = (SInt32) (fSeekTime * listEntry->HintTrack->GetTimeScale());
    mediaTime -= listEntry->HintTrack->GetFirstEditMediaTime();
    if (mediaTime < 0)
      mediaTime = 0;
...
QTRTPFile::ErrorCode QTRTPFile::SeekToPacketNumber(UInt32 inTrackID,
  ...
  for (RTPTrackListEntry *listEntry = fFirstTrack; listEntry != NULL;
       listEntry = listEntry->NextTrack) {
    if (!listEntry->IsTrackActive)
      continue;
...
QTRTPFile::ErrorCode QTRTPFile::ScanToCorrectPacketNumber(UInt32 inTrackID,
                                                          UInt64 inPacketNumber) {
  int theLen = 0;
  for (UInt32 packetCount = 0; packetCount < 100; packetCount++) {
    if ((fLastPacketTrack != NULL) && (fLastPacketTrack->TrackID == inTrackID)
        && (fLastPacketTrack->HTCB->fCurrentPacketNumber == inPacketNumber)) {
      UInt32 newSampleMediaTime;
      //
      // Figure out what time this sample is at.
      if (!fLastPacketTrack->HintTrack->GetSampleMediaTime(fLastPacketTrack->CurSampleNumber,
                                                           &newSampleMediaTime,
                                                           &fLastPacketTrack->HTCB->fsttsSTCB))
        return errInvalidQuickTimeFile;

      newSampleMediaTime +=
          fLastPacketTrack->HintTrack->GetFirstEditMediaTime();
      fRequestedSeekTime = (Float64) newSampleMediaTime
          * fLastPacketTrack->HintTrack->GetTimeScaleRecip();
...
UInt32 QTRTPFile::GetSeekTimestamp(UInt32 trackID) {
  ...
  DEEP_DEBUG_PRINT(("Calculating RTP timestamp for track #%"
                       _U32BITARG_
  ...
  if (trackEntry->CurPacket) //  we have the packet
  {
    ...
    mediaTime =
        (UInt32) (fRequestedSeekTime * trackEntry->HintTrack->GetTimeScale());
    if (trackEntry->HintTrack->GetRTPTimescale()
        == trackEntry->HintTrack->GetTimeScale())
      rtpTimestamp = mediaTime;
    else
      rtpTimestamp = (UInt32) (mediaTime
          * (trackEntry->HintTrack->GetRTPTimescale()
              * trackEntry->HintTrack->GetTimeScaleRecip()));

    //
    // Add the appropriate offsets.
    rtpTimestamp += trackEntry->BaseTimestampRandomOffset
        + trackEntry->FileTimestampRandomOffset;
  }

  //
  // Return the RTP timestamp.
  DEEP_DEBUG_PRINT(("..timestamp=%"
                       _U32BITARG_
                       "\n", rtpTimestamp));
...
Float64 QTRTPFile::GetNextPacket(char **outPacket, int *outPacketLength) {
  ...
  *outPacketLength = 0;
  ...
  firstPacket->LastSequenceNumber =
      ntohs(*(UInt16 *) ((char *) firstPacket->CurPacket + 2));
  firstPacket->LastSequenceNumber -=
      (UInt16) (firstPacket->BaseSequenceNumberRandomOffset
          + firstPacket->FileSequenceNumberRandomOffset
          + firstPacket->SequenceNumberAdditive);
...
SInt32 QTRTPFile::GetMovieHintType() {
  ...
  for (RTPTrackListEntry *listEntry = fFirstTrack; listEntry != NULL;
    ...
    if (trackHintType
        > 0) // always set movie hint type to unoptimized if a track is unoptimized
    {
...
bool QTRTPFile::PrefetchNextPacket(RTPTrackListEntry *trackEntry, bool doSeek) {
  ...
  while (getPacketErr == QTTrack::errIsSkippedPacket) {
    ...
    if ((trackEntry->CurPacketNumber > trackEntry->NumPacketsInThisSample)
      ...
      if (trackEntry->HintTrack->IsSyncSample(trackEntry->CurSampleNumber, 0)) {
        ...
        if (trackEntry->NextSyncSampleNumber != trackEntry->CurSampleNumber)
          trackEntry->NextSyncSampleNumber =
              0;   // this must have been obsolete
      }
      ...
      if (trackEntry->QualityLevel == kKeyFramesOnly) {
        trackEntry->HintTrack->GetNextSyncSample(trackEntry->CurSampleNumber,
                                                 &trackEntry->NextSyncSampleNumber);
        if (!fHasRTPMetaInfoFieldArray && !fWasLastSeekASeekToPacketNumber) {
          fNumSkippedSamples +=
              trackEntry->NextSyncSampleNumber - trackEntry->CurSampleNumber;
        ...
        if (!trackEntry->HintTrack->IsSyncSample(trackEntry->CurSampleNumber,
                                                 0)) {
          //
          ...
          if (trackEntry->CurSampleNumber > trackEntry->LastSyncSampleNumber) {
            Assert(trackEntry->NextSyncSampleNumber
                       != trackEntry->LastSyncSampleNumber);

            UInt32 percentageIn =
                (trackEntry->CurSampleNumber - trackEntry->LastSyncSampleNumber)
                    * 100 /
                    (trackEntry->NextSyncSampleNumber
                        - trackEntry->LastSyncSampleNumber);
    ...
    while (trackEntry->NumPacketsInThisSample == 0) {
      if (trackEntry->HintTrack->GetNumPackets(trackEntry->CurSampleNumber,
                                               &trackEntry->NumPacketsInThisSample,
                                               trackEntry->HTCB)
          != QTTrack::errNoError)
        return false;
    ...
    packetTimer.Start();
#endif
    getPacketErr = trackEntry->HintTrack->GetPacket(trackEntry->CurSampleNumber,
                                                    trackEntry->CurPacketNumber,
                                                    trackEntry->CurPacket,
                                                    &trackEntry->CurPacketLength,
                                                    &trackEntry->CurPacketTime,
                                                    (trackEntry->QualityLevel
                                                        >= kNoBFrames),
                                                    fDropRepeatPackets,
                                                    trackEntry->SSRC,
                                                    trackEntry->HTCB);
...
#if QT_PROFILE
    packetTimer.Stop();
    s_printf("GetPacket sample time %li NumPackets: %li Packet fetched %li. len: %li\n",
  ...
  if (doSeek || (trackEntry->QualityLevel > kAllPackets))
    trackEntry->SequenceNumberAdditive +=
        (trackEntry->LastSequenceNumber + 1) - ntohs(*pSequenceNumber);

  *pSequenceNumber = htons((SInt16) (((SInt32) ntohs(*pSequenceNumber))
      + trackEntry->BaseSequenceNumberRandomOffset
      + trackEntry->FileSequenceNumberRandomOffset
      + trackEntry->SequenceNumberAdditive));
  *pTimestamp = htonl(ntohl(*pTimestamp) + trackEntry->BaseTimestampRandomOffset
                          + trackEntry->FileTimestampRandomOffset);
