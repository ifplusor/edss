...
//
QTAtom_stss::QTAtom_stss(QTFile *File,
...
bool QTAtom_stss::Initialize() {
  ...
  if (initSucceeds) {
    ...
    if (((PointerSizedInt) fSyncSampleTable & (PointerSizedInt) 0x3) == 0)
      ...
      fTable = (UInt32 *) (((PointerSizedInt) fSyncSampleTable + 4)
          & ~((PointerSizedInt) 0x3));

    initSucceeds =
        ReadBytes(stssPos_SampleTable, (char *) fTable, fNumEntries * 4);
...
// Accessors
//
void QTAtom_stss::PreviousSyncSample(UInt32 SampleNumber,
...
void QTAtom_stss::NextSyncSample(UInt32 SampleNumber,
                                 UInt32 *SyncSampleNumber) {
  //
  // We assume that we won't find an answer
...
void QTAtom_stss::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_stss::DumpAtom - ..Number of sync sample entries: %"
                  _S32BITARG_
                  "\n", fNumEntries));
}
