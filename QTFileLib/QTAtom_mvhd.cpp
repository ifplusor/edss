...
//
QTAtom_mvhd::QTAtom_mvhd(QTFile *File,
...
bool QTAtom_mvhd::Initialize() {
  ...
  if (0 == fVersion) {
    ...
    if (fTOCEntry.AtomDataLength != 100) {
      DEEP_DEBUG_PRINT(("QTAtom_mvhd::Initialize failed. Expected AtomDataLength == 100 version: %d AtomDataLength: %"
                           _64BITARG_
    ...
    if (fTOCEntry.AtomDataLength != 112) {
      DEEP_DEBUG_PRINT(("QTAtom_mvhd::Initialize failed. Expected AtomDataLength = 112 version: %d AtomDataLength: %"
                           _64BITARG_
                           "u\n", fVersion, fTOCEntry.AtomDataLength));
      return false;
    }
...
void QTAtom_mvhd::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_mvhd::DumpAtom - ..Creation date: %s", s_asctime(
