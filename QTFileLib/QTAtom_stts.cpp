...
//
QTAtom_stts::QTAtom_stts(QTFile *File,
...
// Accessors
//
bool QTAtom_stts::MediaTimeToSampleNumber(UInt32 MediaTime,
                                          UInt32 *SampleNumber,
  ...
  if (STCB == nullptr) {
    //      s_printf("QTAtom_stts::MediaTimeToSampleNumber  ( STCB == nullptr ) \n");
  ...
  if (MediaTime < STCB->fMTtSN_CurMediaTime) {
    //      s_printf(" QTAtom_stts::MediaTimeToSampleNumber RESET \n");
    STCB->Reset();
  }
  //
  ...
  for (; STCB->fMTtSN_CurEntry < fNumEntries; STCB->fMTtSN_CurEntry++) {
    ...
    memcpy(&SampleDuration,
           fTimeToSampleTable + (STCB->fMTtSN_CurEntry * 8) + 4,
           4);
    SampleDuration = ntohl(SampleDuration);

    //
    // Can we skip over this entry?
    if (STCB->fMTtSN_CurMediaTime + (SampleCount * SampleDuration)
        < MediaTime) {
    ...
    if (SampleNumber != nullptr) {
      *SampleNumber = STCB->fMTtSN_CurSample;
      if (SampleDuration > 0)
        *SampleNumber +=
            (MediaTime - STCB->fMTtSN_CurMediaTime) / SampleDuration;
      result = true;
      break;
    }
  }
...
bool QTAtom_stts::SampleNumberToMediaTime(UInt32 SampleNumber,
                                          UInt32 *MediaTime,
                                          QTAtom_stts_SampleTableControlBlock *STCB) {
  // General vars
  UInt32 SampleCount, SampleDuration;
  ...
  if (STCB->fGetSampleMediaTime_SampleNumber == SampleNumber) {
    //      s_printf("QTTrack::GetSampleMediaTime cache hit SampleNumber %" _S32BITARG_ " \n", SampleNumber);
  ...
  if (SampleNumber < STCB->fSNtMT_CurSample) {
    //      s_printf(" QTAtom_stts::SampleNumberToMediaTime reset \n");
    STCB->Reset();
  }
  //
  // Linearly search through the sample table until we find the sample
  ...
  for (; STCB->fSNtMT_CurEntry < fNumEntries; STCB->fSNtMT_CurEntry++) {
    ...
    if (MediaTime != nullptr)
      *MediaTime = STCB->fSNtMT_CurMediaTime
          + ((SampleNumber - STCB->fSNtMT_CurSample) * SampleDuration);
...
void QTAtom_stts::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_stts::DumpAtom - ..Number of TTS entries: %"
                  _S32BITARG_
                  "\n", fNumEntries));
}
...
void QTAtom_stts::DumpTable() {
  ...
  s_printf("\n");
  s_printf("  Entry Num.   Sample Count   Sample Duration\n");
  s_printf("  ----------   ------------   ---------------\n");
  ...
  for (UInt32 CurEntry = 0; CurEntry < fNumEntries; CurEntry++) {
    ...
    // Print out a listing.
    s_printf("  %10"   _U32BITARG_   " : %10"   _U32BITARG_   "  %10"   _U32BITARG_   "\n",
                CurEntry,
                SampleCount,
                SampleDuration);
  }
...
// Constructors and destructors
//
QTAtom_ctts::QTAtom_ctts(QTFile *File,
                         QTFile::AtomTOCEntry *TOCEntry,
                         bool Debug,
                         bool DeepDebug)
    : QTAtom(File, TOCEntry, Debug, DeepDebug),
...
// Accessors
//
bool QTAtom_ctts::MediaTimeToSampleNumber(UInt32 MediaTime,
                                          UInt32 *SampleNumber,
  ...
  // Use the default STCB if one was not passed in to us.
  if (STCB == nullptr) {
    //      s_printf("QTAtom_ctts::MediaTimeToSampleNumber  ( STCB == nullptr ) \n");
  ...
  // Reconfigure the STCB if necessary.
  if (MediaTime < STCB->fMTtSN_CurMediaTime) {
    //      s_printf(" QTAtom_ctts::MediaTimeToSampleNumber RESET \n");
    STCB->Reset();
  }
  //
  // Linearly search through the sample table until we find the sample
...
bool QTAtom_ctts::SampleNumberToMediaTimeOffset(UInt32 SampleNumber,
  ...
  // Reconfigure the STCB if necessary.
  if (SampleNumber < STCB->fSNtMT_CurSample) {
    //      s_printf(" QTAtom_ctts::SampleNumberToMediaTime reset \n");
    STCB->Reset();
  }
  //
  // Linearly search through the sample table until we find the sample
...
void QTAtom_ctts::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_ctts::DumpAtom - ..Number of CTTS entries: %"
                  _S32BITARG_
                  "\n", fNumEntries));
}
...
void QTAtom_ctts::DumpTable() {
  ...
  s_printf("\n");
  s_printf("  Entry Num.   Sample Count   Sample Offset\n");
  s_printf("  ----------   ------------   ---------------\n");
