...
//
QTAtom_mdhd::QTAtom_mdhd(QTFile *File,
...
bool QTAtom_mdhd::Initialize() {
  ...
  if (0 == fVersion) {
    ...
    if (fTOCEntry.AtomDataLength != 24) {
      DEEP_DEBUG_PRINT(("QTAtom_mdhd::Initialize failed. Expected AtomDataLength == 24 version: %d AtomDataLength: %"
                           _64BITARG_
    ...
    if (fTOCEntry.AtomDataLength != 36) {
      DEEP_DEBUG_PRINT(("QTAtom_mdhd::Initialize failed. Expected AtomDataLength == 36 version: %d AtomDataLength: %"
                           _64BITARG_
                           "u\n", fVersion, fTOCEntry.AtomDataLength));
      return false;
    }
...
void QTAtom_mdhd::DumpAtom() {
  ...
  DEBUG_PRINT(("QTAtom_mdhd::DumpAtom - ..Creation date: %s",
      s_asctime(s_gmtime(&unixCreationTime, &timeResult), buffer, sizeof(buffer))));
}
