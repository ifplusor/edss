...
class QTHintTrackRTPHeaderData {
 ...
 public:
  ...
  UInt32 tlvSize;
  SInt32
      tlvTimestampOffset; //'rtpo' TLV which is the timestamp offset for this packet
};
...
class QTHintTrack_HintTrackControlBlock {
 ...
 public:
  ...
  void SetupRTPMetaInfo(RTPMetaInfoPacket::FieldID *inFieldArray,
                        bool isVideo) {
    Assert(fRTPMetaInfoFieldArray == NULL);
    fRTPMetaInfoFieldArray = inFieldArray;
    fIsVideo = isVideo;
  }
  ...
  UInt16
      fLastPacketNumberFetched;   // for optimizing Getting a packet from a cached sample
  char *
      fPointerToNextPacket;       // after we get one, we point the next at this...
...
class QTHintTrack : public QTTrack {
 ...
 public:
  ...
  inline UInt64 GetTotalRTPBytes() {
    return fHintInfoAtom ? fHintInfoAtom->GetTotalRTPBytes() : 0;
  }
  inline UInt64 GetTotalRTPPackets() {
    return fHintInfoAtom ? fHintInfoAtom->GetTotalRTPPackets() : 0;
  }

  inline UInt32 GetFirstRTPTimestamp() { return fFirstRTPTimestamp; }
  inline void SetAllowInvalidHintRefs(bool inAllowInvalidHintRefs) {
    fAllowInvalidHintRefs = inAllowInvalidHintRefs;
  }
  ...
  inline ErrorCode GetSampleData(QTHintTrack_HintTrackControlBlock *htcb,
                                 char **buffPtr,
                                 char **ppPacketBufOut,
                                 UInt32 sampleNumber,
                                 UInt16 packetNumber,
 ...
 protected:
  ...
  inline QTTrack::ErrorCode GetSamplePacketPtr(char **samplePacketPtr,
                                               UInt32 sampleNumber,
                                               UInt16 packetNumber,
                                               QTHintTrackRTPHeaderData &hdrData,
                                               QTHintTrack_HintTrackControlBlock &htcb);
  inline void GetSamplePacketHeaderVars(char *samplePacketPtr,
                                        char *maxBuffPtr,
                                        QTHintTrackRTPHeaderData &hdrData);
};
