...
//
QTAtom_dref::QTAtom_dref(QTFile *File,
...
bool QTAtom_dref::Initialize() {
  ...
  if (fNumRefs > 0) {
    ...
    for (UInt32 CurRef = 0; CurRef < fNumRefs; CurRef++) {
      ...
      fRefs[CurRef].ReferenceType =
          FOUR_CHARS_TO_INT('N', 'U', 'L', 'L'); // nullptr
      ...
      if (fRefs[CurRef].Data == nullptr) {
        ...
        //
        // Read the entry data.
        ReadBytes(refPos + drefRefPos_Data,
                  fRefs[CurRef].Data,
                  fRefs[CurRef].DataLength);
...
// Read functions.
//
bool QTAtom_dref::Read(UInt32 RefID,
                       UInt64 Offset,
                       char *const Buffer,
                       UInt32 Length,
  ...
  //  s_printf("QTAtom_dref::Read Offset = %qd, Length = %" _S32BITARG_ " \n", Offset, Length);
  //
  ...
  if (!Entry->IsFileOpen) {
    ...
    if ((p = strrchr(MoviePath, QT_PATH_SEPARATOR)) == nullptr) {
      ...
      memcpy(NewPath + MoviePathClippedLength,
             AliasPath,
             strlen(AliasPath) + 1);
    }
...
void QTAtom_dref::DumpAtom() {
  ...
  for (UInt32 CurRef = 1; CurRef <= fNumRefs; CurRef++)
    DEBUG_PRINT(("QTAtom_dref::DumpAtom - ..Ref #%"
                    _U32BITARG_
                    " is in file: %s\n", CurRef, IsRefInThisFile(CurRef) ? "yes"
                                                                         : "no"));
}
...
//
char *QTAtom_dref::ResolveAlias(char *const AliasData,
                                UInt32 /* AliasDataLength */) {
  // General vars
  ...
  for (int loopCount = kMaxMark - 1; loopCount >= 0; loopCount--) {
    ...
    if (((short) ntohs(AliasVarInfo->what) == kAbsPath)
        || ((short) ntohs(AliasVarInfo->what) == kEndMark))
      break;

    //
    // Otherwise we need to move to the next data unit.
    AliasVarInfo = (varInfo *) ((char *) AliasVarInfo
        + ((ntohs(AliasVarInfo->len) + 1) & ~1) + 4 /* header size */);
  }
