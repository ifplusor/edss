...
class UserAgentParser {
 public:
  ...
  void Parse(CF::StrPtrLen *inStream);

  CF::StrPtrLen *GetUserID() { return &(fFieldData[eQtid].fData); };
  CF::StrPtrLen *GetUserVersion() { return &(fFieldData[eQtver].fData); };
  CF::StrPtrLen *GetUserLanguage() { return &(fFieldData[eLang].fData); };
  CF::StrPtrLen *GetrUserOS() { return &(fFieldData[eOs].fData); };
  CF::StrPtrLen *GetUserOSVersion() { return &(fFieldData[eOsver].fData); };
  CF::StrPtrLen *GetUserCPU() { return &(fFieldData[eCpu].fData); };

  UserAgentParser(CF::StrPtrLen *inStream) {
    if (inStream != nullptr)
      Parse(inStream);
  }
