...
bool RTCPCompressedQTSSPacket::ParseCompressedQTSSPacket(UInt8 *inPacketBuffer,
                                                         UInt32 inPacketLength) {
  if (!this->ParseAPPPacket(inPacketBuffer, inPacketLength))
    return false;

  if (this->GetAppPacketName()
      != RTCPCompressedQTSSPacket::kCompressedQTSSPacketName)
    return false;
...
bool RTCPCompressedQTSSPacket::ParseAPPData(UInt8 *inPacketBuffer,
                                            UInt32 inPacketLength) {
  ...
  APPEND_TO_DUMP_ARRAY("H_ssrc = %"
                           _U32BITARG_
                           ", ", this->GetPacketSSRC());
  APPEND_TO_DUMP_ARRAY("H_src_ID = %"
                           _U32BITARG_
                           ", ", this->GetQTSSReportSourceID());
  ...
  while (bytesRemaining >= 4) //items must be at least 32 bits
  {
    ...
    UInt32 theHeader =
        ntohl(CF::Utils::GetUInt32FromMemory((UInt32 *) &qtssDataBuffer[kQTSSItemTypeOffset]));
    UInt16 itemType =
        (UInt16) ((theHeader & kQTSSItemTypeMask) >> kQTSSItemTypeShift);
    UInt8 itemVersion =
        (UInt8) ((theHeader & kQTSSItemVersionMask) >> kQTSSItemVersionShift);
    ...
    qtssDataBuffer +=
        sizeof(UInt32);   //advance past the above UInt16's & UInt8's (point it at the actual item data)
    ...
    switch (itemType) {
      ...
      {
        fReceiverBitRate =
            ntohl(CF::Utils::GetUInt32FromMemory((UInt32 *) qtssDataBuffer));
        qtssDataBuffer += sizeof(fReceiverBitRate);
        APPEND_TO_DUMP_ARRAY(", rcvr_bit_rate=%"
                                 _U32BITARG_
                                 "", fReceiverBitRate);
      }
        break;
      ...
      case TW0_CHARS_TO_INT('d', 'l'): //'dl':    //'bdly':
      {
        ...
        APPEND_TO_DUMP_ARRAY(", avg_buf_delay=%u",
                             fAverageBufferDelayMilliseconds);
      }
        break;
      ...
      {
        fNumEyes = ntohl(CF::Utils::GetUInt32FromMemory((UInt32 *) qtssDataBuffer));
        qtssDataBuffer += sizeof(fNumEyes);
        APPEND_TO_DUMP_ARRAY(", eyes=%"
                                 _U32BITARG_
                                 "", fNumEyes);
        ...
        if (itemLengthInBytes >= 2) {
          ...
          APPEND_TO_DUMP_ARRAY(", eyes_actv=%"
                                   _U32BITARG_
                                   "", fNumEyesActive);
        }
        if (itemLengthInBytes >= 3) {
          ...
          APPEND_TO_DUMP_ARRAY(", eyes_pausd=%"
                                   _U32BITARG_
                                   "", fNumEyesPaused);
        }
      }
      ...
      {
        fTotalPacketsReceived =
            ntohl(CF::Utils::GetUInt32FromMemory((UInt32 *) qtssDataBuffer));
        qtssDataBuffer += sizeof(fTotalPacketsReceived);
        APPEND_TO_DUMP_ARRAY(", pckts_rcvd=%"
                                 _U32BITARG_
                                 "", fTotalPacketsReceived);
      }
        break;
      ...
      {
        fOverbufferWindowSize =
            ntohl(CF::Utils::GetUInt32FromMemory((UInt32 *) qtssDataBuffer));
        qtssDataBuffer += sizeof(fOverbufferWindowSize);
        APPEND_TO_DUMP_ARRAY(", ovr_buffr_windw_siz=%"
                                 _U32BITARG_
                                 "", fOverbufferWindowSize);
      }
        break;
