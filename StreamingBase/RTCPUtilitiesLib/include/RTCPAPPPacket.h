...
#define APPEND_TO_DUMP_ARRAY(f, v) { \
  if (fDebug && mDumpArray != NULL) { \
    (void) s_snprintf(mDumpArray, kmDumpArraySize, f, v); \
    fDumpReport.Put(mDumpArray); \
  } \
} \
...
class RTCPAPPPacket : public RTCPPacket {
 ...
 public:
  ...
  virtual void Dump();
  virtual bool ParseAPPPacket(UInt8 *inPacketBuffer,
                              UInt32 inPacketLength); //default app header check
  virtual bool ParseAPPData(UInt8 *inPacketBuffer,
                            UInt32 inPacketLength) { return false; }; //derived class implements
  ...
  enum {
    kAppSSRCOffset = 4,
    kAppNameOffset =
    8, //byte offset to four char App identifier               //All are UInt32
  ...
  char *mDumpArray;
  CF::StrPtrLenDel mDumpArrayStrDeleter;
  CF::ResizeableStringFormatter fDumpReport;
  bool fDebug;

 private:
  virtual bool ParseAPPPacketHeader(UInt8 *inPacketBuffer,
...
inline FourCharCode RTCPAPPPacket::GetAppPacketName(char *outName, UInt32 len) {
  ...
  UInt32
      packetName = (UInt32) (*(UInt32 *) &(GetPacketBuffer()[kAppNameOffset]));
