...
bool RTCPAckPacket::ParseAckPacket(UInt8 *inPacketBuffer,
...
bool RTCPAckPacket::ParseAPPData(UInt8 *inPacketBuffer, UInt32 inPacketLength) {
  ...
  Assert(inPacketLength == (UInt32) ((this->GetPacketLength() * 4))
      + RTCPPacket::kRTCPHeaderSizeInBytes);
...
bool RTCPAckPacket::IsAckPacketType() {
  ...
  //  if ( theAppType == kAckPacketAlternateName ) s_printf("ack\n");
  //  if ( theAppType == kAckPacketName ) s_printf("qtack\n");
...
void RTCPAckPacket::Dump() {
  ...
  CF::CharArrayDeleter deleter(maskBytesBuffer);
  maskBytesBuffer[0] = 0;
  ...
  for (UInt32 maskCount = 0; maskCount < theAckMaskSizeInBits; maskCount++) {
    if (this->IsNthBitEnabled(maskCount)) {
      s_sprintf(&maskBytesBuffer[::strlen(maskBytesBuffer)],
                   "%"   _U32BITARG_   ", ",
                   theSeqNum + 1 + maskCount);
    }
  }
  Assert(::strlen(maskBytesBuffer) < numBufferBytes);
  s_printf(" H_name=%s H_seq=%u H_len=%u mask_size=%"   _U32BITARG_   " seq_nums_bit_set=%s\n",
              name,
              theSeqNum,
              thePacketLen,
              theAckMaskSizeInBits,
              maskBytesBuffer);
