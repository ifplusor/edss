...
void RTCPNaduPacket::GetTestPacket(CF::StrPtrLen *resultPtr) {
  /*
  ...
  *(theWriter++) =
      htonl(0x01000001);     // fraction lost | cumulative num packets lost 1% , 1 packet
  *(theWriter++) =
      htonl(0x0000361A);     // extended highest seq number received = 13850
  *(theWriter++) = htonl(0x00C7ED4D);     // interarrival jitter = 13102413
  *(theWriter++) = htonl(0x00000000);     // LSR last sender report = 0
  *(theWriter++) =
      htonl(0x04625238);     // Delay since last SR (DLSR) = 73552440 (garbage)
  ...
  *(theWriter++) =
      htonl(0x00000000);     // NULL item = end of list + 32bit padding



  *(theWriter++) =
      htonl(0x80CC0000);     // 1 APP packet header, needs len -> assigned beow

  UInt32 *appPacketLenStart = theWriter;

  *(theWriter++) = htonl(FOUR_CHARS_TO_INT('S', 'S', 'R', 'C')); //nadu ssrc
  *(theWriter++) =
      htonl(FOUR_CHARS_TO_INT('P', 'S', 'S', '0')); //nadu app packet name
  ...
  *(theWriter++) = htonl(0xFFFFAD9C); //nun | fbs= 31 | 44444
  ...
  UInt16 packetLenInWords =
      htons(((UInt32 *) theWriter - (UInt32 *) appPacketLenStart));

  *packetLenOffsetPtr = packetLenInWords;
  s_printf("packetLenInWords =%lu\n", ntohs(packetLenInWords));
...
bool RTCPNaduPacket::ParseNaduPacket(UInt8 *inPacketBuffer,
                                     UInt32 inPacketLength) {
...
bool RTCPNaduPacket::ParseAPPData(UInt8 *inPacketBuffer,
                                  UInt32 inPacketLength) {
  ...
  UInt32
      *naduDataBuffer = (UInt32 *) (this->GetPacketBuffer() + kNaduDataOffset);
  ...
  if (0 != (wordsLen
      % 3))// blocks are 3x32bits so there is a bad block somewhere.
    return false;
...
void RTCPNaduPacket::DumpNaduPacket() {
  ...
  s_printf(" H_app_packet_name = %s, ", printName);
  ...
  for (; count < fNumBlocks; count++) {
    ...
    s_printf("              ");
    s_printf("RTCP APP NADU Report[%"   _U32BITARG_   "] ", ssrcIndex);
    s_printf("h_ssrc = %"   _U32BITARG_, ssrc);
    s_printf(", h_playoutdelay = %u", playoutDelay);
    s_printf(", h_sequence_num = %u", nsn);
    s_printf(", h_nun_unit_num = %u", nun);
    s_printf(", h_fbs_free_buf = %u", fbs);
...
UInt16 RTCPNaduPacket::GetPlayOutDelay(SInt32 index) {
  ...
  UInt32 *blockBufferPtr = fNaduDataBuffer + (index * 3);
  UInt16 delay = (UInt16)
      ((ntohl(*(UInt32 *) &blockBufferPtr[kOffsetNaduPlayoutDelay])
          & kPlayoutMask) >> 16);
...
/* class NaduReport */
NaduReport::NaduReport(UInt8 *inPacketBuffer,
                       UInt32 inPacketLength,
                       UInt32 id) {
...
void NaduList::Initialize(UInt32 listSize) {
  ...
  ::memset((void *) fNaduReportList,
           0,
           sizeof(NaduReport *) * listSize); //initialize ptr array with 0.
  fListSize = listSize;
...
bool NaduList::AddReport(UInt8 *inPacketBuffer,
                         UInt32 inPacketLength,
                         UInt32 *outID) {
  if (NULL == fNaduReportList)
    return false;
  ...
  fNaduReportList[index] =
      new NaduReport(inPacketBuffer, inPacketLength, resultID);
...
UInt32 NaduList::LastReportedFreeBuffSizeBytes() {
  ...
  return ((UInt32) theNADUPacketData->GetFBS(0))
      * 64; //64 byte blocks are in the report
}
...
void NaduList::DumpList() {
  ...
  while (lastReportPtr) {
    ...
    s_printf("NADU Record: list_index = %"   _U32BITARG_   " list_recordID = %"   _U32BITARG_   "\n",
                this->GetReportIndex(thisID),
                thisID);
