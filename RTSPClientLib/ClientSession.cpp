...
enum {
  kMaxDropOut =
  3000,             //The sequence number can be kMaxDropOut ahead of the reference.
  kMaxMisorder =
  100,             //The sequence number can be kMaxMisorder behind of the reference.
...
static const SInt64
    kIdleTimeoutInMsec = 20000; // Time out in 20 seconds if nothing's doing
static const SInt16 kSanitySeqNumDifference =
    3000; //how large a difference can two sequence number be and still be considered to be in range
...
ClientSession::ClientSession(UInt32 inAddr,
                             UInt16 inPort,
                             char *inURL,
                             ClientType inClientType,
                             UInt32 inDurationInSec,
                             UInt32 inStartPlayTimeInSec,
                             UInt32 inRTCPIntervalInMS,
                             UInt32 inOptionsIntervalInSec,
                             UInt32 inHTTPCookie,
                             bool inAppendJunkData,
                             ...
                             bool sendOptions,
                             bool requestRandomData,
                             SInt32 randomDataSize,
                             bool enable3GPP,
                             UInt32 GBW,
                             UInt32 MBW,
                             UInt32 MTD,
                             bool enableForcePlayoutDelay,
                             UInt32 playoutDelay,
                             UInt32 bandwidth,
                             UInt32 bufferSpace,
                             UInt32 delayTime,
                             UInt32 startPlayDelay,
                             char *controlID,
                             char *name,
                             char *password)
    : fSocket(NULL),
  ...
  switch (inClientType) {
    case kRTSPUDPClientType: {
      ...
      fSocket = NEW
      TCPClientSocket(Socket::kNonBlockingSocketType);
      break;
    }
    ...
    case kRTSPHTTPClientType: {
      ...
      fTransportType = kTCPTransportType;
      fSocket = NEW
      HTTPClientSocket(theURL, inHTTPCookie, Socket::kNonBlockingSocketType);
      break;
    }
  ...
  if (enable3GPP) {
    fClient->Set3GPPLinkChars(fGuarenteedBitRate,
                              fMaxBitRate,
                              fMaxTransferDelay);
    ...
    switch (fState) {
      case kSendingOptions: {
        if (true == fOptionsRequestRandomData)
          theErr =
              fClient->SendOptionsWithRandomDataRequest(fOptionsRandomDataSize);
        else
            ...
            if (fVerboseLevel >= 3) {
              qtss_printf("--- Options transaction time ms = %qd  ---\n",
                          (SInt64) (OS::Milliseconds()
                              - fTransactionStartTimeMilli));
              SInt32 receivedLength = (SInt32) fClient->GetContentLength();
              if (receivedLength != 0)
                qtss_printf(
                    "--- Options Request Random Data Received requested = %" _S32BITARG_ " received = %" _S32BITARG_ "  ---\n",
                    fOptionsRandomDataSize,
                    receivedLength);

              StrPtrLenDel theBody
                  (ConvertBytesToCHexString(fClient->GetContentBody(),
                                            receivedLength));
              theBody.PrintStr("\n");
            }
            fState = kSendingDescribe;
      ...
      case kSendingDescribe: {
        ...
        if (theErr == OS_NoErr) {
          ...
          if (fClient->GetStatus() != 200) {
            ...
            fSDPParser.Parse(fClient->GetContentBody(),
                             fClient->GetContentLength());
          ...
          fPlayerSimulator.Setup(fSDPParser.GetNumStreams(),
                                 fDelayTime,
                                 fStartPlayDelay);
      ...
      case kSendingSetup: {
        ...
        if (fTransportType == kUDPTransportType) {
          theErr =
              fClient->SendUDPSetup(fSDPParser.GetStreamInfo(fNumSetups)->fTrackID,
          ...
          theErr =
              fClient->SendTCPSetup(fSDPParser.GetStreamInfo(fNumSetups)->fTrackID,
                                    fNumSetups * 2,
                                    (fNumSetups * 2) + 1);
        } else if (fTransportType == kReliableUDPTransportType) {
          theErr =
              fClient->SendReliableUDPSetup(fSDPParser.GetStreamInfo(fNumSetups)->fTrackID,
                                            fUDPSocketArray[fNumSetups
                                                * 2]->GetLocalPort());
        }
        if (fVerboseLevel >= 3)
        ...
        if (theErr == OS_NoErr) {
          if (fClient->GetStatus() != 200) {
            ...
            StringParser parser =
                StringParser(&fSDPParser.GetStreamInfo(fNumSetups)->fPayloadName);
            parser.GetThru(NULL, '/');
      ...
      case kSendingPlay: {
        ...
        if (theErr == OS_NoErr) {
          ...
          for (UInt32 ssrcCount = 0; ssrcCount < fSDPParser.GetNumStreams();
               ssrcCount++)
            fStats[ssrcCount].fServerSSRC =
                fClient->GetSSRCByTrack(fSDPParser.GetStreamInfo(ssrcCount)->fTrackID);
      ...
      case kPlaying: {
        ...
        if (((curTime - fPlayTime) > fDurationInSec * 1000)
            || (fTeardownImmediately)) {
          sPlayingConnections--;
        ...
        bool sendRTCP = ((curTime - fLastRTCPTime) > fRTCPIntervalInMs)
            && (fTransportType != kTCPTransportType);
        sendRTCP |=
            (fPlayTime == fLastRTCPTime);        //send the first RTCP ASAP.
        if (sendRTCP) {
        ...
        return curTime < nextRTCPTime ? MIN(nextRTCPTime - curTime,
                                            fReadInterval) : 1;
      }
...
void ClientSession::SetupUDPSockets() {
  ...
  fUDPSocketArray = NEW
  UDPSocket *[fSDPParser.GetNumStreams() * 2];
  for (UInt32 x = 0; x < fSDPParser.GetNumStreams() * 2; x++) {
    fUDPSocketArray[x] = NEW
    UDPSocket(this, Socket::kNonBlockingSocketType);
  ...
  for (UInt32 y = 0; y < fSDPParser.GetNumStreams(); y++) {
    for (UInt32 portCheck = 0; true; portCheck++) {
      ...
      if (theErr == OS_NoErr)
        theErr = fUDPSocketArray[(y * 2) + 1]->Bind(INADDR_ANY,
                                                    sCurrentRTPPortToUse + 1);
...
OS_Error ClientSession::ReadMediaData() {
  ...
  while (true) {
    ...
    if (fTransportType == kTCPTransportType) {
      thePacket = NULL;
      theErr =
          fClient->GetMediaPacket(&theTrackID, &isRTCP, &thePacket, &theLength);
      if (thePacket == NULL)
        break;
    } else {
      ...
      // Get a packet from one of the UDP sockets.
      theErr = fUDPSocketArray[theUDPSockIndex]->RecvFrom(&theRemoteAddr,
                                                          &theRemotePort,
                                                          &thePacketBuf[0],
                                                          kMaxPacketSize,
                                                          &theLength);
      if ((theErr != OS_NoErr) || (theLength == 0)) {
        ...
        if ((fTransportType == kReliableUDPTransportType)
            && (!(theUDPSockIndex & 1))) {
          UInt32 trackIndex = TrackID2TrackIndex(fSDPParser.GetStreamInfo(
              theUDPSockIndex / 2)->fTrackID);
...
void ClientSession::ProcessRTPPacket(char *inPacket,
                                     UInt32 inLength,
                                     UInt32 inTrackID) {
  ...
  if (trackIndex == kUInt32_Max) {
    if (fVerboseLevel >= 3)
      qtss_printf(
          "ClientSession::ProcessRTPPacket fatal packet processing error. unknown track\n");
    return;
  }
  ...
  if (trackStats.fHighestSeqNum
      == kUInt32_Max)     //this is the first sequence number received
    packetSeqNum = trackStats.fBaseSeqNum = trackStats.fHighestSeqNum =
        static_cast<UInt32>(rtpPacket.GetSeqNum());
  else
  ...
  if (packetSeqNum == kUInt32_Max)            //sequence number is out of range
  {
    ...
    //RTP-Meta-Info
    RTPMetaInfoPacket::FieldID
        *theMetaInfoFields = fClient->GetFieldIDArrayByTrack(inTrackID);
    if (theMetaInfoFields != NULL) {
      ...
      bool packetOK = theMetaInfoPacket.ParsePacket((UInt8 *) inPacket,
                                                    inLength,
                                                    theMetaInfoFields);
      if (!packetOK) {
        if (fVerboseLevel >= 2)
    ...
    if (fEnable3GPP) {
      ...
      bool packetIsDuplicate = fPlayerSimulator.ProcessRTPPacket(trackIndex,
                                                                 inLength,
                                                                 rtpPacket.GetBody().Len,
                                                                 packetSeqNum,
                                                                 timeStamp);
...
void ClientSession::ProcessRTCPPacket(char *inPacket,
                                      UInt32 inLength,
                                      UInt32 inTrackID) {
  UInt32 trackIndex = TrackID2TrackIndex(inTrackID);
  if (trackIndex == kUInt32_Max) {
    if (fVerboseLevel >= 3)
      qtss_printf(
          "ClientSession::ProcessRTCPPacket fatal packet processing error. unknown track\n");
    return;
  }
  TrackStats &trackStats = fStats[trackIndex];
  ...
  bool badPacket =
      !packet.ParseReport(reinterpret_cast<UInt8 *>(inPacket), inLength);
  if (!badPacket) {
    if (trackStats.fServerSSRC != 0) {
...
void ClientSession::SendAckPackets(UInt32 inTrackIndex) {
  ...
  fUDPSocketArray[(inTrackIndex * 2) + 1]->SendTo(fSocket->GetHostAddr(),
                                                  trackStats.fDestRTCPPort,
                                                  sendBuffer,
                                                  packetLength);
}
...
OS_Error ClientSession::SendRTCPPackets(UInt32 trackIndex) {
  ...
  if (trackStats.fHighestSeqNum != kUInt32_Max) {
    ...
    if (trackStats.fLastSenderReportLocalTime != 0)
      dlsr = static_cast<UInt32>(OS::TimeMilli_To_Fixed64Secs(
          curTime - trackStats.fLastSenderReportLocalTime) >> 16);

    RRPacket.AddReportBlock(trackStats.fServerSSRC,
                            fracLost,
                            cumLostPackets,
                            trackStats.fHighestSeqNum,
                            lsr,
...
#define OB 0x6F620004
    ...
    *(theWriter++) = htonl(PD);
    *(theWriter++) =
        htonl(0);      // should be a short, but we need to pad to a long for the entire RTCP app packet
  }
  ...
  if (fEnable3GPP && trackStats.fHighestSeqNum != kUInt32_Max) {
    ...
    RTCPNADUPacketFmt
        nadu = RTCPNADUPacketFmt(buf, kMaxUDPPacketSize - packetLen);
  ...
  if (fUDPSocketArray != NULL) {
    Assert(trackStats.fDestRTCPPort != 0);
    fUDPSocketArray[(trackIndex * 2) + 1]->SendTo(fSocket->GetHostAddr(),
                                                  trackStats.fDestRTCPPort,
                                                  buffer,
                                                  packetLen);
  } else {
    OS_Error theErr =
        fClient->PutMediaPacket(fSDPParser.GetStreamInfo(trackIndex)->fTrackID,
                                true,
                                buffer,
                                packetLen);
    if (theErr != OS_NoErr)
...
void ClientSession::CalcRTCPRRPacketsLost(UInt32 trackIndex,
                                          UInt8 &outFracLost,
                                          SInt32 &outCumLostPackets) {
  TrackStats &trackStats = fStats[trackIndex];
  ...
  UInt32 receivedInterval =
      trackStats.fNumPacketsReceived - trackStats.fReceivedPrior;
...
UInt32 ClientSession::CalcSeqNum(UInt32 referenceSeqNum, UInt16 newSeqNum) {
  ...
  if (static_cast<UInt16>(newSeqNum - refSeqNum16) <= kMaxDropOut) {
    ...
    if (newSeqNum >= refSeqNum16)
      ...
      return ((refSeqNumHighBits + 1) << 16)
          | newSeqNum;        //wrapped-around
