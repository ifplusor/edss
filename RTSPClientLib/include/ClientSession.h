...
class ClientSession : public Task {
 public:
  ...
  ClientSession(UInt32 inAddr,
                ...
                UInt32 MBW = 0,
                UInt32 MTD = 0,
                bool enableForcePlayoutDelay = false,
                UInt32 playoutDelay = 0,
                UInt32 bandwidth = 0,
                UInt32 bufferSpace = 0,
                UInt32 delayTime = 0,
                UInt32 startPlayDelay = 0,
                char *controlID = NULL,
                char *name = NULL,
                char *password = NULL);
  ...
  enum {
    ...
    kRequestFailed =
    2,    // Session couldn't be setup because the server returned an error
    ...
    kDiedWhilePlaying =
    6     // Connection was forceably closed while playing the movie
  };
  ...
  UInt32 Get3gNumPacketsLost(UInt32 inTrackIndex) {
    return fPlayerSimulator.GetNumPacketsLost(inTrackIndex);
  }
  UInt32 Get3gNumDuplicates(UInt32 inTrackIndex) {
    return fPlayerSimulator.GetNumDuplicates(inTrackIndex);
  }
  UInt32 Get3gNumLatePackets(UInt32 inTrackIndex) {
    return fPlayerSimulator.GetNumLatePackets(inTrackIndex);
  }
  UInt32 Get3gNumBufferOverflowedPackets(UInt32 inTrackIndex) {
    return fPlayerSimulator.GetNumBufferOverflowedPackets(inTrackIndex);
  }
  //include packets with bad SSRC
  UInt32 GetNumMalformedPackets(UInt32 inTrackIndex) { return fStats[inTrackIndex].fNumMalformedPackets; }

  //Will reset the counter everytime it is called
  UInt32 GetSessionPacketsReceived() {
    UInt32 result = fNumPacketsReceived;
  ...
  static UInt32 GetConnectionBytesReceived() {
    UInt32 result = sBytesReceived;
    sBytesReceived = 0;
    return result;
  }
  static UInt32 GetConnectionPacketsReceived() {
    UInt32 result = sPacketsReceived;
    sPacketsReceived = 0;
    return result;
  }
 ...
 private:
  ...
  UInt32 TrackID2TrackIndex(UInt32 trackID) {
    for (UInt32 trackIndex = 0; trackIndex < fSDPParser.GetNumStreams();
         trackIndex++) {
  ...
  TimeoutTask
      fTimeoutTask; // Kills this connection in the event the server isn't responding
  ...
  UInt32
      fStartPlayDelay;            //how much buffer should we keep before we start playing? in milliseconds
  bool fEnable3GPP;
  ...
  struct TrackStats {
    ...
    UInt32
        fNumPacketsReceived;                //track only good packets(but include late and duplicates)
    ...
    UInt32
        fNumAcks;                            //cumulative; counts ACK packets with masks as 1 ACK
    ...
    TrackStats()
        : fNumPacketsReceived(0),
          fNumBytesReceived(0),
          fNumOutOfOrderPackets(0),
          fNumOutOfBoundPackets(0),
          fNumMalformedPackets(0),
          fNumAcks(0),
          fDestRTCPPort(0),
          fServerSSRC(0),
          fClientSSRC(0),
          fLastSenderReportNTPTime(0),
          fLastSenderReportLocalTime(0),
          fHighestSeqNum(kUInt32_Max),
          fBaseSeqNum(0),
          fExpectedPrior(0),
          fReceivedPrior(0) {}
  };
  ...
  */
  ...
  UInt32
      fNumPacketsReceived;                //track only good packets(but include late and duplicates; see RFC3550 6.4.1)
  UInt32 fNumBytesReceived;                  //includes RTP header

  UInt32 fVerboseLevel;

  SVector<TrackStats>
      fStats;             //the index of this vector is the same as fSDPParser.GetStreamInfo
  ...
  void CalcRTCPRRPacketsLost(UInt32 trackIndex,
                             UInt8 &outFracLost,
