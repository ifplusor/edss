...
class PlayerSimulator {
 public:
  ...
  void Setup(UInt32 numTracks,
             UInt32 targetBufferingDelay = 0,
  ...
  void SetupTrack(UInt32 trackIndex,
                  UInt32 samplingRate,
                  UInt32 bufferSize = 0) {
    fTrackInfo[trackIndex].fBufferSize =
        bufferSize == 0 ? kUInt32_Max : bufferSize;
  ...
  Bool16 ProcessRTPPacket(UInt32 trackIndex,
                          UInt32 packetLen,
                          UInt32 payloadSize,
                          UInt32 seqNum,
                          UInt32 timeStamp) {
    ...
    if (!fIsPlaying) {
      ...
      for (UInt32 index = 0; index < fTrackInfo.size(); ++index) {
        if (fTrackInfo[index].fPlayBuffer.empty()
            || GetBufferingDelay(index) <= fStartPlayDelay) {
        ...
        for (UInt32 index = 0; index < fTrackInfo.size(); ++index) {
          ...
          if (fVerboseLevel >= 1)
            qtss_printf("Track %"  _U32BITARG_  " is now playing; initial seq=%"  _U32BITARG_  ", initial time stamp=%"  _U32BITARG_  "\n",
                        index,
                        curTrack.fNextSeqNum,
                        curTrack.fRTPTimeStampBase);
        }
        AdvanceTime();
      }
    }
    ...
    if (fVerboseLevel >= 2) {
      if (fIsPlaying)
        qtss_printf("Processed packet: track=%" _U32BITARG_ ", len=%" _U32BITARG_ ", seq=%" _U32BITARG_ ", time=%" _U32BITARG_ "(%" _U32BITARG_ "); "
                    ...
                    seqNum,
                    timeStamp,
                    RTPTime2MediaTime(trackIndex, timeStamp),
                    GetBufferingDelay(trackIndex),
                    GetFreeBufferSpace(trackIndex),
                    fCurrentMediaTime);
      else
        qtss_printf("Processed packet: track=%" _U32BITARG_ ", len=%" _U32BITARG_ ", seq=%" _U32BITARG_ ", time=%" _U32BITARG_ "(%" _U32BITARG_ "); "
  ...
  UInt32 GetBufferingDelay(UInt32 trackIndex) {
    ...
    UInt32 bufferDelay = static_cast<UInt32>((delta * 1000)
        / fTrackInfo[trackIndex].fSamplingRate);
    if (fIsPlaying)
 ...
 private:
  ...
  struct TrackInfo {
    ...
    UInt32
        fNumBufferOverflowedPackets;    //number of packets lost due to buffer overflow
    UInt32
        fNumLatePackets;            //packet that did not arrive on time, but did arrive
    ...
    SVector<UInt32>
        fLostPackets;                //All the packets with seq number less than fNextSeqNum that has not yet arrived.
    SVector<PacketInfo>
        fPlayBuffer;                //Can contain only packets with seq number >= than fNextSeqNum

    TrackInfo()
        : fNumDuplicates(0),
          fNumBufferOverflowedPackets(0),
          fNumLatePackets(0),
          fBufferSize(0),
          fSamplingRate(0),
          fRTPTimeStampBase(0),
    ...
    for (UInt32 trackIndex = 0; trackIndex < fTrackInfo.size(); ++trackIndex) {
      ...
      for (; i < playBuffer.size(); ++i) {
        if (prevTimeStamp > playBuffer[i].fTimeStamp) {
          if (fVerboseLevel
              >= 2) //this can happen from out of order packets from udp network routing or retransmits from rudp. It should never happen over tcp or http.
            qtss_printf("WARNING: RTP timestamp is not monotonic! seq=%" _U32BITARG_ " timestamp=%" _U32BITARG_ "\n",
                        playBuffer[i].fSeqNum,
                        playBuffer[i].fTimeStamp);
        } else
          prevTimeStamp = playBuffer[i].fTimeStamp;

        if (fCurrentMediaTime
            >= RTPTime2MediaTime(trackIndex, playBuffer[i].fTimeStamp)) {
  ...
  UInt32 LocalTime2MediaTime(SInt64 localTime) {
    ...
    Assert(fIsPlaying && (SInt64) 0 <= (SInt64) delta
  ...
  UInt32
      fCurrentMediaTime;                //current media time(in millisecond); valid only while playing
