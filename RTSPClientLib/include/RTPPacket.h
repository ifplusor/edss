...
class RTPPacket {
 public:
  ...
  RTPPacket(StrPtrLen inPacket)
      : fPacket(reinterpret_cast<RTPHeader *>(inPacket.Ptr)),
        fLen(inPacket.Len) {}
  ...
  UInt8 GetCSRCCount() const {
    return (ntohs(fPacket->rtpheader) & 0x0F00) >> 8;
  }
  ...
  StrPtrLen GetBody() const {
    return StrPtrLen(reinterpret_cast<char *>(fPacket) + GetHeaderLen(),
                     fLen - GetHeaderLen());
  }
