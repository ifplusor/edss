...
static char *sDescription =
    "Provides support for streaming from users' home directories";
...
static UInt32
    sMaxNumConnsPerHomeDir = 0; //Max conns. allowed per home directory
...
// FUNCTIONS
static QTSS_Error QTSSHomeDirectoryDispatch(QTSS_Role inRole,
...
static void RewriteRootDir(QTSS_StandardRTSP_Params *inParams,
                           StrPtrLen *inHomeDir);
...
QTSS_Error QTSSHomeDirectoryDispatch(QTSS_Role inRole,
                                     QTSS_RoleParamPtr inParams) {
  switch (inRole) {
...
QTSS_Error RereadPrefs() {
  ...
  sMoviesDirectory = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                         "movies_directory",
                                                         kDefaultMoviesDirectory);
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "max_num_conns_per_home_directory",
                                qtssAttrDataTypeUInt32,
                                &sMaxNumConnsPerHomeDir,
                                &kDefaultMaxNumConnsPerHomeDir,
                                sizeof(sMaxNumConnsPerHomeDir));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "max_bandwidth_kbps_per_home_directory",
                                qtssAttrDataTypeUInt32,
                                &sMaxBWInKbpsPerHomeDir,
                                &kDefaultMaxBWInKbpsPerHomeDir,
                                sizeof(sMaxBWInKbpsPerHomeDir));
  return QTSS_NoErr;
}
...
void RewriteRootDir(QTSS_StandardRTSP_Params *inParams, StrPtrLen *inHomeDir) {
  ...
  ResizeableStringFormatter
      theHomeDirFormatter(homeDirFormatterBuf, kDefaultHomeDirSize);
  ...
  StrPtrLen theRootDir
      (theHomeDirFormatter.GetBufPtr(), theHomeDirFormatter.GetBytesWritten());
  ...
  if (*theMethod == qtssPlayMethod) {
    ...
    if (theDirectoryInfoRef == NULL) {
      DirectoryInfo *newDirInfo = NEW
      DirectoryInfo(&theRootDir);
...
QTSS_Error AuthorizeRequest(QTSS_StandardRTSP_Params *inParams) {
  ...
  if ((theHomeDir.Ptr == NULL) || (theHomeDir.Len
      == 0)) // If it's NULL it's not served out of the home dir
    goto end_authorize;
  ...
  if ((sMaxNumConnsPerHomeDir != 0)
      && ((theDirInfo->NumSessions()) >= sMaxNumConnsPerHomeDir))
    QTSSModuleUtils::SendErrorResponse(inParams->inRTSPRequest,
                                       qtssClientNotEnoughBandwidth,
                                       qtssMsgTooManyClients);
  else if ((sMaxBWInKbpsPerHomeDir != 0)
      && ((theDirInfo->CurrentTotalBandwidthInKbps())
          >= sMaxBWInKbpsPerHomeDir))
    QTSSModuleUtils::SendErrorResponse(inParams->inRTSPRequest,
                                       qtssClientNotEnoughBandwidth,
...
end_authorize:
  ...
  (void) QTSS_SetValue(inParams->inRTSPSession,
                       sIsFirstRequestAttr,
                       0,
                       &sTrue,
                       sizeof(sTrue));
