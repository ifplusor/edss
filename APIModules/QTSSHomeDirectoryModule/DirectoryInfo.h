...
class SessionListElement {
 public:
  SessionListElement(QTSS_ClientSessionObject *inSessionPtr) {
    fSession = *inSessionPtr;
  }
...
class DirectoryInfo {
 public:
  DirectoryInfo(StrPtrLen *inHomeDir)
      : fNumSessions(0), fHomeDir(inHomeDir->GetAsCString()) {
    fClientSessionList = NEW
    PLDoubleLinkedList<SessionListElement>;
