...
Bool16 SessionListElement::Equal(QTSS_ClientSessionObject *inSessionPtr) {
  ...
  (void) QTSS_GetValuePtr(fSession,
                          qtssCliSesCounterID,
                          0,
                          (void **) &theSessionID,
                          &theLen);
  Assert(theLen != 0);
  (void) QTSS_GetValuePtr(*inSessionPtr,
                          qtssCliSesCounterID,
                          0,
                          (void **) &inSessionID,
                          &theLen);
...
static bool IsSession(PLDoubleLinkedListNode<SessionListElement> *node,
                      void *userData) {
  /*
...
static void AddCurrentBitRate(PLDoubleLinkedListNode<SessionListElement> *node,
                              void *userData) {
...
void DirectoryInfo::AddSession(QTSS_ClientSessionObject *sessionPtr) {
  ...
  SessionListElement *theElement = NEW
  SessionListElement(sessionPtr);
  PLDoubleLinkedListNode<SessionListElement>
      *sessionNode = new PLDoubleLinkedListNode<SessionListElement>(theElement);
