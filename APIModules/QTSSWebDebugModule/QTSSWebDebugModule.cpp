...
QTSS_Error QTSSWebDebugModuleDispatch(QTSS_Role inRole,
...
//we don't want any of the values we are reporting to change
  ...
  char buffer[1024];
  s_sprintf(buffer, "<HTML><TITLE>TimeShare Debug Page</TITLE><BODY>Total dynamic memory allocated: %" _S32BITARG_ "<P>List of objects:<BR>", OSMemory::GetAllocatedMemory());
  ...
  for (OSQueueIter iter(OSMemory::GetTagQueue()); !iter.IsDone(); iter.Next())
  {
      ...
      {
          s_sprintf(buffer, "Object allocated at: %s, %d. Number of currently allocated objects: %" _S32BITARG_ ", Total size: %" _S32BITARG_ "<BR>", elem->fileName, elem->line, elem->numObjects, elem->totMemory);
          (void)QTSS_Write(inParams->inRTSPRequest, buffer, ::strlen(buffer), &theLen, 0);
      }
  }
