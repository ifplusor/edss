...
static QTSS_Error QTSSSplitterModuleDispatch(QTSS_Role inRole,
...
static ReflectorSession *FindOrCreateSession(StrPtrLen *inPath,
                                             QTSS_StandardRTSP_Params *inParams);
static QTSS_Error HandleSourceInfoErr(QTSS_Error rtspSourceInfoErr,
                                      QTSS_StandardRTSP_Params *inParams,
                                      ReflectorSession *inSession,
                                      RTSPClient *inClient);
static void DeleteSessionOnError(ReflectorSession *inSession,
                                 QTSS_ClientSessionObject inCliSession);
...
QTSS_Error QTSSSplitterModuleDispatch(QTSS_Role inRole,
                                      QTSS_RoleParamPtr inParams) {
  switch (inRole) {
...
QTSS_Error Register(QTSS_Register_Params *inParams) {
  ...
  static char *sRemoteHostRespondedWithAnErrorName =
      "QTSSSplitterModuleRemoteHostError";
  static char
      *sRemoteHostRefusedConnectionName = "QTSSSplitterModuleRemoteHostRefused";
  static char
      *sExpectedDigitFilenameName = "QTSSSplitterModuleExpectedDigitFilename";
  static char *sBadTrackIDErrName = "QTSSSplitterModuleBadTrackID";

  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sRemoteHostRespondedWithAnErrorName,
                                 NULL,
  ...
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sRemoteHostRefusedConnectionName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sRemoteHostRefusedConnectionName,
...
ReflectorSession *FindOrCreateSession(StrPtrLen *inPath,
                                      QTSS_StandardRTSP_Params *inParams) {
  ...
  if (theFileData.Len > 0)
    theInfo = NEW
  RTSPSourceInfo(Socket::kNonBlockingSocketType);
  else
  return NULL;
  ...
  OSRef
      *theSessionRef = sSessionMap->Resolve(theInfo->GetRTSPClient()->GetURL());
  ReflectorSession *theSession = NULL;
  ...
  if (theSessionRef == NULL) {
    ...
    theSession = NEW
    ReflectorSession(theInfo->GetRTSPClient()->GetURL(), theInfo);
    ...
    theErr =
        QTSS_CreateStreamFromSocket(theInfo->GetRTSPClient()->GetSocket()->GetSocket()->GetSocketFD(),
                                    &theSockStream);
    Assert(theErr == QTSS_NoErr);
...
QTSS_Error HandleSourceInfoErr(QTSS_Error rtspSourceInfoErr,
                               QTSS_StandardRTSP_Params *inParams,
                               ReflectorSession *inSession,
                               RTSPClient *inClient) {
  ...
  if ((rtspSourceInfoErr == EAGAIN) || (rtspSourceInfoErr == EINPROGRESS)) {
    ...
    RequestSocketEvent(inSession->GetSocketStream(),
                       inClient->GetSocket()->GetEventMask());
  ...
  if (rtspSourceInfoErr == QTSS_RequestFailed) {
    ...
    char tempBuf[20];
    s_sprintf(tempBuf, "%"_U32BITARG_"", inClient->GetStatus());
...
void DeleteSessionOnError(ReflectorSession *inSession,
                          QTSS_ClientSessionObject inCliSession) {
...
void NullOutSessionAttr(QTSS_ClientSessionObject inSession) {
  ...
  QTSS_Error theErr =
      QTSS_SetValue(inSession, sSessionAttr, 0, (void *) &theNull, theLen);
  Assert(theErr == QTSS_NoErr);
}
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params *inParams,
  ...
  SourceInfo::StreamInfo *theStreamInfo =
      inSession->GetSourceInfo()->GetStreamInfoByTrackID(theTrackID);
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params *inParams,
  ...
  QTSS_Error theErr = QTSS_Play(inParams->inClientSession,
                                inParams->inRTSPRequest,
...
void IssueTeardown(ReflectorSession *inSession) {
  ...
  QTSS_Error
      theErr = ((RTSPSourceInfo *) inSession->GetSourceInfo())->Teardown();
  if ((theErr == EAGAIN) || (theErr == EINPROGRESS)) {
    RTSPClient *theClient =
        ((RTSPSourceInfo *) inSession->GetSourceInfo())->GetRTSPClient();
    theClient->GetSocket()->GetSocket()->DontAutoCleanup();
    RequestSocketEvent(inSession->GetSocketStream(),
                       theClient->GetSocket()->GetEventMask());
  } else {
    // Make sure to destroy the socket stream as well
    Assert(inSession->GetSocketStream() != NULL);
    (void) QTSS_DestroySocketStream(inSession->GetSocketStream());
