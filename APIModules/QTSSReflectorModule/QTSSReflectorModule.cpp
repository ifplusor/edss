...
static StrPtrLen sSDPTimeNotValidMessage
    ("SDP time is not valid or movie not available at this time.");
...
static void RemoveOutput(ReflectorOutput *inOutput,
                         ReflectorSession *inSession,
                         bool killClients);

static ReflectorSession *
DoSessionSetup(QTSS_StandardRTSP_Params *inParams,
...
static bool InfoPortsOK(QTSS_StandardRTSP_Params *inParams,
...
QTSS_Error Register(QTSS_Register_Params *inParams) {
  ...
  static char
      *sExpectedDigitFilenameName = "QTSSReflectorModuleExpectedDigitFilename";
  static char *sReflectorBadTrackIDErrName = "QTSSReflectorModuleBadTrackID";
  static char *sDuplicateBroadcastStreamName =
  ...
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sDuplicateBroadcastStreamName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sDuplicateBroadcastStreamName,
                        &sDuplicateBroadcastStreamErr);

  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sAnnounceRequiresSDPinName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sAnnounceRequiresSDPinName,
                        &sAnnounceRequiresSDPinNameErr);

  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sAnnounceDisabledName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sAnnounceDisabledName,
  ...
  static char *sSDPcontainsInvalidMinumumPortErrName =
      "QTSSReflectorModuleSDPPortMinimumPort";
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sSDPcontainsInvalidMinumumPortErrName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sSDPcontainsInvalidMinumumPortErrName,
                        &sSDPcontainsInvalidMinimumPortErr);

  static char *sSDPcontainsInvalidMaximumPortErrName =
      "QTSSReflectorModuleSDPPortMaximumPort";
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sSDPcontainsInvalidMaximumPortErrName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sSDPcontainsInvalidMaximumPortErrName,
                        &sSDPcontainsInvalidMaximumPortErr);

  static char
      *sStaticPortsConflictErrName = "QTSSReflectorModuleStaticPortsConflict";
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sStaticPortsConflictErrName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sStaticPortsConflictErrName,
  ...
  static char
      *sInvalidPortRangeErrName = "QTSSReflectorModuleStaticPortPrefsBadRange";
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sInvalidPortRangeErrName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sInvalidPortRangeErrName,
  ...
  static char
      *sBroadcasterSessionName = "QTSSReflectorModuleBroadcasterSession";
  ...
  (void) QTSS_AddStaticAttribute(qtssRTSPSessionObjectType,
                                 ...
                                 NULL,
                                 qtssAttrDataTypeVoidPointer);
  (void) QTSS_IDForAttr(qtssRTSPSessionObjectType,
                        sBroadcasterSessionName,
...
char *GetTrimmedKeyWord(char *prefKeyWord) {
  ...
  theRequestPathParser.ConsumeUntil(&theKeyWordStr,
                                    kPathDelimiterChar); // stop when we see a / and don't include
...
QTSS_Error ProcessRTPData(QTSS_IncomingData_Params *inParams) {
  ...
  QTSS_Error theErr = QTSS_GetValue(inParams->inRTSPSession,
                                    sRTSPBroadcastSessionAttr,
                                    0,
                                    &theSession,
                                    &theLen);
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params *inParams) {
  ...
  UInt32 theLen = 0;
  if ((QTSS_GetValuePtr(inParams->inRTSPRequest,
                        qtssRTSPReqMethod,
                        0,
                        (void **) &theMethod,
                        &theLen)
  ...
  if ((theErr != QTSS_NoErr)
      || (theLen != sizeof(RTPSessionOutput *))) // a broadcaster push session
  {
...
ReflectorSession *DoSessionSetup(QTSS_StandardRTSP_Params *inParams,
  ...
  QTSS_Error theErr = QTSS_GetValueAsString(inParams->inRTSPRequest,
                                            qtssRTSPReqFileName,
                                            0,
                                            &theFileNameStr);
  ...
  theErr = QTSS_GetValueAsString(inParams->inRTSPRequest,
  ...
  if (theFullPath.Len > sMOVSuffix.Len) {
    StrPtrLen endOfPath2
        (&theFullPath.Ptr[theFullPath.Len - sMOVSuffix.Len], sMOVSuffix.Len);
  ...
  if (sAllowNonSDPURLs && !isPush) {
    ...
    if (theFullPath.Len > sSDPSuffix.Len) {
      ...
      StrPtrLen endOfPath2
          (&theFullPath.Ptr[theFullPath.Len - sSDPSuffix.Len], sSDPSuffix.Len);
      if (endOfPath2.Equal(sSDPSuffix)) {
        if (resultFilePath != NULL)
          // TODO(james): no channel?
          *resultFilePath = theFullPath.GetAsCString();
        return FindOrCreateSession(&theFullPath,
                                   inParams,
                                   theChannelNum,
                                   NULL,
                                   isPush,
...
void DoAnnounceAddRequiredSDPLines(QTSS_StandardRTSP_Params *inParams,
  ...
  if (!checkedSDPContainer.HasReqLines()) {
    ...
    if (!checkedSDPContainer.HasLineType('s')) { // add s line
      ...
      (void) QTSS_GetValueAsString(inParams->inRTSPRequest,
                                   qtssRTSPReqFilePath,
                                   0,
                                   &theSDPName);
    ...
    if (!checkedSDPContainer.HasLineType('o')) { // add o line
      ...
      editedSDP->Put(" ");
      qtss_snprintf(tempBuff, sizeof(tempBuff) - 1, "%" _64BITARG_ "d",
...
QTSS_Error DoAnnounce(QTSS_StandardRTSP_Params *inParams) {
  ...
  if (!sAnnounceEnabled)
    return QTSSModuleUtils::SendErrorResponse(inParams->inRTSPRequest,
                                              qtssPreconditionFailed,
                                              sAnnounceDisabledNameErr);
  ...
  char *theQueryString = NULL;
  QTSS_GetValueAsString(inParams->inRTSPRequest,
                        qtssRTSPReqQueryString,
                        0,
                        &theQueryString);
  QTSSCharArrayDeleter theQueryStringDeleter(theQueryString);
  ...
  if (sAnnouncedKill && theFullPath.Len > sSDPKillSuffix.Len) {
    StrPtrLen endOfPath
        (theFullPath.Ptr + (theFullPath.Len - sSDPKillSuffix.Len),
         sSDPKillSuffix.Len);
  ...
  if (!pathOK) {
    if (theFullPath.Len <= sSDPSuffix.Len) {
      StrPtrLen endOfPath(theFullPath.Ptr + (theFullPath.Len - sSDPSuffix.Len),
                          sSDPSuffix.Len);
      if (!endOfPath.Equal(sSDPSuffix))
        return QTSSModuleUtils::SendErrorResponse(inParams->inRTSPRequest,
                                                  qtssPreconditionFailed,
  ...
  if ((sMaxAnnouncedSDPLengthInKbytes != 0)
      && (*theContentLenP > (sMaxAnnouncedSDPLengthInKbytes * 1024)))
  ...
  theErr = QTSS_GetValue(inParams->inRTSPRequest,
                         sRequestBodyAttr,
                         0,
                         &theRequestBody,
                         &theLen);
  ...
  if (theErr != QTSS_NoErr) {
    ...
    theLen = sizeof(theRequestBody);
    theErr = QTSS_SetValue(inParams->inRTSPRequest,
                           sRequestBodyAttr,
                           0,
                           &theRequestBody,
                           theLen);// SetValue creates an internal copy.
    Assert(theErr == QTSS_NoErr);
  ...
  theErr = QTSS_Read(inParams->inRTSPRequest,
                     theRequestBody + theBufferOffset,
  ...
  if ((theErr == QTSS_WouldBlock)
      || (theLen < (*theContentLenP - theBufferOffset))) {
    //
  ...
  if (!InfoPortsOK(inParams, &theSDPSourceInfo,
                   &theFullPath)) // All validity checks like this check should be done before touching the file.
  {
  ...
  return QTSS_SendStandardRTSPResponse(inParams->inRTSPRequest,
                                       inParams->inClientSession,
                                       0);
}
  ...
  if (!checkedSDPContainer.HasReqLines()) {
    ...
    if (!checkedSDPContainer.HasLineType('o')) { // add o line
      ...
      qtss_snprintf(tempBuff,
                    sizeof(tempBuff) - 1,
                    "%" _64BITARG_ "d",
                    (SInt64) (modDate / 1000) + 2208988800LU);
      editedSDP->Put(tempBuff);
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params *inParams) {
  ...
  ReflectorSession *theSession =
      DoSessionSetup(inParams, qtssRTSPReqFileName, false, NULL, &theFileName);
  ...
  (void) QTSSModuleUtils::ReadEntireFile(theFileName,
                                         &theFileData,
                                         inModDate,
                                         &outModDate);
  ...
  SDPSourceInfo tempSDPSourceInfo(theFileData.Ptr,
                                  theFileData.Len); // will make a copy and delete in destructor
  theSDPData.Ptr =
      tempSDPSourceInfo.GetLocalSDP(&theSDPData.Len); // returns a new buffer with processed sdp
  OSCharArrayDeleter sdpDeleter
      (theSDPData.Ptr); // delete the temp sdp source info buffer returned by GetLocalSDP

  if (theSDPData.Len
      <= 0) // can't find it on disk or it failed to parse just use the one in the session.
  {
    // 注意：虽然 DoSessionSetup 返回的 theFileName 不是 streamName, ReadEntireFile 获取不到内容,
    // 但这里可以补全
    theSDPData.Ptr =
        theSession->GetLocalSDP()->Ptr; // this sdp isn't ours it must not be deleted
  ...
  ResizeableStringFormatter editedSDP(NULL, 0);
  DoDescribeAddRequiredSDPLines(inParams,
                                theSession,
                                outModDate,
                                &editedSDP,
                                &theSDPData);
  ...
  SDPLineSorter sortedSDP
      (&checkedSDPContainer, adjustMediaBandwidthPercent, insertMediaLines);
...
bool InfoPortsOK(QTSS_StandardRTSP_Params *inParams,
                 SDPSourceInfo *theInfo,
                 StrPtrLen *inPath) {
  ...
  if (sEnforceStaticSDPPortRange) {
    for (UInt32 x = 0; x < theInfo->GetNumStreams(); x++) {
      ...
      if (theErrorMessageID != qtssIllegalAttrID) {
        ...
        (void) QTSSModuleUtils::LogError(qtssWarningVerbosity,
                                         theErrorMessageID,
                                         0,
                                         thePathPort);

        StrPtrLen thePortStr(thePort);
        (void) QTSSModuleUtils::SendErrorResponse(inParams->inRTSPRequest,
                                                  qtssUnsupportedMediaType,
                                                  theErrorMessageID,
                                                  &thePortStr);
  ...
  if (theSessionRef == NULL) {
    ...
    do {
      ...
      if (isPush && theSession && !(theSession->IsSetup())) {
        UInt32 theSetupFlag =
            ReflectorSession::kMarkSetup | ReflectorSession::kIsPushSession;
        // 最后调用SetupReflectorSession()方法
        QTSS_Error theErr =
            theSession->SetupReflectorSession(NULL, inParams, theSetupFlag);
        if (theErr != QTSS_NoErr) {
          theSession = NULL;
          break;
        }
...
QTSS_Error AddRTPStream(ReflectorSession *theSession,
  ...
  // do overbuffering.
  QTSS_Error theErr = QTSS_AddRTPStream(inParams->inClientSession,
                                        inParams->inRTSPRequest,
                                        newStreamPtr,
                                        qtssASFlagsForceUDPTransport);
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params *inParams) {
  ...
  QTSS_Error theErr =
      QTSS_GetValuePtr(inParams->inRTSPRequest, qtssRTSPReqTransportMode, 0,
                       (void **) &transportModePtr, &theLen);
  bool isPush = transportModePtr != NULL
      && *transportModePtr == qtssRTPTransportModeRecord;
  ...
  if (theLen != sizeof(RTPSessionOutput *)) {
    ...
    if (theErr != QTSS_NoErr && !isPush) {
      ...
      // 增加客户端输出管道（多路转发）;
      RTPSessionOutput *theNewOutput =
          new RTPSessionOutput(inParams->inClientSession,
                               theSession,
                               sServerPrefs,
                               sStreamCookieAttr);
      ...
      (void) QTSS_SetValue(inParams->inClientSession,
                           ...
                           0,
                           &sBroadcasterSessionTimeoutMilliSecs,
                           sizeof(sBroadcasterSessionTimeoutMilliSecs));
    }
  } else {
    // 2) 如果已经存在输出会话，即直接调用。
  ...
  if (isPush) {
    ...
    SourceInfo::StreamInfo *theStreamInfo =
        theSession->GetSourceInfo()->GetStreamInfoByTrackID(theTrackID);
  ...
  QTSS_RTPStreamObject newStream = NULL;
  {
    ...
    theErr = QTSS_AddRTPStream(inParams->inClientSession,
                               inParams->inRTSPRequest,
                               &newStream,
                               0);
...
bool HaveStreamBuffers(QTSS_StandardRTSP_Params *inParams,
                       ReflectorSession *inSession) {
  if (inSession == NULL || inParams == NULL)
    return false;

  bool haveBufferedStreams =
      true; // set to false and return if we can't set the packets
  UInt32 y = 0;
  ...
  for (theStreamIndex = 0;
       QTSS_GetValuePtr(inParams->inClientSession,
    ...
    if ((theReflectorStream == NULL)
        || (false == theReflectorStream->HasFirstRTP())) {
    ...
    haveBufferedStreams = theSender->GetFirstPacketInfo(&firstSeqNum,
                                                        &firstTimeStamp,
                                                        &packetArrivalTime);
    ...
    QTSS_Error theErr = QTSS_SetValue(*theRef,
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params *inParams,
  ...
  if (inSession == NULL)    // 推送端
  {
    ...
    if (rtpInfoEnabled) {
      flags =
          qtssPlayRespWriteTrackInfo; //write first timestampe and seq num to rtpinfo
...
void KillCommandPathInList() {
  ...
  for (OSRefHashTableIter theIter(sSessionMap->GetHashTable());
...
bool AcceptSession(QTSS_StandardRTSP_Params *inParams) {
  ...
  if (QTSSModuleUtils::UserInGroup(QTSSModuleUtils::GetUserProfileObject(
      theRTSPRequest), sBroadcasterGroup.Ptr,
                                   sBroadcasterGroup.Len))
  ...
  if (QTSSModuleUtils::AddressInList(sPrefs,
                                     sIPAllowListID,
                                     &theClientIPAddressStr))
    return true;
...
QTSS_Error ReflectorAuthorizeRTSPRequest(QTSS_StandardRTSP_Params *inParams) {
  ...
  QTSS_ActionFlags
      noAction = ~qtssActionFlagsWrite; //no action anything but a write
  QTSS_ActionFlags authorizeAction =
      QTSSModuleUtils::GetRequestActions(inParams->inRTSPRequest);
  ...
  accessFile.AuthorizeRequest(inParams,
                              allowNoAccessFiles,
                              noAction,
                              authorizeAction,
                              &outAuthorized,
  ...
  if ((outAuthorized == false)
      && (authorizeAction & qtssActionFlagsWrite)) //handle it
  {
    //printf("ReflectorAuthorizeRTSPRequest SET not allowed\n");
    bool allowed = false;
    (void) QTSSModuleUtils::AuthorizeRequest(inParams->inRTSPRequest,
                                             &allowed,
                                             &allowed,
                                             &allowed);
  }
...
QTSS_Error RedirectBroadcast(QTSS_StandardRTSP_Params *inParams) {
  ...
  char *requestPathStr;
  (void) QTSS_GetValueAsString(theRequest,
                               qtssRTSPReqFilePath,
                               0,
                               &requestPathStr);
  ...
  if ((sRedirectBroadcastsKeyword && sRedirectBroadcastsKeyword[0] != 0)
      ...
      && theFirstPath.EqualIgnoreCase(sRedirectBroadcastsKeyword,
                                      ::strlen(sRedirectBroadcastsKeyword))) {
    // set qtssRTSPReqRootDir
    (void) QTSS_SetValue(theRequest,
                         qtssRTSPReqRootDir,
                         0,
                         sBroadcastsRedirectDir,
                         ::strlen(sBroadcastsRedirectDir));

    // set the request file path to the new path with the keyword stripped
    StrPtrLen theStrippedRequestPath;
    theRequestPathParser.ConsumeLength(&theStrippedRequestPath,
                                       theRequestPathParser.GetDataRemaining());
    (void) QTSS_SetValue(theRequest,
                         qtssRTSPReqFilePath,
                         0,
                         theStrippedRequestPath.Ptr,
                         theStrippedRequestPath.Len);
  }
  ...
  if (inParams->inDevice && inParams->inStreamType == easyRTSPType) {
    ...
    if (theSessionRef) {
      ...
      QTSS_ClientSessionObject
          clientSession = theSession->GetBroadcasterSession();
      Assert(theSession != NULL);

      if (clientSession) {
        char *theFullRequestURL = NULL;
        (void) QTSS_GetValueAsString(clientSession,
                                     qtssCliSesFullURL,
                                     0,
                                     &theFullRequestURL);
