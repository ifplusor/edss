...
void RCFSourceInfo::Parse(XMLTag *relayTag) {
  ...
  if (prefTag != NULL) {
    ...
    if (srcAddrStr != NULL)
      srcIPAddr = CF::Net::SocketUtils::ConvertStringToAddr(srcAddrStr);
  }
...
void RCFSourceInfo::ParseRelayDestinations(XMLTag *relayTag) {
  ...
  for (UInt32 y = 0; y < numTags; y++) {
    XMLTag *destTag = relayTag->GetEmbeddedTagByNameAndAttr("OBJECT",
                                                            "CLASS",
                                                            "destination",
                                                            y);
    if (destTag == NULL)
...
void RCFSourceInfo::ParseDestination(XMLTag *destTag, UInt32 index) {
  ...
  if (prefTag != NULL) {
    ...
    if (outAddrStr != NULL)
      fOutputArray[index].fLocalAddr =
          CF::Net::SocketUtils::ConvertStringToAddr(outAddrStr);
  }
  prefTag = destTag->GetEmbeddedTagByNameAndAttr("PREF", "NAME", "dest_addr");
  if (prefTag != NULL) {
    char *destAddrStr = prefTag->GetValue();
    if (destAddrStr != NULL)
      fOutputArray[index].fDestAddr =
          CF::Net::SocketUtils::ConvertStringToAddr(destAddrStr);
  }
  ...
  prefTag =
      destTag->GetEmbeddedTagByNameAndAttr("LIST-PREF", "NAME", "udp_ports");
  if (prefTag != NULL) {
    ...
    ::memset(fOutputArray[index].fPortArray,
             0,
             fOutputArray[index].fNumPorts * sizeof(UInt16));
    ...
    prefTag =
        destTag->GetEmbeddedTagByNameAndAttr("PREF", "NAME", "udp_base_port");
    if (prefTag == NULL)
      s_printf(
          "Missing both 'udp_base_port' and 'udp_ports' tags.\n Cannot set up the destination!\n");
    else {
