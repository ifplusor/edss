...
static QTSS_Error QTSSWebStatsModuleDispatch(QTSS_Role inRole,
                                             QTSS_RoleParamPtr inParams);
static QTSS_Error Register(QTSS_Register_Params *inParams);
static QTSS_Error Initialize(QTSS_Initialize_Params *inParams);
static QTSS_Error FilterRequest(QTSS_Filter_Params *inParams);
static void SendStats(QTSS_StreamRef inStream,
                      UInt32 refreshInterval,
                      bool displayHelp,
                      StrPtrLen *fieldList);
static char *GetPrefAsString(QTSS_ModulePrefsObject inPrefsObject,
                             char *inPrefName);
...
QTSS_Error QTSSWebStatsModuleDispatch(QTSS_Role inRole,
                                      QTSS_RoleParamPtr inParams) {
  switch (inRole) {
...
QTSS_Error Initialize(QTSS_Initialize_Params *inParams) {
  ...
  QTSS_ModuleObject theSvrControlModule =
      QTSSModuleUtils::GetModuleObjectByName("QTSSSvrControlModule");
  if (theSvrControlModule != NULL)
    sSvrControlPrefs =
        QTSSModuleUtils::GetModulePrefsObject(theSvrControlModule);
  ...
  sDefaultURLStr.Set(QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                         sDefaultURLPrefName,
                                                         sDefaultURL));
...
QTSS_Error FilterRequest(QTSS_Filter_Params *inParams) {
  ...
  (void) QTSS_GetValuePtr(theRequest,
  ...
  if (strPtr.Equal(StrPtrLen("GET")))   //it's a "Get" request
  {
    ...
    if (fullRequest.Expect('/')) {
      ...
      OSCharArrayDeleter
          theWebStatsURL(GetPrefAsString(sPrefs, sDefaultURLPrefName));
      ...
      if (strPtr.Len != 0
          && strPtr.Equal(theWebStatsURLPtr))   //it's a "stats" request
      {
        if (fullRequest.Expect('?')) {
          do {
            ...
            if (paramName.Len != 0) {
              ...
              if (paramName.Equal(StrPtrLen("refresh", strlen("refresh")))) {
                ...
                displayHelp = true;
              } else if (paramName.Equal(StrPtrLen("fields",
                                                   strlen("fields")))) {
                if (fullRequest.Expect('='))
                  fullRequest.ConsumeUntil(&fieldsList,
                                           (UInt8 *) sParamStopMask);
        ...
        (void) QTSS_SetValue(theRequest,
                             ...
                             &sFalse,
                             sizeof(sFalse));
        SendStats(inParams->inRTSPRequest,
                  refreshInterval,
                  displayHelp,
                  (fieldsList.Len != 0) ? &fieldsList : NULL);
      }
    }
  }
  return QTSS_NoErr;
...
void SendStats(QTSS_StreamRef inStream,
               UInt32 refreshInterval,
               bool displayHelp,
               StrPtrLen *fieldList) {
  struct FieldIndex {
  ...
  if (refreshInterval > 0) {
    qtss_sprintf(buffer,
                 "<META HTTP-EQUIV=Refresh CONTENT=%"   _U32BITARG_   ">\n",
                 refreshInterval);
  ...
  if (displayHelp) {
...
#ifndef __MacOSX__
    static StrPtrLen
        sHelpLine1("<P><b>Streaming Server Statistics Help</b></P>\n");
#else
    ...
    static StrPtrLen sHelpLine3
        ("<BLOCKQUOTE><P>http://server/statsURL?help&amp;refresh=15&amp;fields=curtime,cpuload </P>\n");

    static StrPtrLen sHelpLine4
        ("\"?\" means that there are options being attached to the stats request.<BR>\n");
    static StrPtrLen
        sHelpLine5("\"&amp;\" separates multiple stats options<BR>\n<BR>\n");

    static StrPtrLen
        sHelpLine6("<P>The three possible parameters to stats are:</P>\n");
    static StrPtrLen sHelpLine7
        ("<P>\"help\"  -- shows the help information you're reading right now.</P>\n");
    static StrPtrLen sHelpLine8
        ("<P>\"refresh=&#91;n&#93;\"  -- tells the browser to automatically update the page every &#91;n&#93; seconds.</P>\n");
    static StrPtrLen sHelpLine9
        ("<P>\"fields=&#91;fieldList&#93;\"  -- show only the fields specified in comma delimited &#91;fieldList&#93;</P>\n");
    static StrPtrLen sHelpLine10
        ("<BLOCKQUOTE>The following fields are available for use with the \"fields\" option:</P><BLOCKQUOTE><DL>\n");
    ...
    static StrPtrLen
        sHelpLine11("</DL></BLOCKQUOTE></BLOCKQUOTE></BLOCKQUOTE><BR><P><HR>");
  ...
  do {
    ...
    if (fieldList != NULL) {
      ...
      for (short i = 0; i < kMaxFieldNum; i++) {
        if (fieldName.Equal(StrPtrLen(kFieldIndexes[i].fieldName,
                                      ::strlen(kFieldIndexes[i].fieldName)))) {
    ...
    switch (fieldNum) {
...
#else
        static StrPtrLen
            sStatsLine1("<TITLE>Streaming Server Stats</TITLE><BR>\n");
        (void) QTSS_Write(inStream, sStatsLine1.Ptr, sStatsLine1.Len, NULL, 0);
#endif
...
#else
        static StrPtrLen sStatsLine2
            ("<center><h1>Streaming Server Statistics</h1></center>\n");
        (void) QTSS_Write(inStream, sStatsLine2.Ptr, sStatsLine2.Len, NULL, 0);
#endif
      }
        break;
      ...
      case 2: {
        ...
        if (theDNS.Ptr == NULL) {   // no DNS, try for the IP address only.
          (void) QTSS_GetValuePtr(sServer,
...
#define kMinuteSeconds 60
        ...
        qtss_snprintf(uptimebuffer,
                      sizeof(uptimebuffer),
                      "<b>Up Time Total Seconds: </b> %"   _U32BITARG_   "<BR>\n",
                      upTime);
        ...
        qtss_snprintf(uptimebuffer,
                      sizeof(uptimebuffer),
                      "<b>Up Time: </b> %"   _U32BITARG_   " days %"   _U32BITARG_   " hours %"   _U32BITARG_   " minutes %"   _U32BITARG_   " seconds <BR>\n",
                      upTimeDays,
                      upTimeHours,
                      upTimeMinutes,
                      upTimeSeconds);
      ...
      case 6: {
        ...
        (void) QTSS_GetValuePtr(sServer,
                                ...
                                0,
                                (void **) &theBuildDate.Ptr,
                                &theBuildDate.Len);
        Assert(theBuildDate.Ptr != NULL);
        qtss_sprintf(buffer,
                     "<b>Server Build Date: </b> %s<BR>\n",
                     theBuildDate.Ptr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 7: {
        ...
        if (theState == qtssRunningState) {
          qtss_snprintf(statusBuffer,
                        sizeof(statusBuffer),
                        "<b>Status: </b> %s since %s<BR>",
                        states[theState],
                        qtss_ctime(&sStartupTime, buffer, sizeof(buffer)));
        } else
          qtss_snprintf(statusBuffer,
                        sizeof(statusBuffer),
                        "<b>Status: </b> %s<BR>",
      ...
      case 14: {
        (void) QTSS_GetValueAsString(sServer,
                                     qtssRTPSvrCurConn,
                                     0,
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Current RTP Connections: </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 15: {
        (void) QTSS_GetValueAsString(sServer,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Current RTSP Connections: </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 16: {
        (void) QTSS_GetValueAsString(sServer,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Current RTSP over HTTP Connections: </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 17: {
        ...
        qtss_sprintf(buffer,
                     "<b>Current Throughput: </b> %"   _U32BITARG_   " kbits<BR>\n",
                     curBandwidth / 1024);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 18: {
        (void) QTSS_GetValueAsString(sServer,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Current Packets Per Second: </b> %s <BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 23: {
        (void) QTSS_GetValueAsString(sServerPrefs,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Maximum Throughput: </b> %s Kbits<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 26: {
        ...
        for (UInt32 theIndex = 0; true; theIndex++) {
          QTSS_Error theErr = QTSS_GetValuePtr(sServer,
                                               qtssSvrRTSPPorts,
                                               theIndex,
                                               (void **) &thePort.Ptr,
                                               &thePort.Len);
      ...
      case 35: {
        if (sAccessLogPrefs != NULL) {
          thePrefStr =
              GetPrefAsString(sAccessLogPrefs, "request_logfile_interval");
          qtss_sprintf(buffer,
                       "<b>Access Log Roll Interval (days): </b> %s<BR>",
                       thePrefStr);
          (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
        }
      }
      ...
      case 41: {
        (void) QTSS_GetValueAsString(sServerPrefs,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Error Log Roll Interval (days): </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 46: {
        (void) QTSS_GetValueAsString(sServerPrefs,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Total Bytes Update Interval: </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 48: {
        if (sReflectorPrefs != NULL) {
          thePrefStr =
              GetPrefAsString(sReflectorPrefs, "reflector_bucket_size");
          qtss_sprintf(buffer,
                       "<b>Reflector Bucket Size: </b> %s<BR>\n",
                       thePrefStr);
          (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
        }
      }
      ...
      case 49: {
        if (sSvrControlPrefs != NULL) {
          thePrefStr =
              GetPrefAsString(sSvrControlPrefs, "history_update_interval");
          qtss_sprintf(buffer,
                       "<b>History Update Interval: </b> %s<BR>\n",
                       thePrefStr);
          (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
        }
      }
      ...
      case 50: {
        ...
        qtss_sprintf(buffer,
                     "<b>Out of file descriptors: </b> %d<BR>\n",
                     isOutOfDescriptors);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 51: {
        (void) QTSS_GetValueAsString(sServer,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Number of UDP sockets: </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 52: {
        ...
        (void) QTSS_GetValue(sServer,
                             ...
                             &size);
        qtss_sprintf(buffer,
                     "<b>API version: </b> %d.%d<BR>\n",
                     (int) ((UInt32) (apiVersion & (UInt32) 0xFFFF0000L) >> 16),
                     (int) (apiVersion & (UInt32) 0x0000FFFFL));
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 53: {
        ...
        (void) QTSS_GetValue(sServer,
                             ...
                             &blahSize);
        qtss_sprintf(buffer,
                     "<b>Num Reliable UDP Retransmit Buffers: </b> %"   _U32BITARG_   "<BR>\n",
                     reliableUDPBuffers);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 54: {
        ...
        (void) QTSS_GetValue(sServer,
                             ...
                             &blahSize2);
        qtss_sprintf(buffer,
                     "<b>Amount of buffer space being wasted in UDP Retrans buffers: </b> %"   _U32BITARG_   "<BR>\n",
                     wastedBufSpace);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
      ...
      case 55: {
        (void) QTSS_GetValueAsString(sServer,
                                     ...
                                     &thePrefStr);
        qtss_sprintf(buffer,
                     "<b>Number of Task Threads: </b> %s<BR>\n",
                     thePrefStr);
        (void) QTSS_Write(inStream, buffer, ::strlen(buffer), NULL, 0);
      }
        break;
