...
// FUNCTION PROTOTYPES
static QTSS_Error EasyRedisModuleDispatch(QTSS_Role inRole,
...
QTSS_Error EasyRedisModuleDispatch(QTSS_Role inRole,
                                   QTSS_RoleParamPtr inParamBlock) {
  switch (inRole) {
    ...
    case Easy_RedisTTL_Role: return RedisTTL();
    case Easy_RedisSetRTSPLoad_Role: return RedisSetRTSPLoad();
    case Easy_RedisUpdateStreamInfo_Role: return RedisUpdateStream(&inParamBlock->easyStreamInfoParams);
    case Easy_RedisGetAssociatedCMS_Role: return RedisGetAssociatedCMS(&inParamBlock->GetAssociatedCMSParams);
    case Easy_RedisJudgeStreamID_Role: return RedisJudgeStreamID(&inParamBlock->JudgeStreamIDParams);
    default: break;
  }
  return QTSS_NoErr;
}
...
QTSS_Error RereadPrefs() {
  delete[] sRedis_IP;
  sRedis_IP = QTSSModuleUtils::GetStringAttribute(modulePrefs,
                                                  "redis_ip",
                                                  sDefaultRedis_IP_Addr);

  QTSSModuleUtils::GetAttribute(modulePrefs,
                                "redis_port",
  ...
  sRedisPassword = QTSSModuleUtils::GetStringAttribute(modulePrefs,
                                                       "redis_password",
                                                       sDefaultRedisPassword);
...
QTSS_Error RedisTTL() {
  ...
  if (reply->integer == 0) {
    ...
    auto
        http = QTSServerInterface::GetServer()->GetPrefs()->GetServiceWanPort();
    auto rtsp = QTSServerInterface::GetServer()->GetPrefs()->GetRTSPWANPort();

    sprintf(chKey,
            "hmset %s:%s %s %s %s %d %s %d %s %d",
            server,
            guid,
            EASY_REDIS_IP,
            ip,
            ...
            load);
    auto replyHmset =
        static_cast<redisReply *>(redisCommand(redisContext_, chKey));
...
QTSS_Error RedisUpdateStream(Easy_StreamInfo_Params *inParams) {
  ...
  sprintf(chKey,
          ...
          inParams->inBitrate,
          EASY_REDIS_OUTPUT,
          inParams->inNumOutputs,
          EASY_REDIS_EASYDARWIN,
          QTSServerInterface::GetServer()->GetCloudServiceNodeID());
  auto reply = static_cast<redisReply *>(redisCommand(redisContext_, chKey));
  auto replyGuard = MakeGuard([&]() {
    if (reply) {
      freeReplyObject(reply);
...
QTSS_Error RedisGetAssociatedCMS(QTSS_GetAssociatedCMS_Params *inParams) {
  ...
  string exists = Format("exists %s:%s",
                         string(EASY_REDIS_DEVICE),
                         string(inParams->inSerial));
  auto reply =
      static_cast<redisReply *>(redisCommand(redisContext_, exists.c_str()));
  auto replyGuard = MakeGuard([&]() {
    if (reply) {
      freeReplyObject(reply);
    }
  ...
  if (reply->integer == 1) {
    string strTemp = Format("hmget %s:%s %s",
                            string(EASY_REDIS_DEVICE),
                            string(inParams->inSerial),
                            string(EASY_REDIS_EASYCMS));
    auto replyHmget =
        static_cast<redisReply *>(redisCommand(redisContext_, strTemp.c_str()));
    ...
    strTemp = Format("hmget %s %s %s ",
                     easycms,
                     string(EASY_REDIS_IP),
                     string(EASY_REDIS_PORT));
    auto replyHmgetEasyDarwin =
        static_cast<redisReply *>(redisCommand(redisContext_, strTemp.c_str()));
    ...
    if (replyHmgetEasyDarwin->type == EASY_REDIS_REPLY_ARRAY
        && replyHmgetEasyDarwin->elements == 2) {
      bool ok = true;
      ...
      if (ok) {
        memcpy(inParams->outCMSIP,
               replyHmgetEasyDarwin->element[0]->str,
               replyHmgetEasyDarwin->element[0]->len);
        memcpy(inParams->outCMSPort,
               replyHmgetEasyDarwin->element[1]->str,
               replyHmgetEasyDarwin->element[1]->len);
      } else {
        return QTSS_RequestFailed;
      }
    }
