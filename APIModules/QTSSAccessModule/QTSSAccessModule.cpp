...
static CF::Core::Mutex *sUserMutex = NULL;
...
static QTSS_Error QTSSAccessModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSAccessModuleDispatch(QTSS_Role inRole,
                                    QTSS_RoleParamPtr inParams) {
  switch (inRole) {
...
QTSS_Error Register() {
  ...
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sUsersFileNotFound,
                        &sUsersFileNotFoundMessageAttrID);
  ...
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sGroupsFileNotFound,
                        &sGroupsFileNotFoundMessageAttrID);
...
char *GetCheckedFileName() {
  ...
  char *result = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                     MODPREFIX_"qtaccessfilename",
...
QTSS_Error RereadPrefs() {
  CF::Core::MutexLocker locker(sUserMutex);
  ...
  sUsersFilePath = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                       MODPREFIX_"usersfilepath",
                                                       sDefaultUsersFilePath);
  sGroupsFilePath = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                        MODPREFIX_"groupsfilepath",
                                                        sDefaultGroupsFilePath);
  ...
  if (sAccessCheckers[0]->HaveFilePathsChanged(sUsersFilePath,
                                               sGroupsFilePath)) {
...
QTSS_Error AuthenticateRTSPRequest(QTSS_RTSPAuth_Params *inParams) {
  ...
  QTSS_AuthScheme authScheme =
      QTAccessFile::FindUsersAndGroupsFilesAndAuthScheme(accessFilePath,
                                                         action,
                                                         &usersFilePath,
                                                         &groupsFilePath);
  ...
  CharArrayDeleter userPathDeleter(usersFilePath);
  CharArrayDeleter groupPathDeleter(groupsFilePath);
  ...
  if (!defaultPaths) {
    ...
    for (index = 1; index < sNumAccessCheckers; index++) {
      ...
      if (!sAccessCheckers[index]->HaveFilePathsChanged(usersFilePath,
                                                        groupsFilePath)) {
    ...
    if (currentChecker == NULL) {
      ...
      sAccessCheckers[sNumAccessCheckers]->UpdateFilePaths(usersFilePath,
                                                           groupsFilePath);
  ...
  (void) QTSS_SetValue(theUserProfile,
                       qtssUserRealm,
                       0,
                       (void *) (authRealm->Ptr),
                       (authRealm->Len));
  ...
  CharArrayDeleter usernameBufDeleter(usernameBuf);
  StrPtrLen username(usernameBuf);
  if (theErr != QTSS_NoErr)
    return theErr;

  // No memory is allocated; just a pointer to the profile is returned
  AccessChecker::UserProfile
      *profile = currentChecker->RetrieveUserProfile(&username);
  ...
  if (authScheme == qtssAuthBasic)
    (void) QTSS_SetValue(theUserProfile,
                         qtssUserPassword,
                         0,
                         (void *) ((profile->cryptPassword).Ptr),
                         (profile->cryptPassword).Len);
  else if (authScheme == qtssAuthDigest)
    (void) QTSS_SetValue(theUserProfile,
                         qtssUserPassword,
                         0,
                         (void *) ((profile->digestPassword).Ptr),
                         (profile->digestPassword).Len);
...
QTSS_Error AccessAuthorizeRTSPRequest(QTSS_StandardRTSP_Params *inParams) {
  ...
  return accessFile.AuthorizeRequest(inParams,
                                     allowNoAccessFiles,
                                     noAction,
                                     authorizeAction,
                                     &authorized,
