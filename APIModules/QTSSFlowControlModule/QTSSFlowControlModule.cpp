...
static QTSS_Error QTSSFlowControlModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSFlowControlModuleDispatch(QTSS_Role inRole,
                                         QTSS_RoleParamPtr inParamBlock) {
  switch (inRole) {
    ...
    case QTSS_RTCPProcess_Role: return ProcessRTCPPacket(&inParamBlock->rtcpProcessParams);
    default: break;
  }
  return QTSS_NoErr;
}
...
QTSS_Error RereadPrefs() {
  ...
  QTSSModuleUtils::GetAttribute(sPrefs,
                                ...
                                qtssAttrDataTypeUInt32,
                                &sLossThinTolerance,
                                &sDefaultLossThinTolerance,
                                sizeof(sLossThinTolerance));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "num_losses_to_thin",
                                qtssAttrDataTypeUInt32,
                                &sNumLossesToThin,
                                &sDefaultNumLossesToThin,
                                sizeof(sNumLossesToThin));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "loss_thick_tolerance",
                                qtssAttrDataTypeUInt32,
                                &sLossThickTolerance,
                                &sDefaultLossThickTolerance,
                                sizeof(sLossThickTolerance));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "num_losses_to_thick",
                                qtssAttrDataTypeUInt32,
                                &sLossesToThick,
                                &sDefaultLossesToThick,
                                sizeof(sLossesToThick));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "num_worses_to_thin",
                                qtssAttrDataTypeUInt32,
                                &sWorsesToThin,
                                &sDefaultWorsesToThin,
                                sizeof(sWorsesToThin));

  QTSSModuleUtils::GetAttribute(sPrefs,
                                "flow_control_udp_thinning_module_enabled",
                                qtssAttrDataTypeBool16,
                                &sModuleEnabled,
                                &sDefaultModuleEnabled,
                                sizeof(sDefaultModuleEnabled));
...
QTSS_Error ProcessRTCPPacket(QTSS_RTCPProcess_Params *inParams) {
  ...
  if ((*thePayloadType != 0) && (*thePayloadType == qtssVideoPayloadType))
      s_printf("Video track reporting:\n");
  else if ((*thePayloadType != 0) && (*thePayloadType == qtssAudioPayloadType))
      s_printf("Audio track reporting:\n");
  else
      s_printf("Unknown track reporting\n");
#endif
  ...
  QTSS_Error theErr = QTSS_GetValue(inParams->inRTPStream,
                                    qtssRTPStrTransportType,
                                    0,
                                    (void *) &theTransportType,
                                    &theLen);
  ...
  (void) QTSS_GetValuePtr(theStream,
                          sNumLossesAboveTolAttr,
                          0,
                          (void **) &uint32Ptr,
                          &theLen);
  ...
  (void) QTSS_GetValuePtr(inParams->inRTPStream,
  ...
  if ((uint16Ptr != NULL) && (theLen == sizeof(UInt16))) {
    ...
    thePercentLoss /=
        256; //Hmmm... looks like the client reports loss percent in multiples of 256
#if FLOW_CONTROL_DEBUGGING
    s_printf("Percent loss: %d\n", thePercentLoss);
#endif
...
#if FLOW_CONTROL_DEBUGGING
        s_printf("Percent loss too high: ratcheting less\n");
#endif
        ratchetLess = true;
      } else {
#if FLOW_CONTROL_DEBUGGING
        s_printf("Percent loss too high: Incrementing percent loss count to %"   _U32BITARG_   "\n", theNumLossesAboveTol);
#endif
        (void) QTSS_SetValue(theStream,
                             sNumLossesAboveTolAttr,
                             0,
                             &theNumLossesAboveTol,
                             sizeof(theNumLossesAboveTol));
...
#if FLOW_CONTROL_DEBUGGING
        s_printf("Percent is low: ratcheting more\n");
#endif
        ratchetMore = true;
      } else {
#if FLOW_CONTROL_DEBUGGING
        s_printf("Percent is low: Incrementing percent loss count to %"   _U32BITARG_   "\n", theNumLossesBelowTol);
#endif
        (void) QTSS_SetValue(theStream,
                             sNumLossesBelowTolAttr,
                             0,
                             &theNumLossesBelowTol,
                             sizeof(theNumLossesBelowTol));
...
#if FLOW_CONTROL_DEBUGGING
        s_printf("Client reporting getting worse. Ratcheting less\n");
#endif
        ratchetLess = true;
      } else {
#if FLOW_CONTROL_DEBUGGING
        s_printf("Client reporting getting worse. Incrementing num worses count to %"   _U32BITARG_   "\n", theNumWorses);
#endif
        (void) QTSS_SetValue(theStream,
                             sNumWorsesAttr,
                             0,
  ...
  if (ratchetMore || ratchetLess) {
    ...
    if ((ratchetLess) && (curQuality < numQualityLevels)) {
      curQuality++;
      if (curQuality
          > 1) // v3.0.1=v2.0.1 make level 2 means key frames in the file or max if reflected.
    ...
    SInt32 numThinned = 0;
    (void) QTSS_GetValuePtr(inParams->inClientSession,
                            qtssCliSesStartedThinning,
                            0,
                            (void **) &startedThinningPtr,
                            &theLen);
...
#if FLOW_CONTROL_DEBUGGING
    s_printf("Clearing num worses count\n");
#endif
...
#if FLOW_CONTROL_DEBUGGING
    s_printf("Clearing num losses above tolerance count\n");
#endif
    (void) QTSS_SetValue(theStream,
                         sNumLossesAboveTolAttr,
                         0,
...
#if FLOW_CONTROL_DEBUGGING
    s_printf("Clearing num losses below tolerance count\n");
#endif
...
void InitializeDictionaryItems(QTSS_RTPStreamObject inStream) {
  ...
  QTSS_Error theErr = QTSS_GetValuePtr(inStream,
                                       sNumLossesAboveTolAttr,
                                       0,
                                       (void **) &theValue,
                                       &theValueLen);
