...
static QTSS_Error QTSSMP3StreamingModuleDispatch(QTSS_Role inRole,
...
static bool CheckPassword(QTSS_Filter_Params *inParams,
...
static QTSS_Error LogRequest(QTSS_RTSPSessionObject inRTSPSession,
                             MP3ClientSession *client);
static void WriteStartupMessage();
static void WriteShutdownMessage();
static void url_strcpy(char *dest, const char *src);
static QTSS_Error ReEnterFilterRequest(QTSS_Filter_Params *inParams,
...
MP3BroadcasterSession::MP3BroadcasterSession(QTSS_RTSPSessionObject sess,
...
QTSS_Error MP3BroadcasterSession::AddClient(QTSS_RTSPSessionObject sess,
                                            QTSS_StreamRef stream) {
  if (fMP3ClientQueue == NULL) {
    Assert(0);
...
QTSS_Error MP3BroadcasterSession::GetBroadcastHeaders() {
  ...
  if (theErr == QTSS_WouldBlock) {
    ...
    DTRACE1(
        "MP3BroadcasterSession::GetBroadcastHeaders() - read failed with err = %"
        _S32BITARG_
        "\n",
        theErr);
    Assert(0);
...
QTSS_Error MP3BroadcasterSession::GetBroadcastData() {
  ...
  if (theErr == QTSS_WouldBlock) {
    ...
    DTRACE(
        "MP3BroadcasterSession::GetBroadcastData() - read failed with err = QTSS_NotConnected\n");
    ShutDownState();
  } else {
    DTRACE1(
        "MP3BroadcasterSession::GetBroadcastData() - read failed with err = %"
        _S32BITARG_
        "\n",
        theErr);
    Assert(0);
...
MP3ClientSession::MP3ClientSession(QTSS_RTSPSessionObject sess,
  ...
  UInt32 len;
  QTSS_CreateObjectValue(sServer,
  ...
  if (fHostName[0] != '\0')
    QTSS_SetValue(fQTSSObject,
                  qtssConnectionHostName,
                  0,
                  fHostName,
                  ::strlen(fHostName) + 1);
  QTSS_SetValuePtr(fQTSSObject,
  ...
  if (err == QTSS_NoErr)
    (void) QTSS_SetValue(fQTSSObject,
...
MP3ClientSession::~MP3ClientSession() {
  ...
  QTSS_LockObject(sServer);
  for (int x = 0;
       QTSS_GetValue(sServer, qtssSvrConnectedUsers, x, &sessionObject, &len)
           == QTSS_NoErr; x++) {
...
QTSS_Error MP3ClientSession::SendResponse() {
  ...
  if (GetState() == MP3ClientSession::kClientInitState) {
    ...
    if (WantsContentLength())
      theErr = QTSS_Write(GetStreamRef(),
                          kClientCLHeader,
                          ::strlen(kClientCLHeader),
                          NULL,
                          qtssWriteFlagsBufferData);
    else
      theErr =
          QTSS_Write(GetStreamRef(), "\r\n", 2, NULL, qtssWriteFlagsBufferData);
    if (theErr != QTSS_NoErr) {
      Assert(0);
      SetState(MP3ClientSession::kClientShutDownState);
      return theErr;
...
QTSS_Error MP3ClientSession::SendMP3Data(char *buffer, UInt32 bufferlen) {
  ...
  if (theErr == QTSS_WouldBlock) {
    ...
    DTRACE2("Got blocked at time %qd, numRetries = %"
                _S32BITARG_
                "\n", curTime, fRetryCount);
    if (fBlockTime == 0)
      fBlockTime = curTime;
    ...
    if (curTime - fBlockTime > sMaxFlowControlTimeInMSec) {
      SetResult(453);
      DTRACE1(
          "MP3ClientSession::SendMP3Data - too many tries. Terminating client session = %"
          _S32BITARG_
          "\n",
          GetSessionID());
      SetState(MP3ClientSession::kClientShutDownState);
    ...
    DTRACE1("MP3ClientSession::SendMP3Data - Terminating client session = %"
                _S32BITARG_
                "\n", GetSessionID());
    SetState(MP3ClientSession::kClientShutDownState);
  } else {
...
bool MP3SessionTable::RegisterSession(MP3Session *session) {
  ...
  MP3SessionRef *newEntry = NEW
  MP3SessionRef(session);
  if (newEntry == NULL) {
    return false;
  }
...
QTSS_Error MP3BroadcasterQueue::CreateBroadcaster(QTSS_RTSPSessionObject sess,
  ...
  if ((fQueue.GetLength() > 0) && (FindByMountPoint(mountpt) != NULL)) {
    theErr = QTSS_Write(stream,
                        kSourceReject,
                        ::strlen(kSourceReject),
                        NULL,
                        qtssWriteFlagsBufferData);
    if (theErr != QTSS_NoErr) {
      return theErr;
  ...
  MP3BroadcasterSession *broadcaster = NEW
  MP3BroadcasterSession(sess, stream);
  if (broadcaster == NULL) {
  ...
  OSQueueElem *elem = NEW
  OSQueueElem(broadcaster);
  if (elem != 0) {
    OSMutexLocker locker(&fMutex);
    fQueue.EnQueue(elem);
    DTRACE1("MP3BroadcasterQueue::CreateBroadcaster() Session(%"
                _S32BITARG_
                ") broadcaster added!\n", uSessID);
...
QTSS_Error MP3BroadcasterQueue::RemoveBroadcaster(QTSS_RTSPSessionObject sess) {
  ...
  while ((elem = iter.GetCurrent()) != 0) {
    MP3BroadcasterSession
        *current = (MP3BroadcasterSession *) elem->GetEnclosingObject();
    if ((current != 0) && (current->GetSession() == sess)) {
      elem->Remove();
      uSessID = current->GetSessionID();
      DTRACE1("MP3BroadcasterQueue::RemoveBroadcaster() Session(%"
                  _S32BITARG_
                  ") broadcaster removed!\n", uSessID);
      delete elem;
      delete current;
...
QTSS_Error MP3ClientQueue::AddClient(QTSS_RTSPSessionObject sess,
                                     QTSS_StreamRef stream,
                                     MP3BroadcasterSession *owner) {
  QTSS_Error theErr = QTSS_NoErr;
  UInt32 uSessID = 0L;
  ...
  MP3ClientSession *client = NEW
  MP3ClientSession(sess, stream, owner);
  if (client == 0) {
    return QTSS_NotEnoughSpace;
  }
  uSessID = GetRTSPSessionID(sess);
  ...
  if (elem != 0) {
    OSMutexLocker locker(&fMutex);
    fQueue.EnQueue(elem);
    DTRACE1("MP3ClientQueue::AddClient() Session(%"
                _S32BITARG_
                ") client added!\n", uSessID);
    // send the "OK" back to the broadcaster
...
QTSS_Error MP3ClientQueue::RemoveClient(QTSS_RTSPSessionObject sess) {
  ...
  while ((elem = iter.GetCurrent()) != 0) {
    MP3ClientSession
        *curClient = (MP3ClientSession *) elem->GetEnclosingObject();
    if ((curClient != 0) && (curClient->GetSession() == sess)) {
      elem->Remove();
      uSessID = curClient->GetSessionID();
      DTRACE1("MP3ClientQueue::RemoveClient() Session(%"
                  _S32BITARG_
                  ") client removed!\n", uSessID);
      delete elem;
      delete curClient;
...
QTSS_Error QTSSMP3StreamingModuleDispatch(QTSS_Role inRole,
                                          QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    ...
    case QTSS_RTSPFilter_Role: return FilterRequest(&inParams->rtspFilterParams);
    case QTSS_RereadPrefs_Role: return RereadPrefs();
    case QTSS_RTSPSessionClosing_Role: return SessionClosing(&inParams->rtspSessionClosingParams);
    case QTSS_StateChange_Role: return StateChange(&inParams->stateChangeParams);
...
QTSS_Error Initialize(QTSS_Initialize_Params *inParams) {
  ...
  sOSBufferPoolPtr = NEW
  OSBufferPool(sBroadcastBufferSize);
  sLogMutex = NEW
  OSMutex();
  sAtomicMutex = NEW
  OSMutex();
  ...
  qtss_sprintf(gClientAcceptHeader,
               kClientAcceptHeader,
               kVersionString,
               kBuildString);
...
QTSS_Error RereadPrefs() {
  ...
  delete[] sLogName;
  sLogName = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                 "mp3_request_logfile_name",
                                                 sDefaultLogName);

  delete[] sLogDir;
  sLogDir = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                "mp3_request_logfile_dir",
                                                sDefaultLogDir);
  ...
  QTSSModuleUtils::GetAttribute(sPrefs,
                                ...
                                &sMP3StreamingEnabled,
                                &sDefaultMP3StreamingEnabled,
                                sizeof(sMP3StreamingEnabled));

  delete[] sBroadcastPassword;
  sBroadcastPassword = QTSSModuleUtils::GetStringAttribute(sPrefs,
                                                           "mp3_broadcast_password",
                                                           sDefaultBroadcastPassword);

  // if broadcast password is blank or empty don't allow MP3 streaming.
  if (sBroadcastPassword == NULL || *sBroadcastPassword == '\0'
      || *sBroadcastPassword == ' ') {
  ...
  QTSSModuleUtils::GetAttribute(sPrefs,
                                ...
                                &sBroadcastBufferSize,
                                &sDefaultBroadcastBufferSize,
                                sizeof(sBroadcastBufferSize));

  QTSSModuleUtils::GetAttribute(sPrefs,
                                "mp3_max_flow_control_time",
                                qtssAttrDataTypeSInt32,
                                &sMaxFlowControlTimeInMSec,
                                &sDefaultFlowControlTimeInMSec,
                                sizeof(sMaxFlowControlTimeInMSec));
  ...
  (void) QTSS_GetValue(sServerPrefs,
                       ...
                       0,
                       (void *) &sMaximumConnections,
                       &len);
  (void) QTSS_GetValue(sServerPrefs,
                       qtssPrefsMaximumBandwidth,
                       0,
                       (void *) &sMaximumBandwidth,
                       &len);
...
void IncrementMP3SessionCount() {
  ...
  if ((err == QTSS_NoErr) && (ptrValue != NULL)) {
    *ptrValue += 1;
    ...
    DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrCurConn failed with err = %"
                _S32BITARG_
                "\n", (SInt32) err);
  }
  ...
  if ((err == QTSS_NoErr) && (ptrValue != NULL)) {
    *ptrValue += 1;
    ...
    DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrTotalConn failed with err = %"
                _S32BITARG_
                "\n", (SInt32) err);
  }
}
...
void IncrementTotalMP3Bytes(UInt32 bytes) {
  ...
  if ((err == QTSS_NoErr) && (ptrValue != NULL)) {
    ...
    *ptrValue += bytes;
    ...
    DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrTotalBytes failed with err = %"
                _S32BITARG_
                "\n", (SInt32) err);
  }
}
...
bool CheckBandwidth(SInt32 bandwidth) {
  ...
  if ((err == QTSS_NoErr) && (ptrValue != NULL)) {
    ...
    DTRACE1("QTSS_GetValuePtr() for qtssMP3SvrCurBandwidth failed with err = %"
                _S32BITARG_
                "\n", (SInt32) err);
  }
  return false;
...
UInt32 GetRTSPSessionID(QTSS_RTSPSessionObject session) {
  ...
  QTSS_Error err =
      QTSS_GetValue(session, qtssRTSPSesID, 0, (void *) &uSessID, &paramLen);
  if (err != QTSS_NoErr) {
    uSessID = 0L;
  }
  return uSessID;
...
bool IsProtocol(StrPtrLen &theRequest, char *protocol) {
  ...
  if (IsProtocolString(currentWord,
                       protocol))//test for line starting with protocol/vers in a response from the client
    return true;
  ...
  if (IsProtocolString(lastWord,
                       protocol))// test for line ending with protocol/version  as in a request from the client
    return true;
...
// incoming request.
bool CheckPassword(QTSS_Filter_Params *inParams,
                   StrPtrLen &theRequest,
                   StrPtrLen &mountpoint) {
...
bool GetHeaderAndValueFromeLine(StrPtrLen *line,
                                StrPtrLen *header,
                                char separator,
                                StrPtrLen *value) {
...
bool NeedsMetaData(StrPtrLen &theRequest) {
  ...
  while (requestParser.GetThruEOL(&line) || line.Len > 0) // pull out each line
  {
    ...
    if (header.EqualIgnoreCase("icy-metadata")
        && (StringParser(&value).ConsumeInteger() > 0))
...
QTSS_Error LogRequest(QTSS_RTSPSessionObject inRTSPSession,
                      MP3ClientSession *client) {
  char logbuffer[1024];
  ...
  qtss_sprintf(logbuffer,
               "%s \"%s\" [%s] \"%s\" %d %" _S32BITARG_ " %" _S32BITARG_ "\n",
               remoteAddress,
               userAgent,
               theDateBuffer,
               reqBuffer,
...
QTSS_Error ReEnterFilterRequest(QTSS_Filter_Params *inParams,
                                MP3Session *mp3Session) {
  QTSS_Error err = QTSS_NoErr;
...
QTSS_Error FilterRequest(QTSS_Filter_Params *inParams) {
  ...
  if (!sMP3StreamingEnabled || sServerIdle) {
    ...
    if (NeedsMetaData(theFullRequest)) // this is an HTTP icy request
      return QTSSModuleUtils::SendHTTPErrorResponse(theRequest,
                                                    qtssServerUnavailable,
                                                    true,
                                                    NULL);
  ...
  StrPtrLen mountpoint;
  if (!IsHTTP(theFullRequest) && IsBroadcastPassword(theFullRequest)
      && CheckPassword(inParams, theFullRequest, mountpoint)) {
    err = sMP3BroadcasterQueue.CreateBroadcaster(inParams->inRTSPSession,
    ...
    if (IsA_m3u_URL(theURL)) {
      ...
      err =
          QTSS_GetValue(sServer, qtssSvrDefaultIPAddr, 0, &serverIPAddr, &ulen);
      if (err == QTSS_NoErr) {
      ...
      err = QTSS_Write(inParams->inRTSPRequest,
                       tmpbuf,
                       ulen,
                       NULL,
                       qtssWriteFlagsBufferData);
