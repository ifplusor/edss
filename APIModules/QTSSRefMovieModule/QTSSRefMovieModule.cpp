...
QTSS_Error QTSSRefMovieModuleDispatch(QTSS_Role inRole,
...
static QTSS_Error SendTheResponse(QTSS_RTSPSessionObject theSession,
...
QTSS_Error QTSSRefMovieModuleDispatch(QTSS_Role inRole,
  ...
  switch (inRole) {
    ...
    case QTSS_RTSPFilter_Role:return Filter(&inParams->rtspFilterParams);
  }
  return QTSS_NoErr;
}
...
QTSS_Error RereadPrefs() {
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "refmovie_xfer_enabled",
                                qtssAttrDataTypeBool16,
                                &sRefMovieXferEnabled,
                                &sDefaultRefMovieXferEnabled,
                                sizeof(sRefMovieXferEnabled));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "refmovie_rtsp_port",
                                qtssAttrDataTypeUInt16,
                                &sRTSPReplyPort,
                                &sDefaultRTSPReplyPort,
                                sizeof(sRTSPReplyPort));
  return QTSS_NoErr;
}
...
QTSS_Error SendTheResponse(QTSS_RTSPSessionObject theSession,
                           QTSS_StreamRef stream,
                           StrPtrLen &movie) {
  QTSS_Error err = QTSS_NoErr;
  ...
  qtss_snprintf(tmp2,
                ...
                (int) ::strlen(tmp));
  err =
      QTSS_Write(stream, tmp2, ::strlen(tmp2), NULL, qtssWriteFlagsBufferData);
  if (err != QTSS_NoErr)
    return QTSS_NoErr;
...
QTSS_Error Filter(QTSS_Filter_Params *inParams) {
  ...
  err = QTSS_GetValueAsString(sServerPrefs,
