...
static char *sDescription =
    "Demonstrates an authorization method using client IP addresses for access control";
static UInt32 sVersion = 0x00010000;

// FUNCTION PROTOTYPES

static QTSS_Error QTSSDemoAuthorizationModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSDemoAuthorizationModuleDispatch(QTSS_Role inRole,
                                               QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    ...
    case QTSS_RTSPAuthenticate_Role:return Authenticate();
    case QTSS_RTSPAuthorize_Role:return Authorize(&inParams->rtspRequestParams);
...
QTSS_Error RereadPrefs() {
  ...
  sIPAccessList = QTSSModuleUtils::GetStringAttribute(sModulePrefs,
                                                      "IPAccessList",
                                                      "127.0.0.*");
...
QTSS_Error Authorize(QTSS_StandardRTSP_Params *inParams) {
  ...
  if ((theErr != QTSS_NoErr) || (allowRequest == false)) //handle it
  {
    ...
    (void) QTSSModuleUtils::SendErrorResponse(theRTSPRequest,
                                              qtssClientUnAuthorized,
                                              0);
...
Bool16 AcceptSession(QTSS_RTSPSessionObject inRTSPSession) {
  ...
  if (QTSSModuleUtils::AddressInList(sModulePrefs,
                                     sIPAccessListID,
