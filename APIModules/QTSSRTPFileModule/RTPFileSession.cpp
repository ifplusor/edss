...
RTPFileSession::ErrorCode RTPFileSession::Initialize(StrPtrLen &inFilePath,
                                                     Float32 inBufferSeconds) {
  Assert(fFile == NULL);
  ...
  fDataBufferSize = this->PowerOf2Floor((UInt32) (inBufferSeconds
      * fFile->GetBytesPerSecond()));
  ...
  fReadBuffer = fDataBuffer = NEW
  UInt8[fDataBufferSize];

  // Allocate a buffer of TrackInfos
  fTrackInfo = NEW
  RTPFileSessionTrackInfo[fFile->GetMaxTrackNumber() + 1];
  ::memset(fTrackInfo,
           0,
           fFile->GetMaxTrackNumber() * sizeof(RTPFileSessionTrackInfo));
  return errNoError;
}
...
RTPFileSession::ErrorCode RTPFileSession::Seek(Float64 inTime) {
  ...
  while (tracksFound < fNumTracksEnabled) {
    Float64 theTransmitTime =
        this->GetNextPacket(&thePacketP, &thePacketLength, &theCookie);
    if (thePacketP == NULL) {
...
Float64 RTPFileSession::GetNextPacket(UInt8 **outPacket,
                                      UInt32 *outPacketLength,
                                      void **outCookie) {
...
void RTPFileSession::ReadAndAdvise() {
  ...
  QTSS_Error theErr =
      QTSS_Read(fFileSource, fDataBuffer, fDataBufferSize, &fDataBufferLen);
  Assert(theErr == QTSS_NoErr);
...
RTPFileSession::ErrorCode RTPFile::Initialize(const StrPtrLen &inFilePath) {
  ...
  // Read the track info
  fTrackInfo = NEW
  RTPFileTrackInfo[fHeader.fNumTracks];
  //theErr = theFile.Read(fTrackInfo, sizeof(RTPFileTrackInfo) * fHeader.fNumTracks);
  theErr = QTSS_Read(theFile,
                     fTrackInfo,
                     sizeof(RTPFileTrackInfo) * fHeader.fNumTracks,
                     &theLengthRead);
  Assert(theErr == QTSS_NoErr);
...
SInt64 RTPFile::GetBlockLocation(Float64 inTimeInSecs) {
  ...
  return fHeader.fDataStartPos + (kBlockSize
      * x);// The requested time must be in the last block (or nonexistent).
}
