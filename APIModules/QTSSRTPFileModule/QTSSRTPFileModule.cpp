...
static QTSS_Error QTSSRTPFileModuleDispatch(QTSS_Role inRole,
...
static QTSS_Error CreateRTPFileSession(QTSS_StandardRTSP_Params *inParamBlock,
                                       const StrPtrLen &inPath,
                                       FileSession **outFile);
...
QTSS_Error QTSSRTPFileModuleDispatch(QTSS_Role inRole,
                                     QTSS_RoleParamPtr inParamBlock) {
  switch (inRole) {
    ...
    case QTSS_RTSPPreProcessor_Role:return ProcessRTSPRequest(&inParamBlock->rtspPreProcessorParams);
...
QTSS_Error Initialize(QTSS_Initialize_Params *inParams) {
  ...
  sdpURL.Ptr =
      QTSSModuleUtils::GetStringAttribute(sFileModulePrefs, "sdp_url", "");
  sdpURL.Len = ::strlen(sdpURL.Ptr);

  adminEmail.Ptr =
      QTSSModuleUtils::GetStringAttribute(sFileModulePrefs, "admin_email", "");
...
QTSS_Error RereadPrefs() {
  QTSSModuleUtils::GetAttribute(sFileModulePrefs,
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params *inParams) {
  ...
  switch (*theMethod) {
    case qtssDescribeMethod:return DoDescribe(inParams);
    case qtssSetupMethod:return DoSetup(inParams);
    case qtssPlayMethod:return DoPlay(inParams);
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params *inParamBlock) {
  ...
  QTSS_Error theError = QTSS_GetValueAsString(inParamBlock->inRTSPRequest,
  ...
  if ((theFullPath.Len <= sRTPSuffix.Len) ||
      (!sRTPSuffix.NumEqualIgnoreCase(&theFullPath.Ptr[theFullPath.Len
          - sRTPSuffix.Len], sRTPSuffix.Len)))
    return QTSS_RequestFailed;
  ...
  (void) QTSS_GetValuePtr(inParamBlock->inRTSPSession,
                          ...
                          0,
                          (void **) &theSDPVec[5].iov_base,
                          (UInt32 *) &theSDPVec[5].iov_len);
...
QTSS_Error CreateRTPFileSession(QTSS_StandardRTSP_Params *inParamBlock,
                                const StrPtrLen &inPath,
                                FileSession **outFile) {
  *outFile = NEW
  FileSession();
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params *inParamBlock) {
  ...
  for (UInt32 x = 0; x < theFile->fFile.GetSourceInfo()->GetNumStreams(); x++) {
    SourceInfo::StreamInfo
        *theStreamInfo = theFile->fFile.GetSourceInfo()->GetStreamInfo(x);
  ...
  while (true) {
    if ((*theFile)->fPacketStruct.packetData == NULL) {
      ...
      Float64 theTransmitTime =
          (*theFile)->fFile.GetNextPacket((UInt8 **) &(*theFile)->fPacketStruct.packetData,
    ...
    // Send the packet!
    theErr = QTSS_Write((*theFile)->fStream,
    ...
    if (theErr == QTSS_WouldBlock) {
      if ((*theFile)->fPacketStruct.packetTransmitTime == -1)
        ...
        Assert((*theFile)->fPacketStruct.packetTransmitTime
