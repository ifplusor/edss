...
QTSS_Error QTSSPosixFileSysModuleDispatch(QTSS_Role inRole,
  ...
  switch (inRole) {
    ...
    case QTSS_OpenFile_Role: return OpenFile(&inParams->openFileParams);
    case QTSS_AdviseFile_Role: return AdviseFile(&inParams->adviseFileParams);
    case QTSS_ReadFile_Role: return ReadFile(&inParams->readFileParams);
    case QTSS_CloseFile_Role: return CloseFile(&inParams->closeFileParams);
    case QTSS_RequestEventFile_Role: return RequestEventFile(&inParams->reqEventFileParams);
    default: break;
  }
  return QTSS_NoErr;
}
...
QTSS_Error OpenFile(QTSS_OpenFile_Params *inParams) {
  ...
  if (inParams->inFlags & qtssOpenFileAsync) {
    EventContext *theEventContext =
        new EventContext(EventContext::kInvalidFileDesc,
