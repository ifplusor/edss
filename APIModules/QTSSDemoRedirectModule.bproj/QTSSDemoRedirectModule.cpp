...
static char *sDescription =
    "Demonstrates a way to redirect file requests based on client bandwidth headers";
static UInt32 sVersion = 0x00010000;

// FUNCTION PROTOTYPES

static QTSS_Error QTSSDemoRedirectModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSDemoRedirectModuleDispatch(QTSS_Role inRole,
                                          QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    ...
    case QTSS_RTSPAuthenticate_Role:return Authenticate(&inParams->rtspRequestParams);
...
QTSS_Error RereadPrefs() {
  ...
  sRedirectUrl = QTSSModuleUtils::GetStringAttribute(sModulePrefs,
                                                     "url_to_redirect",
                                                     sRedirectUrlDefault);
...
QTSS_Error Authenticate(QTSS_StandardRTSP_Params *inParams) {
  ...
  if (::strncmp(movieUrlFilename,
                sRedirectUrl,
                (::strlen(movieUrlFilename) <= ::strlen(sRedirectUrl))
  ...
  (void) QTSS_GetValue(theRTSPRequest,
  ...
  if (bandwidthBits <= 0) {
    qtss_snprintf(theNewFilename,
                  sizeof(theNewFilename),
                  "%s/%s_50kbit.3gp",
                  movieUrlTrunc,
                  movieUrlFilename);
