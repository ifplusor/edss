...
class IPAddrTableElem {
 public:
  ...
  IPAddrTableElem(UInt32 inIPAddr)
      : fIPAddr(inIPAddr), fRefCount(0), fNextHashEntry(NULL) {}
  ~IPAddrTableElem() {}
...
class IPAddrTableKey {
 ...
 private:
  ...
  friend int operator==(const IPAddrTableKey &key1,
                        const IPAddrTableKey &key2) {
...
// FUNCTION PROTOTYPES
static QTSS_Error QTSSSpamDefenseModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSSpamDefenseModuleDispatch(QTSS_Role inRole,
                                         QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    ...
    case QTSS_RTSPAuthorize_Role:return Authorize(&inParams->rtspAuthParams);
...
QTSS_Error Register(QTSS_Register_Params *inParams) {
  ...
  static char
      *sTooManyConnectionsName = "QTSSSpamDefenseModuleTooManyConnections";
...
QTSS_Error Initialize(QTSS_Initialize_Params *inParams) {
  ...
  OSMutex();
  sHashTable = NEW
  IPAddrHashTable(277);//277 is prime, I think...
  RereadPrefs();
  return QTSS_NoErr;
}
...
QTSS_Error RereadPrefs() {
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "num_conns_per_ip_addr",
                                qtssAttrDataTypeUInt32,
                                &sNumConnsPerIP,
                                &sDefaultNumConnsPerIP,
                                sizeof(sNumConnsPerIP));
  return QTSS_NoErr;
}
...
QTSS_Error Authorize(QTSS_StandardRTSP_Params *inParams) {
  ...
  if (theElem->GetRefCount() >= sNumConnsPerIP) {
    QTSSModuleUtils::LogErrorStr(qtssMessageVerbosity,
                                 "Blocking connection from IP address");
