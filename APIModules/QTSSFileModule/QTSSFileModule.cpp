...
class FileSession {
 public:
  ...
  FileSession() : fAdjustedPlayTime(0),
                  fNextPacketLen(0),
                  fLastQualityCheck(0),
    ...
    fPacketStruct.packetData = NULL;
    fPacketStruct.packetTransmitTime = -1;
    fPacketStruct.suggestedWakeupTime = -1;
  }
...
static QTSS_AttributeID
    sRTPStreamLastSentPacketSeqNumAttrID = qtssIllegalAttrID;
...
static QTSS_Error QTSSFileModuleDispatch(QTSS_Role inRole,
...
static QTSS_Error CreateQTRTPFile(QTSS_StandardRTSP_Params *inParamBlock,
                                  char *inPath,
                                  FileSession **outFile);
...
static UInt32 WriteSDPHeader(FILE *sdpFile,
                             iovec *theSDPVec,
                             SInt16 *ioVectorIndex,
                             StrPtrLen *sdpHeader);
...
inline UInt16 GetLastPacketSeqNum(QTSS_Object stream) {
  ...
  (void) QTSS_GetValue(stream,
                       sRTPStreamLastPacketSeqNumAttrID,
                       0,
                       (void *) &lastSeqNum,
                       &theLen);
...
inline SInt32 GetLastSentSeqNumber(QTSS_Object stream) {
  UInt16 lastSeqNum = 0;
  UInt32 theLen = sizeof(lastSeqNum);
  QTSS_Error error = QTSS_GetValue(stream,
                                   sRTPStreamLastSentPacketSeqNumAttrID,
                                   0,
                                   (void *) &lastSeqNum,
                                   &theLen);
...
inline void SetPacketSequenceNumber(UInt16 newSequenceNumber,
                                    void *packetDataPtr) {
...
inline UInt32 CalculatePauseTimeStamp(UInt32 timescale,
                                      SInt64 totalPauseTime,
                                      UInt32 currentTimeStamp) {
  SInt64 pauseTime =
      (SInt64) ((Float64) timescale * (((Float64) totalPauseTime) / 1000.0));
  UInt32 pauseTimeStamp = (UInt32) (pauseTime + currentTimeStamp);

  return pauseTimeStamp;
}

UInt32 SetPausetimeTimeStamp(FileSession *fileSessionPtr,
                             QTSS_Object theRTPStream,
                             UInt32 currentTimeStamp) {
  if (false == fileSessionPtr->fAdjustPauseTime
      || fileSessionPtr->fTotalPauseTime == 0)
    return currentTimeStamp;

  UInt32 timeScale = 0;
  UInt32 theLen = sizeof(timeScale);
  (void) QTSS_GetValue(theRTPStream,
                       qtssRTPStrTimescale,
                       0,
                       (void *) &timeScale,
                       &theLen);
  ...
  UInt32 pauseTimeStamp = CalculatePauseTimeStamp(timeScale,
                                                  fileSessionPtr->fTotalPauseTime,
                                                  currentTimeStamp);
  if (pauseTimeStamp != currentTimeStamp)
    SetPacketTimeStamp(pauseTimeStamp,
                       fileSessionPtr->fPacketStruct.packetData);

  return pauseTimeStamp;
}

UInt32 WriteSDPHeader(FILE *sdpFile,
                      iovec *theSDPVec,
                      SInt16 *ioVectorIndex,
                      StrPtrLen *sdpHeader) {
  ...
  Assert(*ioVectorIndex
             < sNumSDPVectors); // if adding an sdp param you need to increase sNumSDPVectors
  ...
  *ioVectorIndex += 1;
  ...
  if (sdpFile != NULL)
    ::fwrite(theSDPVec[theIndex].iov_base,
             theSDPVec[theIndex].iov_len,
             sizeof(char),
             sdpFile);
...
QTSS_Error QTSSFileModuleDispatch(QTSS_Role inRole,
                                  QTSS_RoleParamPtr inParamBlock) {
  switch (inRole) {
    case QTSS_Register_Role: return Register(&inParamBlock->regParams);
    case QTSS_Initialize_Role: return Initialize(&inParamBlock->initParams);
    case QTSS_RereadPrefs_Role: return RereadPrefs();
    case QTSS_RTSPRequest_Role: return ProcessRTSPRequest(&inParamBlock->rtspRequestParams);
    case QTSS_RTPSendPackets_Role: return SendPackets(&inParamBlock->rtpSendPacketsParams);
    case QTSS_ClientSessionClosing_Role: return DestroySession(&inParamBlock->clientSessionClosingParams);
  }
  return QTSS_NoErr;
}
...
QTSS_Error Register(QTSS_Register_Params *inParams) {
  ...
  static char
      *sSeekToNonexistentTimeName = "QTSSFileModuleSeekToNonExistentTime";
  ...
  static char
      *sExpectedDigitFilenameName = "QTSSFileModuleExpectedDigitFilename";
  static char *sTrackDoesntExistName = "QTSSFileModuleTrackDoesntExist";

  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sSeekToNonexistentTimeName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sSeekToNonexistentTimeName,
  ...
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sTrackDoesntExistName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sTrackDoesntExistName,
                        &sTrackDoesntExistErr);
  ...
  (void) QTSS_IDForAttr(qtssClientSessionObjectType,
                        sFileSessionName,
                        &sFileSessionAttr);
  ...
  static char *sRTPStreamLastSentPacketSeqNumName =
      "QTSSFileModuleLastSentPacketSeqNum";
  (void) QTSS_AddStaticAttribute(qtssRTPStreamObjectType,
                                 sRTPStreamLastSentPacketSeqNumName,
                                 NULL,
  ...
  static char
      *sRTPStreamLastPacketSeqNumName = "QTSSFileModuleLastPacketSeqNum";
  (void) QTSS_AddStaticAttribute(qtssRTPStreamObjectType,
                                 sRTPStreamLastPacketSeqNumName,
                                 NULL,
                                 qtssAttrDataTypeUInt16);
  (void) QTSS_IDForAttr(qtssRTPStreamObjectType,
                        sRTPStreamLastPacketSeqNumName,
...
void BuildPrefBasedHeaders() {
  ...
  (void) QTSS_GetValuePtr(sServer,
                          qtssSvrDefaultDNSName,
                          0,
                          (void **) &theDefaultDNS.Ptr,
                          &theDefaultDNS.Len);
  ...
  adminEmail.Ptr =
      QTSSModuleUtils::GetStringAttribute(sPrefs, "admin_email", "");
  ...
  sEmailHeader.Ptr = NEW
  char[sEmailHeader.Len];
...
QTSS_Error RereadPrefs() {
  ...
  QTSSModuleUtils::GetAttribute(sPrefs,
                                ...
                                qtssAttrDataTypeUInt32,
                                &sFlowControlProbeInterval,
                                &sDefaultFlowControlProbeInterval,
                                sizeof(sFlowControlProbeInterval));

  QTSSModuleUtils::GetAttribute(sPrefs,
                                "max_allowed_speed",
                                qtssAttrDataTypeFloat32,
                                &sMaxAllowedSpeed,
                                &sDefaultMaxAllowedSpeed,
                                sizeof(sMaxAllowedSpeed));

  // File Cache prefs

  sEnableSharedBuffers = true;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "enable_shared_file_buffers",
                                  qtssAttrDataTypeBool16,
                                  &sEnableSharedBuffers,
                                  sizeof(sEnableSharedBuffers));

  sEnablePrivateBuffers = false;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "enable_private_file_buffers",
                                  qtssAttrDataTypeBool16,
                                  &sEnablePrivateBuffers,
                                  sizeof(sEnablePrivateBuffers));

  sSharedBufferInc = 8;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "num_shared_buffer_increase_per_session",
                                  qtssAttrDataTypeUInt32,
                                  &sSharedBufferInc,
                                  sizeof(sSharedBufferInc));

  sSharedBufferUnitKSize = 256;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "shared_buffer_unit_k_size",
                                  qtssAttrDataTypeUInt32,
                                  &sSharedBufferUnitKSize,
                                  sizeof(sSharedBufferUnitKSize));

  sPrivateBufferUnitKSize = 256;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "private_buffer_unit_k_size",
                                  qtssAttrDataTypeUInt32,
                                  &sPrivateBufferUnitKSize,
                                  sizeof(sPrivateBufferUnitKSize));

  sSharedBufferUnitSize = 1;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "num_shared_buffer_units_per_buffer",
                                  qtssAttrDataTypeUInt32,
                                  &sSharedBufferUnitSize,
                                  sizeof(sSharedBufferUnitSize));

  sPrivateBufferUnitSize = 1;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "num_private_buffer_units_per_buffer",
                                  qtssAttrDataTypeUInt32,
                                  &sPrivateBufferUnitSize,
                                  sizeof(sPrivateBufferUnitSize));

  sSharedBufferMaxUnits = 8;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "max_shared_buffer_units_per_buffer",
                                  qtssAttrDataTypeUInt32,
                                  &sSharedBufferMaxUnits,
                                  sizeof(sSharedBufferMaxUnits));

  sPrivateBufferMaxUnits = 8;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "max_private_buffer_units_per_buffer",
                                  qtssAttrDataTypeUInt32,
                                  &sPrivateBufferMaxUnits,
                                  sizeof(sPrivateBufferMaxUnits));
  ...
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  ...
                                  sizeof(sAddClientBufferDelaySecs));
  ...
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  ...
                                  qtssAttrDataTypeBool16,
                                  &sRecordMovieFileSDP,
                                  sizeof(sRecordMovieFileSDP));

  sEnableMovieFileSDP = false;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "enable_movie_file_sdp",
                                  qtssAttrDataTypeBool16,
                                  &sEnableMovieFileSDP,
                                  sizeof(sEnableMovieFileSDP));
  ...
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  ...
                                  sizeof(sAdjustMediaBandwidthPercent));

  sAdjustRTPStartTimeMilli = 500;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "compatibility_adjust_rtp_start_time_milli",
                                  qtssAttrDataTypeSInt64,
                                  &sAdjustRTPStartTimeMilli,
                                  sizeof(sAdjustRTPStartTimeMilli));

  sAllowInvalidHintRefs = false;
  QTSSModuleUtils::GetIOAttribute(sPrefs,
                                  "allow_invalid_hint_track_refs",
                                  qtssAttrDataTypeBool16,
                                  &sAllowInvalidHintRefs,
                                  sizeof(sAllowInvalidHintRefs));
  ...
  (void) QTSS_GetValue(sServerPrefs,
                       qtssPrefsDisableThinning,
                       0,
                       (void *) &sDisableThinning,
                       &len);
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params *inParamBlock) {
  ...
  if ((QTSS_GetValuePtr(inParamBlock->inRTSPRequest, qtssRTSPReqMethod, 0,
                        (void **) &theMethod, &theMethodLen) != QTSS_NoErr)
  ...
  switch (*theMethod) {
    case qtssDescribeMethod: err = DoDescribe(inParamBlock);
      break;
    case qtssSetupMethod: err = DoSetup(inParamBlock);
      break;
    case qtssPlayMethod: err = DoPlay(inParamBlock);
      break;
    case qtssTeardownMethod: (void) QTSS_Teardown(inParamBlock->inClientSession);
    ...
    case qtssPauseMethod: {
      ...
      UInt32 theLen = 0;
      QTSS_Error theErr = QTSS_GetValuePtr(inParamBlock->inClientSession,
                                           sFileSessionAttr,
                                           0,
                                           (void **) &theFile,
                                           &theLen);
...
bool isSDP(QTSS_StandardRTSP_Params *inParamBlock) {
  ...
  QTSS_Error theErr = QTSS_GetValuePtr(inParamBlock->inRTSPRequest,
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params *inParamBlock) {
  if (isSDP(inParamBlock)) {
    ...
    (void) QTSS_GetValuePtr(inParamBlock->inRTSPRequest,
                            qtssRTSPReqFilePath,
                            0,
                            (void **) &pathStr.Ptr,
                            &pathStr.Len);
    QTSS_Error err =
        QTSSModuleUtils::SendErrorResponse(inParamBlock->inRTSPRequest,
                                           qtssClientNotFound,
                                           sNoSDPFileFoundErr,
                                           &pathStr);
  ...
  UInt32 thePathLen = 0;
  OSCharArrayDeleter thePath
      (QTSSModuleUtils::GetFullPath(inParamBlock->inRTSPRequest,
                                    qtssRTSPReqFilePath,
                                    &thePathLen,
                                    &sSDPSuffix));

  //first locate the target movie
  thePath.GetObject()[thePathLen - sSDPSuffix.Len] =
      '\0';//truncate the .sdp added in the GetFullPath call
  StrPtrLen requestPath(thePath.GetObject(), ::strlen(thePath.GetObject()));
  if (requestPath.Len > sSDPSuffix.Len) {
    StrPtrLen endOfPath
        (&requestPath.Ptr[requestPath.Len - sSDPSuffix.Len], sSDPSuffix.Len);
  ...
  if (theFile != NULL) {
    ...
    if (!requestPath.Equal(moviePath)) {
      ...
      (void) QTSS_SetValue(inParamBlock->inClientSession,
                           sFileSessionAttr,
                           0,
                           &theFile,
                           sizeof(theFile));
  ...
  iovec theSDPVec[sNumSDPVectors
  ];//1 for the RTSP header, 6 for the sdp header, 1 for the sdp body
  ...
  OSCharArrayDeleter sdpDataDeleter
      (theSDPData.Ptr); // Just in case we fail we know to clean up. But we clear the deleter if we succeed.
  ...
  if (theSDPData.Len > 0) {
    ...
    (void) QTSS_AppendRTSPHeader(inParamBlock->inRTSPRequest,
                                 qtssLastModifiedHeader,
    ...
    QTSSModuleUtils::SendDescribeResponse(inParamBlock->inRTSPRequest,
                                          inParamBlock->inClientSession,
                                          &theSDPVec[0],
                                          3,
                                          theSDPData.Len);
    ...
    if ((theLen == sizeof(QTSS_TimeVal)) && (*theTime > 0)) {
      ...
      if (*theTime == theFile->fFile.GetQTFile()->GetModDate()) {
        theErr =
            QTSS_SetValue(inParamBlock->inRTSPRequest, qtssRTSPReqStatusCode, 0,
        ...
        theErr = QTSS_SendStandardRTSPResponse(inParamBlock->inRTSPRequest,
                                               inParamBlock->inClientSession,
                                               0);
        Assert(theErr == QTSS_NoErr);
        return QTSS_NoErr;
    ...
    FILE *sdpFile = NULL;
    if (sRecordMovieFileSDP
        && !pathEndsWithSDP) // don't auto create sdp for an sdp file because it would look like a broadcast
    {
      sdpFile = ::fopen(thePath.GetObject(),
                        "r"); // see if there already is a .sdp for the movie
    ...
    (void) QTSS_GetValueAsString(inParamBlock->inRTSPRequest,
                                 qtssRTSPReqFilePath,
                                 0,
                                 &fileNameStr);
    ...
    //Get IP addr
    StrPtrLen ipStr;
    (void) QTSS_GetValuePtr(inParamBlock->inRTSPSession,
    ...
    qtss_sprintf(ownerLine,
                 "o=StreamingServer %" _64BITARG_ "d %" _64BITARG_ "d IN IP4 %s",
                 (SInt64) OS::UnixTime_Secs() + 2208988800LU,
                 (SInt64) theFile->fFile.GetQTFile()->GetModDate(),
                 ipCstr);
    ...
    if (sAddClientBufferDelaySecs
        > 0) // increase the client buffer delay by the preference amount.
    {
      Float32 bufferDelay =
          3.0; // the client doesn't advertise it's default value so we guess.
      ...
      if (delayStr.Len > 0) {
        UInt32 offset = (delayStr.Ptr - theSDPData.Ptr)
            + delayStr.Len; // step past the string
    ...
    StrPtrLen fullSDPBuffSPL
        (theFullSDPBuffer.GetBufPtr(), theFullSDPBuffer.GetBytesWritten());
    ...
    if (sPlayerCompatibility)
      adjustMediaBandwidth = QTSSModuleUtils::HavePlayerProfile(sServerPrefs,
                                                                inParamBlock,
                                                                QTSSModuleUtils::kAdjustBandwidth);

    if (adjustMediaBandwidth)
      adjustMediaBandwidthPercent =
          (Float32) sAdjustMediaBandwidthPercent / 100.0;

    ResizeableStringFormatter buffer;
    SDPContainer *insertMediaLines =
        QTSS3GPPModuleUtils::Get3GPPSDPFeatureListCopy(buffer);
    SDPLineSorter sortedSDP
        (&rawSDPContainer, adjustMediaBandwidthPercent, insertMediaLines);
...
*/
    ...
    totalSDPLength += ::WriteSDPHeader(sdpFile,
                                       theSDPVec,
                                       &vectorIndex,
                                       theSessionHeadersPtr);
    totalSDPLength +=
        ::WriteSDPHeader(sdpFile, theSDPVec, &vectorIndex, theMediaHeadersPtr);
...
QTSS_Error CreateQTRTPFile(QTSS_StandardRTSP_Params *inParamBlock,
                           char *inPath,
                           FileSession **outFile) {
  *outFile = NEW
  FileSession();
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params *inParamBlock) {
  ...
  FileSession *theFile = NULL;
  UInt32 theLen = sizeof(FileSession *);
  QTSS_Error theErr = QTSS_GetValue(inParamBlock->inClientSession,
                                    sFileSessionAttr,
                                    0,
                                    (void *) &theFile,
                                    &theLen);
  if ((theErr != QTSS_NoErr) || (theLen != sizeof(FileSession *))) {
    ...
    theErr = QTSS_GetValueAsString(inParamBlock->inRTSPRequest,
                                   qtssRTSPReqLocalPath,
                                   0,
                                   &theFullPath);
    Assert(theErr == QTSS_NoErr);
  ...
  Float32 bufferDelay =
      (Float32) 3.0; // FIXME need a constant defined for 3.0 value. It is used multiple places

  for (UInt32 x = 0; x < theFile->fSDPSource.GetNumStreams(); x++) {
    SourceInfo::StreamInfo
        *theStreamInfo = theFile->fSDPSource.GetStreamInfo(x);
  ...
  theErr = QTSS_AddRTPStream(inParamBlock->inClientSession,
                             inParamBlock->inRTSPRequest,
                             &newStream,
                             0);
  if (theErr != QTSS_NoErr)
    return theErr;
  ...
  theErr = QTSS_SetValue(newStream,
  ...
  (void) QTSS_GetValuePtr(newStream,
                          qtssRTPStrSSRC,
                          0,
                          (void **) &theTrackSSRC,
                          &theTrackSSRCSize);
  ...
  StrPtrLen theHeader;
  theErr = QTSS_GetValuePtr(inParamBlock->inRTSPHeaders,
                            qtssXRTPMetaInfoHeader,
                            0,
                            (void **) &theHeader.Ptr,
                            &theHeader.Len);
  if (theErr == QTSS_NoErr) {
    ...
    ::memcpy(theFields,
             QTRTPFile::GetSupportedRTPMetaInfoFields(),
             sizeof(RTPMetaInfoPacket::FieldID)
                 * RTPMetaInfoPacket::kNumFields);

    //
    // This function does the work of appending the response header based on the
    // fields we support and the requested fields.
    theErr =
        QTSSModuleUtils::AppendRTPMetaInfoHeader(inParamBlock->inRTSPRequest,
                                                 &theHeader,
                                                 theFields);
...
QTSS_Error SetupCacheBuffers(QTSS_StandardRTSP_Params *inParamBlock,
                             FileSession **theFile) {
  ...
  if (sEnableSharedBuffers && playCount
      == 1) // increments num buffers after initialization so do only once per session
    (*theFile)->fFile.AllocateSharedBuffers(sSharedBufferUnitKSize,
                                            sSharedBufferInc,
                                            sSharedBufferUnitSize,
                                            sSharedBufferMaxUnits);

  if (sEnablePrivateBuffers) // reinitializes buffers to current location so do every time
    (*theFile)->fFile.AllocatePrivateBuffers(sSharedBufferUnitKSize,
                                             sPrivateBufferUnitSize,
                                             sPrivateBufferMaxUnits);
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params *inParamBlock) {
  ...
  for (UInt32 x = 0; x < (*theFile)->fSDPSource.GetNumStreams(); x++) {
    SourceInfo::StreamInfo
        *theStreamInfo = (*theFile)->fSDPSource.GetStreamInfo(x);
    if (!(*theFile)->fFile.FindTrackEntry(theStreamInfo->fTrackID,
                                          &thePacketTrack))
      break;
    //(*theFile)->fFile.SetTrackQualityLevel(thePacketTrack, QTRTPFile::kAllPackets);
    (*theFile)->fFile.SetTrackQualityLevel(thePacketTrack,
                                           sDefaultStreamingQuality);
  }
  ...
  theErr = QTSS_GetValuePtr(inParamBlock->inRTSPHeaders,
                            qtssXPacketRangeHeader,
                            0,
                            (void **) &thePacketRangeHeader,
                            &theLen);
  if (theErr == QTSS_NoErr) {
    ...
    qtFileErr = (*theFile)->fFile.SeekToPacketNumber((*theFile)->fStopTrackID,
                                                     theStartPN);
    (*theFile)->fStartTime = (*theFile)->fFile.GetRequestedSeekTime();
  } else {
    Float64 *theStartTimeP = NULL;
    Float64 currentTime = 0;
    theErr = QTSS_GetValuePtr(inParamBlock->inRTSPRequest,
                              qtssRTSPReqStartTime,
                              0,
                              (void **) &theStartTimeP,
                              &theLen);
    if ((theErr != QTSS_NoErr) || (theLen
        != sizeof(Float64))) {   // No start time so just start at the last packet ready to send
    ...
    if (*theMaxBackupTime == -1) {
      ...
      Float64 theFirstPacketTransmitTime =
          (*theFile)->fFile.GetFirstPacketTransmitTime();
      ...
      theLen = 0;
      theErr = QTSS_GetValuePtr(inParamBlock->inClientSession,
                                qtssCliSesOverBufferEnabled,
                                0,
                                (void **) &overBufferEnabledPtr,
                                &theLen);
      if ((theErr == QTSS_NoErr) && (theLen == sizeof(bool))
          && *overBufferEnabledPtr)
        (*theFile)->fStartTime = *theStartTimeP;
      else
  ...
  bool adjustPauseTime =
      kAddPauseTimeToRTPTime; //keep rtp time stamps monotonically increasing
  if (true == QTSSModuleUtils::HavePlayerProfile(sServerPrefs,
                                                 inParamBlock,
                                                 QTSSModuleUtils::kDisablePauseAdjustedRTPTime))
  ...
  if ((**theFile).fLastPauseTime > 0)
    (**theFile).fTotalPauseTime +=
        OS::Milliseconds() - (**theFile).fLastPauseTime;
  ...
  for (UInt32 theStreamIndex = 0;
       QTSS_GetValuePtr(inParamBlock->inClientSession,
    ...
    theErr = QTSS_GetValuePtr(*theRef,
                              qtssRTPStrTrackID,
                              0,
                              (void **) &theTrackID,
                              &theLen);
    ...
    UInt16 theSeqNum = 0;
    UInt32 theTimestamp =
        (*theFile)->fFile.GetSeekTimestamp(*theTrackID); // this is the base timestamp need to add in paused time.

    Assert(theRef != NULL);

    if ((**theFile).fAdjustPauseTime) {
      UInt32 *theTimescale = NULL;
      QTSS_GetValuePtr(*theRef,
                       qtssRTPStrTimescale,
                       0,
                       (void **) &theTimescale,
                       &theLen);
      if (theLen
          != 0) // adjust the timestamps to reflect paused time else leave it alone we can't calculate the timestamp without a timescale.
      {
        UInt32 pauseTimeStamp = CalculatePauseTimeStamp(*theTimescale,
                                                        (*theFile)->fTotalPauseTime,
                                                        (UInt32) theTimestamp);
  ...
  theErr = QTSS_GetValue(inParamBlock->inRTSPRequest,
                         qtssRTSPReqSpeed,
                         0,
                         &theSpeed,
                         &theLen);
  ...
  // add the range header.
  {
    ...
    qtss_snprintf(rangeHeader,
                  sizeof(rangeHeader) - 1,
                  "npt=%.5f-%.5f",
                  (*theFile)->fStartTime,
                  (*theFile)->fStopTime);
  ...
  if (sPlayerCompatibility && QTSSModuleUtils::HavePlayerProfile(sServerPrefs,
                                                                 inParamBlock,
                                                                 QTSSModuleUtils::kDelayRTPStreamsUntilAfterRTSPResponse))
  ...
  theErr = QTSS_Play(inParamBlock->inClientSession,
                     inParamBlock->inRTSPRequest,
...
QTSS_Error SendPackets(QTSS_RTPSendPackets_Params *inParams) {
  ...
  if (theFile == NULL || (*theFile)->fStartTime == -1
      || (*theFile)->fPaused == true) //something is wrong
  {
    Assert(theFile != NULL);
  ...
  QTRTPFile::RTPTrackListEntry
      *theLastPacketTrack = (*theFile)->fFile.GetLastPacketTrack();

  while (true) {
    if ((*theFile)->fPacketStruct.packetData == NULL) {
      Float64 theTransmitTime =
          (*theFile)->fFile.GetNextPacket((char **) &(*theFile)->fPacketStruct.packetData,
                                          &(*theFile)->fNextPacketLen);
      ...
      if (((*theFile)->fStopTime != -1)
          && (theTransmitTime > (*theFile)->fStopTime)) {
      ...
      if (((*theFile)->fStopTrackID != 0)
          && ((*theFile)->fStopTrackID == theLastPacketTrack->TrackID)
          && (theLastPacketTrack->HTCB->fCurrentPacketNumber
              > (*theFile)->fStopPN)) {
        // We should indeed stop playing
        (void) QTSS_Pause(inParams->inClientSession);
        inParams->outNextPacketTime = qtssDontCallSendPacketsAgain;
      ...
      theTransmitTime = (*theFile)->fStartTime
          + (theOffsetFromStartTime / (*theFile)->fSpeed);
      ...
      (*theFile)->fPacketStruct.packetTransmitTime =
          (*theFile)->fAdjustedPlayTime + ((SInt64) (theTransmitTime * 1000));
    ...
    if ((!sDisableThinning) && (inParams->inCurrentTime
        > ((*theFile)->fLastQualityCheck + kQualityCheckIntervalInMsec))) {
      QTSS_RTPPayloadType
          thePayloadType = (QTSS_RTPPayloadType) theLastPacketTrack->Cookie2;
      if (thePayloadType == qtssVideoPayloadType) {
        ...
        theErr = QTSS_GetValuePtr(theStream,
                                  qtssRTPStrQualityLevel,
                                  0,
                                  (void **) &theQualityLevel,
                                  &theLen);
        Assert(theErr == QTSS_NoErr);
        Assert(theQualityLevel != NULL);
        Assert(theLen == sizeof(UInt32));

        (*theFile)->fFile.SetTrackQualityLevel(theLastPacketTrack,
                                               *theQualityLevel);
      }
    }
    ...
    UInt32 pauseTimeStamp =
        SetPausetimeTimeStamp(*theFile, theStream, currentTimeStamp);

    UInt16 curSeqNum = GetPacketSequenceNumber(theStream);
    (void) QTSS_SetValue(theStream,
                         sRTPStreamLastPacketSeqNumAttrID,
                         0,
                         &curSeqNum,
                         sizeof(curSeqNum));

    theErr = QTSS_Write(theStream,
                        &(*theFile)->fPacketStruct,
                        (*theFile)->fNextPacketLen,
                        NULL,
                        theFlags);
    ...
    if (theErr == QTSS_WouldBlock) {
      ...
      if (currentTimeStamp
          != pauseTimeStamp) // reset the packet time stamp so we adjust it again when we really do send it
      ...
      if ((*theFile)->fPacketStruct.suggestedWakeupTime == -1)
        inParams->outNextPacketTime =
            sFlowControlProbeInterval;    // for buffering, try me again in # MSec
      else {
        Assert((*theFile)->fPacketStruct.suggestedWakeupTime
                   > inParams->inCurrentTime);
        inParams->outNextPacketTime =
            (*theFile)->fPacketStruct.suggestedWakeupTime
                - inParams->inCurrentTime;
      }
...
QTSS_Error DestroySession(QTSS_ClientSessionClosing_Params *inParams) {
  ...
  (void) QTSS_SetValue(inParams->inClientSession,
                       ...
                       0,
                       &theNumSkippedSamples,
                       sizeof(theNumSkippedSamples));
