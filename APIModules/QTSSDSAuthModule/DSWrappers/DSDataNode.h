...
class DSDataNode {
 public:
  ...
  DSDataNode(tDirReference inDirRef,
      ...
      : mDirRef(inDirRef),
        mNode(::dsDataNodeAllocateString(inDirRef,
                                         inString)) {
    if (!mNode)
      throw_ds_error (eDSAllocationFailed);
  }
  ...
  size_t size(void) const throw() { return (size_t) ::dsDataNodeGetSize(mNode); }
  ...
  // Casting operators.
  operator tDataNodePtr() const throw() { return mNode; }
