...
static SInt32 _GetRecordList(
  ...
  *outRecCount = 0;
  ...
  status = ::dsFindDirNodes(inDSRef,
                            pDataBuff,
                            NULL,
                            eDSSearchNodeName,
                            (ds_API_PTR) &nodeCount,
                            &context);
  if (context != NULL) {
  ...
  if (status != eDSNoErr) {
    ...
    status = ::dsGetRecordList(nodeRef,
                               pDataBuff,
                               recName,
                               eDSExact,
                               recType,
...
static SInt32 _FindRecordNode(
  ...
  for (attrIndex = 1;
       (attrIndex <= pRecEntry->fRecordAttributeCount) && (status == eDSNoErr);
       attrIndex++) {
    tAttributeEntryPtr pAttrEntry = NULL;
    tAttributeValueListRef valueRef = 0;

    status = ::dsGetAttributeEntry(nodeRef,
                                   pRecBuff,
                                   attrListRef,
                                   attrIndex,
                                   &valueRef,
    ...
    if (std::strcmp(pAttrEntry->fAttributeSignature.fBufferData,
                    kDSNAttrMetaNodeLocation) == 0) {
      tAttributeValueEntry *pValueEntry = NULL;

      // If it matches then get the value of the attribute
      status =
          ::dsGetAttributeValue(nodeRef, pRecBuff, 1, valueRef, &pValueEntry);
      if ((status == eDSNoErr) && (pValueEntry != NULL)) {
        ...
        if (outHomeNodeName->fDataNodeCount != 0) {
          ...
          status = ::dsBuildListFromPathAlloc(inDSRef,
                                              outHomeNodeName,
...
Bool16 DSAccessChecker::CheckPassword(const char *inUsername,
  ...
  if (status != eDSNoErr) {
    ...
    debug_printf(
          "QTSSODAuthModule: Could not open Directory Services - error: %"_S32BITARG_"",
          status);
    return false;
  }

  status = _FindRecordNode(dsRef,
                           NULL,
                           inUsername,
  ...
  if (status == eDSNoErr) {
    ...
    tDataNode *pAuthType =
        ::dsDataNodeAllocateString(dsRef, kDSStdAuthNodeNativeClearTextOK);
    tDataBuffer *pStepBuff = ::dsDataBufferAllocate(dsRef, 128);
    tDataBuffer *pAuthBuff = ::dsDataBufferAllocate(dsRef,
                                                    (sizeof(UInt32)
                                                        + sizeof(UInt32) + uiLen
                                                        + std::strlen(inPassword)));
    ...
    if ((pStepBuff != NULL) && (pAuthType != NULL) && (pAuthBuff != NULL)) {
      ...
      status =
          ::dsDoDirNodeAuth(nodeRef, pAuthType, 1, pAuthBuff, pStepBuff, NULL);
...
Bool16 DSAccessChecker::CheckDigest(const char *inUsername,
                                    const char *inServerChallenge,
                                    const char *inClientResponse,
                                    const char *inMethod) {
  return false;
}
...
Bool16 DSAccessChecker::CheckGroupMembership(const char *inUsername,
