...
static QTSS_Error QTSSRawFileModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSRawFileModuleDispatch(QTSS_Role inRole,
                                     QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    case QTSS_Register_Role:return Register(&inParams->regParams);
    case QTSS_RTSPPreProcessor_Role:return Preprocess(&inParams->rtspPreProcessorParams);
  }
  return QTSS_NoErr;
}
...
QTSS_Error Preprocess(QTSS_StandardRTSP_Params *inParams) {
  ...
  if ((theStateP == NULL) || (theLen != sizeof(UInt32))) {
    ...
    theErr =
        QTSS_OpenFileObject(rawPath.GetObject(), qtssOpenFileAsync, &theFile);
#else
    ...
    if (theErr != QTSS_NoErr) {
      ...
      if (theLen > sRawSuffix.Len) {
        StrPtrLen
            comparer((theFilePath + theLen) - sRawSuffix.Len, sRawSuffix.Len);
        if (comparer.Equal(sRawSuffix)) {
#if RAWFILE_FILE_ASYNC
          theErr = QTSS_OpenFileObject(rawPath.GetObject(),
                                       qtssOpenFileAsync,
                                       &theFile);
#else
