...
static QTSS_Error QTSSProxyModuleDispatch(QTSS_Role inRole,
...
static QTSS_Error HandleRTSPClientErr(QTSS_RTSPRequestObject inRequest,
                                      ProxyClientInfo *inClient,
                                      QTSS_Error inErr);
static void RequestSocketEvent(QTSS_StreamRef inStream, UInt32 inEventMask);
static QTSS_Error DoRequestPreProcessing(ProxyClientInfo *inProxyClientInfo,
                                         QTSS_RTSPRequestObject inRequest,
                                         QTSS_ClientSessionObject inSession);
static QTSS_Error DoRequestPostProcessing(ProxyClientInfo *inProxyClientInfo,
                                          QTSS_RTSPRequestObject inRequest,
                                          QTSS_ClientSessionObject inClientSession);
static UInt32 ReplaceSessionAndTransportHeaders(StrPtrLen *inResponse,
                                                iovec *outNewResponse,
                                                StrPtrLen *inNewSessionID,
                                                StrPtrLen *inNewTransportHeader,
                                                UInt32 *outNewResponseLen);
...
QTSS_Error QTSSProxyModuleDispatch(QTSS_Role inRole,
                                   QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    ...
    case QTSS_RTSPRequest_Role:return ProcessRTSPRequest(&inParams->rtspRequestParams);
    case QTSS_ClientSessionClosing_Role:return DestroySession(&inParams->clientSessionClosingParams);
    case QTSS_RTCPProcess_Role:return ProcessIncomingRTCPPacket(&inParams->rtcpProcessParams);
...
QTSS_Error Register(QTSS_Register_Params *inParams) {
  ...
  (void) QTSS_AddStaticAttribute(qtssClientSessionObjectType,
                                 sProxyClientInfoName,
                                 NULL,
                                 qtssAttrDataTypeVoidPointer);
  (void) QTSS_IDForAttr(qtssClientSessionObjectType,
                        sProxyClientInfoName,
                        &sProxyClientInfoAttr);

  static char *sPortNumberToBigErrName = "QTSSProxyModulePortNumberTooBig";
  static char *sRemoteHostRefusedConnectionErrName =
      "QTSSProxyModuleRemoteHostRefusedConnection";
  static char *sNoTransportHeaderInSetupErrName =
      "QTSSProxyModuleNoTransportHeaderInSetup";

  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sPortNumberToBigErrName,
                                 NULL,
  ...
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sRemoteHostRefusedConnectionErrName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sRemoteHostRefusedConnectionErrName,
  ...
  (void) QTSS_AddStaticAttribute(qtssTextMessagesObjectType,
                                 sNoTransportHeaderInSetupErrName,
                                 NULL,
                                 qtssAttrDataTypeCharArray);
  (void) QTSS_IDForAttr(qtssTextMessagesObjectType,
                        sNoTransportHeaderInSetupErrName,
                        &sNoTransportHeaderInSetupErr);
...
QTSS_Error DestroySession(QTSS_ClientSessionClosing_Params *inParams) {
  ...
  QTSS_Error
      theErr = QTSS_GetValue(inParams->inClientSession, sProxyClientInfoAttr, 0,
                             &theClient, &theLen);
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params *inParams) {
  ...
  if (theErr != QTSS_NoErr) {
    ...
    theErr = QTSS_GetValuePtr(inParams->inRTSPHeaders,
                              qtssHostHeader,
                              0,
                              (void **) &theDNSNameAndPort.Ptr,
                              &theDNSNameAndPort.Len);
  ...
  if (!theClient->GetRTSPClient()->IsTransactionInProgress())
    theErr = DoRequestPreProcessing(theClient,
  ...
  if (theErr != QTSS_NoErr)
    ...
    theErr = DoRequestPostProcessing(theClient,
                                     inParams->inRTSPRequest,
                                     inParams->inClientSession);
...
QTSS_Error HandleRTSPClientErr(QTSS_RTSPRequestObject inRequest,
                               ProxyClientInfo *inClient,
                               QTSS_Error inErr) {
  if (inClient->GetSocketStream() == NULL) {
    ...
    QTSS_Error theErr =
        QTSS_CreateStreamFromSocket(inClient->GetRTSPClient()->GetSocket()->GetSocket()->GetSocketFD(),
                                    &theSockStream);
    Assert(theErr == QTSS_NoErr);
  ...
  if ((inErr == EAGAIN) || (inErr == EINPROGRESS)) {
    ...
    RequestSocketEvent(inClient->GetSocketStream(),
                       inClient->GetRTSPClient()->GetSocket()->GetEventMask());
...
QTSS_Error DoRequestPreProcessing(ProxyClientInfo *inProxyClientInfo,
  ...
  StrPtrLen *theProxyClientSessionIDPtr =
      inProxyClientInfo->GetRTSPClient()->GetSessionID();
  ...
  if (*theMethod == qtssSetupMethod) {
    ...
    theErr = QTSS_AddRTPStream(inSession,
                               inRequest,
                               &theStream,
    ...
    ProxyDemuxerTask
        *theProxyDemuxerTask = inProxyClientInfo->AddStream(theStream);

    //
    // Build a new Transport header
    UInt16 theRTPPort =
        theProxyDemuxerTask->GetSockets()->GetSocketA()->GetLocalPort();

    qtss_sprintf(theTransportHeaderBuf,
  ...
  UInt32 theNumVecs = ReplaceSessionAndTransportHeaders(&theRequest,
                                                        theNewRequest,
                                                        &theProxyClientSessionID,
                                                        &theTransportHeader,
                                                        &totalResponseLen);

  return inProxyClientInfo->GetRTSPClient()->SendRTSPRequest(theNewRequest,
                                                             theNumVecs);
}

QTSS_Error DoRequestPostProcessing(ProxyClientInfo *inProxyClientInfo,
                                   QTSS_RTSPRequestObject inRequest,
                                   QTSS_ClientSessionObject inClientSession) {

  QTSS_RTSPMethod *theMethod = NULL;
  UInt32 theLen = 0;
  QTSS_Error theErr = QTSS_GetValuePtr(inRequest,
                                       qtssRTSPReqMethod,
                                       0,
                                       (void **) &theMethod,
                                       &theLen);
  ...
  theErr = QTSS_GetValuePtr(inClientSession,
                            qtssCliSesRTSPSessionID,
                            0,
                            (void **) &theSessionID.Ptr,
                            &theSessionID.Len);
  Assert(theErr == QTSS_NoErr);

  StrPtrLen theResponse(inProxyClientInfo->GetRTSPClient()->GetResponse(),
                        inProxyClientInfo->GetRTSPClient()->GetResponseLen());

  if ((*theMethod == qtssSetupMethod) &&
      (inProxyClientInfo->GetRTSPClient()->GetStatus() == 200)) {
    ProxyDemuxerTask
        *thisStreamsDemuxer = inProxyClientInfo->GetLastDemuxerTask();
    ...
    theErr = QTSS_GetValue(thisStreamsDemuxer->GetStream(),
    ...
    theLen = 20;
    theErr = QTSS_GetValue(inClientSession,
    ...
    qtss_sprintf(theTransportHeader.Ptr,
                 "Transport: RTP/AVP;unicast;client_port=%d-%d;server_port=%d-%d;source=%s\r\n",
                 theCliRTPPort,
                 theCliRTPPort + 1,
                 theSvrRTPPort,
                 theSvrRTPPort + 1,
  ...
  UInt32 theNumVecs = ReplaceSessionAndTransportHeaders(&theResponse,
                                                        &theNewResponse[1],
                                                        &theSessionID,
                                                        &theTransportHeader,
                                                        &totalResponseLen);
  ...
  theNewResponse[theNumVecs].iov_base =
      inProxyClientInfo->GetRTSPClient()->GetContentBody();
  theNewResponse[theNumVecs].iov_len =
      inProxyClientInfo->GetRTSPClient()->GetContentLength();
  totalResponseLen += theNewResponse[theNumVecs].iov_len;
  theNumVecs++;

  UInt32 lenWritten = 0;
  theErr = QTSS_WriteV(inRequest,
                       theNewResponse,
                       theNumVecs,
                       totalResponseLen,
                       &lenWritten);
...
UInt32 ReplaceSessionAndTransportHeaders(StrPtrLen *inResponse,
  ...
  *outNewResponseLen = 0;
  ...
  while (reqParser.GetDataRemaining() > 0) {
    ...
    if (theHeaderName.EqualIgnoreCase(sTransportHeader.Ptr,
                                      sTransportHeader.Len)) {
      //
      // Mark off the length of the last section of the header
      outNewResponse[curVecIndex].iov_len =
          (UInt32) ((PointerSizedUInt) theHeaderName.Ptr)
              - ((PointerSizedUInt) outNewResponse[curVecIndex].iov_base);
      ...
      *outNewResponseLen += inNewTransportHeader->Len;
    ...
    } else if (theHeaderName.EqualIgnoreCase(sSessionHeader.Ptr,
                                             sSessionHeader.Len)) {
      ...
      // Mark off the length of the last section of the header
      outNewResponse[curVecIndex].iov_len =
          (UInt32) ((PointerSizedUInt) reqParser.GetCurrentPosition())
              - ((PointerSizedUInt) outNewResponse[curVecIndex].iov_base);
      *outNewResponseLen += outNewResponse[curVecIndex].iov_len;
      ...
      *outNewResponseLen += inNewSessionID->Len;
      ...
      outNewResponse[curVecIndex].iov_base =
          &theSessionID.Ptr[theSessionID.Len];
...
QTSS_Error ProcessIncomingRTCPPacket(QTSS_RTCPProcess_Params *inParams) {
  ...
  ProxyDemuxerTask
      *theTask = theClient->GetDemuxerTaskForStream(inParams->inRTPStream);
  Assert(theTask != NULL);
  if (theTask == NULL)
    return QTSS_NoErr;
  ...
  (void) theTask->GetSockets()->GetSocketB()->
      SendTo(theTask->GetRemoteAddr(),
             theTask->GetOriginServerPort() + 1,
             inParams->inRTCPPacketData,
             inParams->inRTCPPacketDataLen);
...
SInt64 ProxyTask::Run() {
  ...
  for (OSQueueIter iter(sSocketPool->GetSocketQueue()); !iter.IsDone();
       iter.Next()) {
    UInt32 theRemoteAddr = 0;
    UInt16 theRemotePort = 0;

    UDPSocketPair
        *thePair = (UDPSocketPair *) iter.GetCurrent()->GetEnclosingObject();
    Assert(thePair != NULL);
    ...
    for (UInt32 x = 0; x < 2; x++) {
      ...
      while (true) {
        ...
        theSocket->RecvFrom(&theRemoteAddr,
                            &theRemotePort,
                            thePacketStruct.packetData,
                            kMaxRTCPPacketSize,
                            &thePacketLen);
        if (thePacketLen == 0)
          break;//no more packets on this socket!

        ProxyDemuxerTask *theDemuxerTask =
            (ProxyDemuxerTask *) theSocket->GetDemuxer()->GetTask(theRemoteAddr,
                                                                  0);
        if (theDemuxerTask != NULL) {
          QTSS_RTPStreamObject theStream = theDemuxerTask->GetStream();
          (void) QTSS_Write(theStream,
                            &thePacketStruct,
                            thePacketLen,
                            NULL,
                            theFlags);
...
UDPSocketPair *ProxySocketPool::ConstructUDPSocketPair() {
  ...
  return NEW
  UDPSocketPair(NEW
  UDPSocket(sProxyTask,
            UDPSocket::kWantsDemuxer | Socket::kNonBlockingSocketType),
      NEW
  UDPSocket(sProxyTask,
            UDPSocket::kWantsDemuxer | Socket::kNonBlockingSocketType));
}
...
ProxyClientInfo::~ProxyClientInfo() {
  ...
  for (OSQueueIter iter(&fDemuxerTaskQueue); !iter.IsDone();) {
    ...
    ProxyDemuxerTask
        *theTask = (ProxyDemuxerTask *) theElem->GetEnclosingObject();
    ...
    thePair->GetSocketA()->GetDemuxer()->UnregisterTask(theHostAddr,
                                                        0,
                                                        theTask);
    thePair->GetSocketB()->GetDemuxer()->UnregisterTask(theHostAddr,
                                                        0,
                                                        theTask);
...
ProxyDemuxerTask *ProxyClientInfo::GetDemuxerTaskForStream(QTSS_RTPStreamObject inStream) {
  ...
  for (OSQueueIter iter(&fDemuxerTaskQueue); !iter.IsDone(); iter.Next()) {
    ProxyDemuxerTask
        *theTask = (ProxyDemuxerTask *) iter.GetCurrent()->GetEnclosingObject();
...
ProxyDemuxerTask *ProxyClientInfo::AddStream(QTSS_RTPStreamObject inStream) {
  ...
  UDPSocketPair
      *thePair = sSocketPool->GetUDPSocketPair(theLocalAddr, 0, theHostAddr, 0);

  fLastDemuxerTask = NEW
  ProxyDemuxerTask(inStream, thePair);
  ...
  thePair->GetSocketA()->GetDemuxer()->RegisterTask(theHostAddr,
                                                    0,
                                                    fLastDemuxerTask);
  thePair->GetSocketB()->GetDemuxer()->RegisterTask(theHostAddr,
                                                    0,
                                                    fLastDemuxerTask);
