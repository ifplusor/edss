...
static StrPtrLen sRespHeaderPrefix
    ("HTTP/1.0 200 OK\r\nServer: QTSS/2.0\r\nConnection: Close");
...
static Bool16 sDefaultHTTPFileXferEnabled =
    false; // This module is not enabled by default.
...
class QTSSHttpAccessLog : public QTSSRollingLog {
 public:
  ...
  QTSSHttpAccessLog()
      : QTSSRollingLog() { this->SetTaskName("QTSSHttpAccessLog"); }
...
// FUNCTIONS
static QTSS_Error QTSSHttpFileModuleDispatch(QTSS_Role inRole,
...
QTSS_Error QTSSHttpFileModuleDispatch(QTSS_Role inRole,
                                      QTSS_RoleParamPtr inParams) {
  switch (inRole) {
    ...
    case QTSS_RTSPSessionClosing_Role:return CloseRTSPSession(&inParams->rtspSessionClosingParams);
  }
  return QTSS_NoErr;
}
...
QTSS_Error RereadPrefs() {
  ...
  QTSSModuleUtils::GetAttribute(sPrefs,
                                ...
                                &sHTTPFileXferEnabled,
                                &sDefaultHTTPFileXferEnabled,
                                sizeof(sHTTPFileXferEnabled));
  QTSSModuleUtils::GetAttribute(sPrefs,
                                "http_logging",
                                qtssAttrDataTypeBool16,
                                &sLogEnabled,
...
QTSS_Error FilterRequest(QTSS_Filter_Params *inParams) {
  ...
  (void) QTSS_GetValuePtr(theSession,
                          sStateAttr,
                          0,
                          (void **) &theStateP,
                          &theLen);
  if ((theStateP == NULL) || (theLen != sizeof(UInt32))) {
    ...
    (void) QTSS_GetValuePtr(theRequest,
                            ...
                            &theFullRequest.Len);
    httpRequest = NEW
    HTTPRequest(&serverHdr, &theFullRequest);
    ...
    QTSS_Error err = QTSS_GetValueAsString(sServerPrefs,
    ...
    OSCharArrayDeleter httpFolderDtr
        (QTSSModuleUtils::GetStringAttribute(sPrefs, "http_folder", ""));
    ...
    if (theMovieFolder.Len != 0) {
      ...
      thePath = NEW
      char[theMovieFolder.Len + sPathSeparator.Len + reqStr.Len + 1];
      ...
      if (theDirectory
          == NULL) {                       // If it's not a directory in the movie folder
#if HTTP_FILE_ASYNC
        ...
        {                           // for a single movie file
          ...
          delete[] thePath;
        } else {                   // If it's not a file in the movie folder,
    ...
    if (type == 0 && theHttpFolder.Len != 0) {
      ...
      thePath = NEW
      char[theHttpFolder.Len + sPathSeparator.Len + reqStr.Len + 1];
      ...
      {                               // it is a  request for progressive download
        ...
        delete[] thePath;
      } else {                                       // If the file is not found in the http folder, just return
        if (theFile != NULL)
          (void) QTSS_CloseFileObject(theFile);
        delete[] thePath;
        type = 0;
    ...
    if (reqMethod == httpHeadMethod) {
      ...
      httpRequest->AppendResponseHeader(httpContentTypeHeader,
                                        &sQuickTimeMimeType);
    ...
    switch (type) {
      case transferHttpFile: {                               // Allocate memory for theFileBuffer
        theFileBuffer = NEW
        char[sFileBufSize];
        httpRequest->CreateResponseHeader(http10Version, httpOK);
        httpRequest->AppendConnectionCloseHeader();
        theLen = sizeof(UInt64);
        theErr = QTSS_GetValue(theFile,
                               qtssFlObjLength,
                               0,
                               (void *) &theFileLength,
                               &theLen);
      ...
      case transferRefMovieFolder: {
        ...
        while ((entry = ::readdir(theDirectory)) != NULL) {
          if (::strcmp(entry->d_name + ::strlen(entry->d_name) - 4, ".mov")
              == 0)
            fileCount++;
        ...
        OSCharArrayDeleter baseUrl(NEW char[sRTSPUrlPrefix.Len
            + serverIPAddr.Len + sPathSeparator.Len + reqStr.Len
            + sPathSeparator.Len + 1]);
        ::memcpy(baseUrl.GetObject(), sRTSPUrlPrefix.Ptr, sRTSPUrlPrefix.Len);
        index = sRTSPUrlPrefix.Len;
        ::memcpy(baseUrl.GetObject() + index,
                 serverIPAddr.Ptr,
                 serverIPAddr.Len);
        ...
        UInt32 *rmdas = NULL, rmdasLen = 0, *moov = NULL, moovLen = 0,
            *tempRmda = NULL, tempRmdaLen = 0;
        UInt32 *rmdasTemp;
        UInt32 arrayIndex;
        char *filePath;
        char *url;
        ...
        for (arrayIndex = 0; arrayIndex < fileCount; arrayIndex++) {
          ...
          filePath = NEW
          char[::strlen(thePath) + sPathSeparator.Len
              + ::strlen(fileNames[arrayIndex]) + 1];
          ...
          url = NEW
          char[index + ::strlen(fileNames[arrayIndex]) + 1];
          ...
          if (tempRmdaLen != 0) {
            rmdasTemp =
                rmdas;  // Store the rmdas in a temporary pointer and reallocate
            rmdas = NEW
            UInt32[rmdasLen + tempRmdaLen];
            ...
            ::memcpy((char *) rmdas,
                     rmdasTemp,
                     rmdasLen); // Copy old contents and append tempRmda
      ...
      case transferRefMovieFile: {
        ...
        OSCharArrayDeleter refMovieBuf(NEW char[sRefMovieBufPrefix.Len
            + sRTSPUrlPrefix.Len + serverIPAddr.Len + sPathSeparator.Len
            + reqStr.Len + 1]);
        ::memcpy(refMovieBuf.GetObject(),
                 sRefMovieBufPrefix.Ptr,
                 sRefMovieBufPrefix.Len);
        index = sRefMovieBufPrefix.Len;
        ::memcpy(refMovieBuf.GetObject() + index,
                 sRTSPUrlPrefix.Ptr,
                 sRTSPUrlPrefix.Len);
        index += sRTSPUrlPrefix.Len;
        ::memcpy(refMovieBuf.GetObject() + index,
    ...
    (void) QTSS_SetValue(theSession,
                         ...
                         0,
                         &theFileBuffer,
                         sizeof(theFileBuffer));
    (void) QTSS_SetValue(theSession,
                         sFileBufferLenAttr,
                         0,
                         &fileBufferLen,
                         sizeof(fileBufferLen));
  ...
  (void) QTSS_GetValuePtr(theSession,
                          sFileBufferLenAttr,
                          0,
                          (void **) &theFileBufferLenP,
                          &theLen);
  Assert(theFileBufferLenP != NULL);
  Assert(theLen == sizeof(UInt32 *));

  UInt32 *theWriteOffsetP =
      NULL;                 // Write offset for the file buffer
  (void) QTSS_GetValuePtr(theSession,
                          sWriteOffsetAttr,
                          0,
                          (void **) &theWriteOffsetP,
                          &theLen);
  ...
  while (!isBlocked) {
    ...
    switch (theState) {
      ...
      case kWritingBufferState:   // For both the ref movie directory transfer and the http file transfer
      {
        ...
        (void) QTSS_Write(theSession,
                          (*theFileBufferP) + theWriteOffset,
                          theFileBufferLen - theWriteOffset,
                          &theWrittenLen,
                          0);
...
void LogRequest(QTSS_RTSPSessionObject inRTSPSession) {
  ...
  Bool16 result =
      QTSSRollingLog::FormatDate(theDateBuffer, true);         // date field
  ...
  if (inRTSPSession != NULL)
    (void) QTSS_GetValue(inRTSPSession,
  ...
  (void) QTSS_GetValuePtr(inRTSPSession,
                          sRequestAttr,
                          0,
                          (void **) &theRequestP,
                          &theLen);
  ...
  qtss_sprintf(tempLogBuffer, "%s %s %s %s \"%s\" %s %s\n",
               ...
               (request.GetObject()[0] == '\0') ? sVoidField
                                                : request.GetObject(),
               sStatus,
