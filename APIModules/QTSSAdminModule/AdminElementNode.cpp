...
ElementNode *ElementNode::CreateArrayAttributeNode(UInt32 index,
  ...
  if (this->GetNodeInfoPtr(index)
      == NULL) {   //qtss_printf("ElementNode::CreateArrayAttributeNode index = %"   _U32BITARG_   " this->GetNodeInfoPtr(index) == NULL \n",index);
  }
  ...
  for (UInt32 i = 0; !nodePtr->IsStopItem(i); i++) {
    ...
    if (fieldPtr->fAPI_Type != qtssAttrDataTypeQTSS_Object) {
      ...
      Warn(err == QTSS_NoErr);
      if (err
          != QTSS_NoErr) {   //qtss_printf("Error Getting Value for %s type = qtssAttrDataTypeQTSS_Object err = %"   _U32BITARG_   "\n", fieldPtr->fFieldName,err);
  ...
  if (err == QTSS_NoErr) {
    ...
    for (UInt32 i = 0; !IsStopItem(i); i++) {
      if (defaultAttributeInfo == NULL) {
        ...
        Assert(err == QTSS_NoErr);
        if (err
            != QTSS_NoErr) {   //qtss_printf("QTSS_GetAttrInfoByIndex returned err = %"   _U32BITARG_   " \n",err);
        }
      } else {
      ...
      if ((SInt32) fFieldIDs[i].fAPI_ID
          < 0) {   //qtss_printf("ElementNode::InitializeAllFields name = %s index = %" _S32BITARG_ " numValues =%"   _U32BITARG_   " \n",fFieldIDs[i].fFieldName, (SInt32) fFieldIDs[i].fAPI_ID,numValues);
      }
      ...
      if (err
          != QTSS_NoErr) {   //qtss_printf("ElementNode::InitializeAllFields  Register returned err = %"   _U32BITARG_   " field = %s node=%s \n",err,GetName(i),GetMyName());
      }
      Assert(err == QTSS_NoErr);
    }
  }
...
bool ElementNode::SetUpOneDataField(UInt32 index) {
  ...
  //Warn(NULL == testPtr);
  if (NULL
      != testPtr) {   //qtss_printf(" ElementNode::SetUpOneDataField skip field already setup parent = %s field name=%s\n",GetNodeName(), GetName(index));
    return isNodeResult;
  }
...
void ElementNode::SetUpSingleElement(QueryURI *queryPtr,
  ...
  } else {   //qtss_printf("ElementNode::SetUpSingleElement filter element=%s\n",GetName(index));
  }
}
...
char *ElementNode::NewIndexElement(QTSS_Object inObject,
  ...
  if (inObject != NULL) {
    ...
    if (err
        != QTSS_NoErr) {   //qtss_printf("ElementNode::NewIndexElement QTSS_GetValueAsString object= %p id=%"   _U32BITARG_   " index=%"   _U32BITARG_   " err= %" _S32BITARG_ " \n",inObject,inID, inIndex, err);
    }
  }
  return resultPtr;
}
...
void ElementNode::RespondWithSelfAdd(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondWithSelfAdd not Initialized EXIT\n");
    return;
  }
  if (NULL
      == queryPtr) {   //qtss_printf("ElementNode::RespondWithSelfAdd NULL == queryPtr EXIT\n");
    return;
  }

  if (NULL
      == inStream) {   //qtss_printf("ElementNode::RespondWithSelfAdd NULL == inStream EXIT\n");
    return;
  }
  ...
  if (NULL
      == dataPtr) {   //qtss_printf("ElementNode::RespondWithSelfAdd NULL == dataPtr EXIT\n");
    dataPtr = nullErr;
    nullData = true;
  }
...
void ElementNode::RespondWithSelf(QTSS_StreamRef inStream, QueryURI *queryPtr) {
  ...
  if (GetNodeName()
      == NULL) {   //qtss_printf("ElementNode::RespondWithSelf Node = %s is Uninitialized no name so LEAVE\n",GetNodeName() );
    return;
  }

  if (!fInitialized) {   //qtss_printf("ElementNode::RespondWithSelf not Initialized EXIT\n");
    return;
  }

  if (NULL
      == queryPtr) {   //qtss_printf("ElementNode::RespondWithSelf NULL == queryPtr EXIT\n");
    return;
  }
  ...
  } else {   //qtss_printf(" ****** ElementNode::RespondWithSelf NOT a node **** \n");
    (void) QTSS_Write(inStream, GetNodeName(), GetNodeNameLen(), NULL, 0);
    (void) QTSS_Write(inStream, "=", 1, NULL, 0);
...
void ElementNode::RespondToAdd(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondToAdd not Initialized EXIT\n");
    return;
  }
  if (NULL
      == queryPtr) {   //qtss_printf("ElementNode::RespondToAdd NULL == queryPtr EXIT\n");
    return;
  }

  if (NULL
      == inStream) {   //qtss_printf("ElementNode::RespondToAdd NULL == inStream EXIT\n");
    return;
  }

  char *dataPtr = GetElementDataPtr(index);
  if (NULL
      == dataPtr) {   //qtss_printf("ElementNode::RespondToAdd NULL == dataPtr EXIT\n");
    //  return;
    dataPtr = nullErr;
    nullData = true;
  }
...
void ElementNode::RespondToSet(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondToSet not Initialized EXIT\n");
    return;
  }
  if (NULL
      == queryPtr) {   //qtss_printf("ElementNode::RespondToSet NULL == queryPtr EXIT\n");
    return;
  }

  if (NULL
      == inStream) {   //qtss_printf("ElementNode::RespondToSet NULL == inStream EXIT\n");
    return;
  }

  char *dataPtr = GetElementDataPtr(index);
  if (NULL
      == dataPtr) {   //qtss_printf("ElementNode::RespondToSet NULL == dataPtr EXIT\n");
    //  return;
    dataPtr = nullErr;
    nullData = true;
  }
  ...
  if (GetFieldType(index) == eNode) {
    ...
    do {
      ...
      err = QTSS_StringToValue(value.GetObject(),
                               ...
                               valueBuff,
                               &len);
      if (err) {   //qtss_sprintf(messageBuffer,  "QTSS_Error=%" _S32BITARG_ " from QTSS_ConvertStringToType",err);
        break;
      }
      ...
      err = QTSS_SetValue(GetSource(),
                          ...
                          valueBuff,
                          len);
      if (err) {   //qtss_sprintf(messageBuffer,  "QTSS_Error=%" _S32BITARG_ " from QTSS_SetValue",err);
        break;
      }
...
void ElementNode::RespondToDel(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondToDel not Initialized EXIT\n");
    return;
  }
  if (NULL
      == queryPtr) {   //qtss_printf("ElementNode::RespondToDel NULL == queryPtr EXIT\n");
    return;
  }

  if (NULL
      == inStream) {   //qtss_printf("ElementNode::RespondToDel NULL == inStream EXIT\n");
    return;
  }
  ...
  char *dataPtr = GetElementDataPtr(index);
  if (NULL
      == dataPtr) {   //qtss_printf("ElementNode::RespondToDel NULL == dataPtr EXIT\n");
    //  return;
    dataPtr = nullErr;
    nullData = true;
  }
...
void ElementNode::RespondToGet(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondToGet not Initialized EXIT\n");
    return;
  }
  if (NULL
      == queryPtr) {   //qtss_printf("ElementNode::RespondToGet NULL == queryPtr EXIT\n");
    return;
  }

  if (NULL
      == inStream) {   //qtss_printf("ElementNode::RespondToGet NULL == inStream EXIT\n");
    return;
  }

  char *dataPtr = GetElementDataPtr(index);
  if (NULL
      == dataPtr) {   //qtss_printf("ElementNode::RespondToGet NULL == dataPtr EXIT\n");
    //  return;
    dataPtr = nullErr;
    nullData = true;
  }
...
void ElementNode::RespondWithNodeName(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {    //qtss_printf("ElementNode::RespondWithNodeName not Initialized EXIT\n");
    return;
  }
...
void ElementNode::RespondWithAllElements(QTSS_StreamRef inStream,
  ...
  if (GetNodeName()
      == NULL) {   //qtss_printf("ElementNode::RespondWithAllElements %s is Uninitialized LEAVE\n",GetNodeName());
    return;
  }

  if (!fInitialized) {   //qtss_printf("ElementNode::RespondWithAllElements %s is Uninitialized LEAVE\n",GetNodeName());
    return;
  }
...
void ElementNode::RespondWithAllNodes(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondWithAllNodes %s is Uninitialized LEAVE\n",GetNodeName());
    return;
  }

  if (GetNodeName()
      == NULL) {   //qtss_printf("ElementNode::RespondWithAllNodes %s is Uninitialized LEAVE\n",GetNodeName());
    return;
  }
...
void ElementNode::RespondToQuery(QTSS_StreamRef inStream,
  ...
  if (!fInitialized) {   //qtss_printf("ElementNode::RespondToQuery %s is Uninitialized LEAVE\n",GetNodeName());
    return;

  }

  if (GetNodeName()
      == NULL) {   //qtss_printf("ElementNode::RespondToQuery %s is Uninitialized LEAVE\n",GetNodeName());
    return;
  }
...
void ElementNode::SetupNodes(QueryURI *queryPtr,
  ...
  if (fSelfPtr
      == NULL) {   //qtss_printf("******* ElementNode::SetupNodes (fSelfPtr == NULLL) \n");
  }
  Assert(NULL != queryPtr);
  Assert(NULL != currentPathPtr);
  ...
  if (queryPtr && currentPathPtr)
    do {
      ...
      if (NULL
          == GetElementDataPtr(index)) {   //qtss_printf("ElementNode::SetupNodes call SetUpSingleElement index=%"   _U32BITARG_   " nextSegment=");PRINT_STR( &nextSegment);
        SetUpSingleElement(queryPtr,
                           currentPathPtr,
                           &nextSegment,
                           index,
...
AdminClass::~AdminClass() {   //qtss_printf("AdminClass::~AdminClass() \n");
