...
// FUNCTIONS
static QTSS_Error QTSSDemoSMILDispatch(QTSS_Role inRole,
...
QTSS_Error CountRequest(QTSS_RTSPRequestObject inRTSPRequest,
...
void InitHitCountFromFile() {
  ...
  int c = 0;
  int i = 0;
  while ((c = fscanf(hitfile,
                     "%s %d",
                     gHitcountArray[i].url,
                     &gHitcountArray[i].hitcount)) == 2) {
...
int HitCountCompare(const void *hitCount1, const void *hitCount2) {
  return ((const HitCount *) hitCount2)->hitcount
      - ((const HitCount *) hitCount1)->hitcount;
}
...
void GenerateHotHitSMIL(char *buffer) {
  ...
  while ((p = strstr(templateCursor, "%s")) != 0) {
    char saveCh = p[2];
    p[2] = '\0';
    int len = qtss_sprintf(bufferCursor,
                           templateCursor,
                           gHitcountArray[hitNum++].url);
    p[2] = saveCh;
    bufferCursor += len;
    templateCursor = &p[2];
  }
...
QTSS_Error QTSSDemoSMILDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams) {
  ...
  switch (inRole) {
    ...
    case QTSS_ClientSessionClosing_Role:return ClientSessionClosing(&inParams->clientSessionClosingParams);
  }
  return QTSS_NoErr;
}
...
QTSS_Error FilterRequest(QTSS_Filter_Params *inParams) {
  ...
  if (!(reqMethod.Equal(StrPtrLen("GET"))
      || reqMethod.Equal(StrPtrLen("HEAD"))))
