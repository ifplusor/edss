...
QTSS_Error QTSSFilePrivsModuleDispatch(QTSS_Role inRole,
...
UInt32 GetPathParentDestructive(const StrPtrLen *thePathPtr,
                                StrPtrLen *resultPathPtr,
                                UInt32 maxLen);
...
QTSS_Error QTSSFilePrivsModuleDispatch(QTSS_Role inRole,
  ...
  switch (inRole) {
    ...
    case QTSS_RTSPAuthorize_Role:return AuthenticateRTSPRequest(&inParams->rtspRequestParams);
      break;
...
Bool16 CheckFileAccess(struct passwd *passwdStructPtr,
                       StrPtrLen */*nameStrPtr*/,
                       char *pathBuff) {
  Bool16 result = false;
  ...
  do // once only check for the current entity
  {
    ...
    if (statData.st_uid == (UInt16) passwdStructPtr->pw_uid) // check if owner
    {
      ...
      {   //qtss_printf("owner access to file =%s\n",pathBuff);
        result = true;
        break;
      } else {   //qtss_printf("no owner access to file =%s\n",pathBuff);
        result = false;
        break;
    ...
    if (statData.st_gid
        == (UInt16) passwdStructPtr->pw_gid)  // check if user's default group owns
    {
      if ((statData.st_mode & 0040) != 0) // has group read access
      {   //qtss_printf("user default group has access to file =%s\n",pathBuff);
        result = true;
        break;
      } else {   //qtss_printf("user default group has no access to file =%s\n",pathBuff);
        result = false;
        break;
      }
    ...
    if (IsUserMember(passwdStructPtr->pw_uid,
                     statData.st_gid)) // check if user in group
    {
      if ((statData.st_mode & 0040) != 0) // has group read access
      {   //qtss_printf("user member group has access to file =%s\n",pathBuff);
        result = true;
        break;
      } else {   //qtss_printf("user member group has no access to file =%s\n",pathBuff);
        result = false;
        break;
      }
...
Bool16 CheckDirAccess(struct passwd *passwdStructPtr,
                      StrPtrLen */*nameStrPtr*/,
                      char *pathBuff) {
  Bool16 result = true;
...
*/
  ...
  if (thePath.Len <= searchPath.Len) {
    ...
    while ((true == result) && (0 != GetPathParentDestructive(&searchPath,
                                                              &searchPath,
                                                              kMaxPathLen))) // loop until fail or have checked full directory path and file
    {
      result = false;
      ...
      do // once only check for the current entity
      {
        ...
        if (statData.st_uid
            == (UInt16) passwdStructPtr->pw_uid) // check if owner
        {
          if ((statData.st_mode & 0100) != 0) // has owner search access
          {   //qtss_printf("owner search access to directory =%s\n",pathBuff);
            result = true;
            break;
          } else {   //qtss_printf("no owner search access to directory =%s\n",pathBuff);
            result = false;
            break;
            ...
            == (UInt16) passwdStructPtr->pw_gid)  // check if user's default group owns
        {
          ...
          {   //qtss_printf("user default group has search access to directory =%s\n",pathBuff);
            result = true;
            break;
          } else {   //qtss_printf("user default group has no search access to directory =%s\n",pathBuff);
            result = false;
            break;
          }
                         ...
                         statData.st_gid)) // check if user in group
        {
          ...
          {   //qtss_printf("user member group has search access to directory =%s\n",pathBuff);
            result = true;
            break;
          } else {   //qtss_printf("user member group has no search access to directory =%s\n",pathBuff);
            result = false;
            break;
          }
...
Bool16 QTSSAuthorize(QTSS_StandardRTSP_Params *inParams, char *pathBuff) {
  ...
  do {
    ...
    if ((theErr != QTSS_NoErr) || (nameStr.Len == 0) || (nameStr.Ptr == NULL)
        || (*nameStr.Ptr == '\0')) {
    ...
    theErr = QTSS_GetValue(inParams->inRTSPRequest,
                           ...
                           &passwordStr.Len);
    //qtss_printf("GetValue qtssRTSPReqUserName err =%"_S32BITARG_" \n",theErr);
    if ((theErr != QTSS_NoErr) || (passwordStr.Len == 0)
        || (passwordStr.Ptr == NULL) || (*passwordStr.Ptr == '\0')) {
    ...
    if (0 == strcasecmp(nameStr.Ptr,
                        sRootUserPtr)) {   //qtss_printf("user is root no root access to file =%s\n",pathBuff); // must log
    ...
    char *theCryptedPassword =
        crypt(passwordStr.Ptr, passwdStructPtr->pw_passwd);
  ...
  if (!result) {   //qtss_printf("QTSSAuthorize: user %s is un authorized for %s\n",nameStr.Ptr,pathBuff);
  }

  return result;
}

Bool16 FileOrSDPFileExists(char *pathBuff,
                           UInt32 *pathLen,
                           const UInt32 maxLen,
                           QTSS_Error *theErrPtr) {
  Bool16 result = false;
  ...
  do {
    ...
    if ((*pathLen + sSDPSuffix.Len + 1) >= maxLen) {
      ...
      if (theErrPtr)
        *theErrPtr =
            ENAMETOOLONG; // don't allow request to succed we can't authorize
      break;
    }
...
*/
  ...
  do {
    ...
    Bool16
        fileOk = FileOrSDPFileExists(pathBuff, &pathLen, kMaxPathLen, &theErr);
    if (!fileOk) {
