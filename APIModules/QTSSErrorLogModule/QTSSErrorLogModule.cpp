...
static QTSS_Error QTSSErrorLogModuleDispatch(QTSS_Role inRole,
...
class ErrorLogCheckTask : public Thread::Task {
 public:
...
QTSS_Error Shutdown() {
  ...
  if (sErrorLogCheckTask != NULL) {
    ...
    sErrorLogCheckTask->Signal(Thread::Task::kKillEvent);
...
QTSS_Error LogError(QTSS_RoleParamPtr inParamBlock) {
  ...
  if (thePrefs->GetErrorLogVerbosity()
    ...
    if (isDuplicate) {   //yes?  increment count and bail if it's not the first time we've seen this message (otherwise fall thourhg and write it to the log)
