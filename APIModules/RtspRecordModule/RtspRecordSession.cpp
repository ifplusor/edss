...
void RTSPRecordSession::Initialize(QTSS_ModulePrefsObject inPrefs) {
  sHttpDir = QTSSModuleUtils::GetStringAttribute(inPrefs,
                                                 "HTTP_ROOT_DIR",
                                                 sDefaultHttpDir);
  sRecordDir = QTSSModuleUtils::GetStringAttribute(inPrefs,
                                                   "Record_DIR",
                                                   sDefaultRecordDir);
...
void RTSPRecordSession::init() {
  ...
  m_pFormatContext = avformat_alloc_context();//��ʼ��AVFormatContext
}
...
int RTSPRecordSession::rtspRead() {
  ...
  if (avformat_open_input(&m_pFormatContext,/*filepath*/
                          sUri.c_str(),
                          NULL,
                          &options) != 0) {//���ļ�
    printf("�޷����ļ�\n");
  ...
  if (avformat_find_stream_info(m_pFormatContext, nullptr) < 0)//��������Ϣ
  {
  ...
  m_videoIndex = -1;
  m_audioIndex = -1;
  for (int i = 0; i < m_pFormatContext->nb_streams; i++) //��ȡ��Ƶ��ID
  {
    if (m_pFormatContext->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
      m_videoIndex = i;
    } else if (m_pFormatContext->streams[i]->codec->codec_type
        == AVMEDIA_TYPE_AUDIO) {
      m_audioIndex = i;
    }
  ...
  AVCodec *pCodec = avcodec_find_decoder(m_pCodecCtx->codec_id);//���ҽ�����
  if (pCodec == NULL) {
    printf("Codec not found.\n");
    return -1;
  }
  if (avcodec_open2(m_pCodecCtx, pCodec, NULL) < 0)//�򿪽�����
  {
  ...
  m_packet = (AVPacket *) malloc(sizeof(AVPacket));//�洢����ǰ���ݰ�AVPacket
...
bool RTSPRecordSession::readFrame() {
  ...
  bool result = false;
  if (av_read_frame(m_pFormatContext, m_packet) < 0)//ѭ����ȡѹ�����ݰ�AVPacket
  {
    mutex->Unlock();
    return true;
  }
  ...
  if (m_packet->stream_index == m_videoIndex) {
    ...
    if (m_bRecording) {
      WriteMP4VideoFrame((unsigned char *) m_packet->data,
                         m_packet->size,
                         bKeyFrame,
                         clock(),
                         m_pCodecCtx->width,
  ...
  if (m_packet->stream_index == m_audioIndex) {
    if (m_bRecording) {
      WriteMP4AudioFrame((unsigned char *) m_packet->data,
                         m_packet->size,
                         clock());
    }
  }
...
int RTSPRecordSession::CreateNewMp4Writer(int sample_rate, int channels) {
  ...
  mkdir(fPlayName);
  qtss_sprintf(fPlayName,
               "%s/MP4/%s/%4d%02d%02d_%02d%02d%02d.mp4",
               movieFolder,
               subDir,
               sys.wYear,
               sys.wMonth,
...
//дMP4�ļ�(¼�����)
int RTSPRecordSession::CreateMP4Writer(char *sFileName,
                                       int nSampleRate,
                                       int nChannel,
                                       int nBitsPerSample,
                                       int nFlag) {
...

  {
    ...
    m_pMP4Writer->WriteAACInfo(ucDecInfoBuff,
                               unBuffSize,
                               nSampleRate,
                               nChannel,
                               nBitsPerSample);
...
int RTSPRecordSession::WriteMP4VideoFrame(unsigned char *pdata,
                                          int datasize,
                                          bool keyframe,
                                          long nTimestamp,
                                          int nWidth,
  ...
  {
    if (m_pMP4Writer) {
      m_pMP4Writer->WriteMp4File(pdata,
                                 datasize,
                                 keyframe,
                                 nTimestamp,
                                 nWidth,
...
int RTSPRecordSession::WriteMP4AudioFrame(unsigned char *pdata,
                                          int datasize,
                                          long timestamp) {
