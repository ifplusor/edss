...
unsigned char *EasyMP4Writer::FindNal(unsigned char *buff,
                                      int inlen,
                                      int &outlen,
                                      bool &end) {
 ...
 */
int EasyMP4Writer::find_nal_unit(unsigned char *buf,
                                 int size,
                                 int *nal_start,
                                 int *nal_end) {
  int i;
  ...
  *nal_end = 0;
  ...
  if (buf[i] != 0 || buf[i + 1] != 0
      || buf[i + 2] != 0x01) { /* error, should never happen */ return 0; }
  i += 3;
  *nal_start = i;
...
bool EasyMP4Writer::WriteH264SPSandPPS(unsigned char *sps,
                                       int spslen,
                                       unsigned char *pps,
                                       int ppslen,
                                       int width,
...
//д��AAC��Ϣ
bool EasyMP4Writer::WriteAACInfo(unsigned char *info,
                                 int len,
                                 int nSampleRate,
                                 int nChannel,
                                 int nBitsPerSample) {
  ...
  esd->decoderConfig->decoderSpecificInfo =
      (GF_DefaultDescriptor *) gf_odf_desc_new(GF_ODF_DSI_TAG);
  ...
  GF_Err gferr = gf_isom_new_mpeg4_description(p_file,
                                               m_audiotrackid,
                                               esd,
                                               NULL,
                                               NULL,
...
//		TRACE("mpeg4_description:%d\n",gferr);
  ...
  gferr = gf_isom_set_audio_info(p_file,
                                 m_audiotrackid,
                                 i_audiodescidx,
                                 nSampleRate,
                                 nChannel,
                                 nBitsPerSample);//44100 2 16
  if (gferr != 0) {
...
//д��һ֡��ǰ���ֽ�Ϊ��֡NAL����
bool EasyMP4Writer::WriteH264Frame(unsigned char *data,
                                   int len,
                                   bool keyframe,
                                   long timestamp) {
  ...
  if (m_videostartimestamp != -1) {
    ...
    GF_Err gferr = gf_isom_add_sample(p_file,
                                      m_videtrackid,
                                      i_videodescidx,
                                      p_videosample);
    if (gferr == -1) {
...
//д��aac���ݣ�ֻ��raw����
bool EasyMP4Writer::WriteAACFrame(unsigned char *data,
                                  int len,
                                  long timestamp) {
...
// 
// 	WriteH264Frame(pRealData, nRealDataSize+4, keyframe, nTimestamp);//����4��λ���������ݳ���ͷ��
// 
// 	if (pRealData)
// 	{	
// 		delete []pRealData;
...
int EasyMP4Writer::WriteMp4File(unsigned char *pdata,
                                int datasize,
                                bool keyframe,
                                long nTimestamp,
                                int nWidth,
