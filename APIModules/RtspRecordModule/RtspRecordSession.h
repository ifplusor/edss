...
class RTSPRecordSession {
 public:
  ...
  static char *getRecordRootPath();//����·��
  static char *getNetRecordRootPath();//����洢·��
 private:

  int CreateNewMp4Writer(int sample_rate, int channels);
  //дMP4�ļ�(¼�����)
  int CreateMP4Writer(char *sFileName,
                      int nSampleRate,
                      int nChannel,
                      int nBitsPerSample,
                      int nFlag = ZOUTFILE_FLAG_FULL);
  int WriteMP4VideoFrame(unsigned char *pdata,
                         int datasize,
                         bool keyframe,
                         long nTimestamp,
                         int nWidth,
