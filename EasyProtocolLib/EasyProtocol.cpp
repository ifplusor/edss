...
EasyDevice::EasyDevice(const string &channel, const string &name)
    : serial_(channel), name_(name) {

}

EasyDevice::EasyDevice(const string &channel,
                       const string &name,
                       const string &status)
    : serial_(channel), name_(name), status_(status) {
}

EasyNVR::EasyNVR() : object_(nullptr) {
}

EasyNVR::EasyNVR(const string &serial,
                 const string &name,
...
EasyMsgDSRegisterREQ::EasyMsgDSRegisterREQ(EasyDarwinTerminalType terminalType,
                                           EasyDarwinAppType appType,
                                           EasyNVR &nvr,
                                           size_t cseq)
    : EasyProtocol(MSG_DS_REGISTER_REQ), nvr_(nvr) {
  SetHeaderValue(EASY_TAG_CSEQ, cseq);
  SetHeaderValue(EASY_TAG_APP_TYPE, EasyProtocol::GetAppTypeString(appType));
  SetHeaderValue(EASY_TAG_TERMINAL_TYPE,
                 EasyProtocol::GetTerminalTypeString(terminalType));
...
EasyMsgDSRegisterREQ::EasyMsgDSRegisterREQ(const string &msg)
  ...
  for (int i = 0; i < size; i++) {
    Json::Value
        &json_camera = root[EASY_TAG_ROOT][EASY_TAG_BODY][EASY_TAG_CHANNELS][i];
    EasyDevice channel;
...
EasyMsgSDRegisterACK::EasyMsgSDRegisterACK(EasyJsonValue &body,
                                           size_t cseq,
                                           size_t error)
  ...
  for (auto it = body.begin(); it != body.end(); ++it) {
    SetBodyValue(it->first.c_str(),
                 boost::apply_visitor(EasyJsonValueVisitor(), it->second));
  }
}
...
EasyMsgDSPushSteamACK::EasyMsgDSPushSteamACK(EasyJsonValue &body,
                                             size_t cseq,
                                             size_t error)
...
// MSG_SD_STREAM_STOP_REQ��Ϣ����
EasyMsgDSStopStreamACK::EasyMsgDSStopStreamACK(EasyJsonValue &body,
                                               size_t cseq,
                                               size_t error)
...
EasyMsgSCDeviceListACK::EasyMsgSCDeviceListACK(EasyDevices &devices,
                                               size_t cseq,
                                               size_t error)
    : EasyProtocol(MSG_SC_DEVICE_LIST_ACK), devices_(devices) {
  SetHeaderValue(EASY_TAG_CSEQ, cseq);
  SetHeaderValue(EASY_TAG_ERROR_NUM, error);
  SetHeaderValue(EASY_TAG_ERROR_STRING, GetErrorString(error));
...
EasyMsgSCDeviceListACK::EasyMsgSCDeviceListACK(const string &msg)
  ...
  for (int i = 0; i < size; i++) {
    Json::Value
        &json_ = root[EASY_TAG_ROOT][EASY_TAG_BODY][EASY_TAG_DEVICES][i];
    EasyDevice device;
...
EasyMsgSCDeviceInfoACK::EasyMsgSCDeviceInfoACK(EasyDevices &cameras,
...
EasyMsgSCDeviceInfoACK::EasyMsgSCDeviceInfoACK(const string &msg)
  ...
  for (int i = 0; i < size; i++) {
    Json::Value
        &json_ = root[EASY_TAG_ROOT][EASY_TAG_BODY][EASY_TAG_CHANNELS][i];
    EasyDevice channel;
...
EasyMsgSCGetStreamACK::EasyMsgSCGetStreamACK(EasyJsonValue &body,
                                             size_t cseq,
                                             size_t error)
...
EasyMsgSCFreeStreamACK::EasyMsgSCFreeStreamACK(EasyJsonValue &body,
                                               size_t cseq,
                                               size_t error)
...
EasyMsgSDPostSnapACK::EasyMsgSDPostSnapACK(EasyJsonValue &body,
                                           size_t cseq,
                                           size_t error)
...
EasyMsgSCPTZControlACK::EasyMsgSCPTZControlACK(EasyJsonValue &body,
                                               size_t cseq,
                                               size_t error)
...
EasyMsgDSControlPTZACK::EasyMsgDSControlPTZACK(EasyJsonValue &body,
                                               size_t cseq,
                                               size_t error)
...
EasyMsgCSPresetControlREQ::EasyMsgCSPresetControlREQ(EasyJsonValue &body,
                                                     size_t cseq)
...
EasyMsgSCPresetControlACK::EasyMsgSCPresetControlACK(EasyJsonValue &body,
                                                     size_t cseq,
                                                     size_t error)
...
EasyMsgSDControlPresetREQ::EasyMsgSDControlPresetREQ(EasyJsonValue &body,
                                                     size_t cseq)
...
EasyMsgDSControlPresetACK::EasyMsgDSControlPresetACK(EasyJsonValue &body,
                                                     size_t cseq,
                                                     size_t error)
...
EasyMsgCSTalkbackControlREQ::EasyMsgCSTalkbackControlREQ(EasyJsonValue &body,
                                                         size_t cseq)
...
EasyMsgSCTalkbackControlACK::EasyMsgSCTalkbackControlACK(EasyJsonValue &body,
                                                         size_t cseq,
                                                         size_t error)
...
EasyMsgSDControlTalkbackREQ::EasyMsgSDControlTalkbackREQ(EasyJsonValue &body,
                                                         size_t cseq)
...
EasyMsgDSControlTalkbackACK::EasyMsgDSControlTalkbackACK(EasyJsonValue &body,
                                                         size_t cseq,
                                                         size_t error)
...
bool EasyMsgSCListRecordACK::AddRecord(const string &record) {
  ...
  value[EASY_TAG_L_TIME] =
      record.substr(pos - 14, 14); // /20151123114500/*.m3u8
...
void EasyMsgSCRecordListACK::AddRecord(const string &starttime,
...
bool strDevice::GetDevInfo(const string &json)//��JSON�ı��õ��豸��Ϣ
{
  ...
  do {
    string strTerminalType =
        proTemp.GetHeaderValue(EASY_TAG_TERMINAL_TYPE);//��ȡ�豸����
    ...
    eDeviceType =
        static_cast<EasyDarwinTerminalType>(EasyProtocol::GetTerminalType(
            strTerminalType));
    if (eDeviceType == -1)
      break;
    eAppType =
        static_cast<EasyDarwinAppType>(EasyProtocol::GetAppType(strAppType));
    if (eAppType == -1)
      break;
    ...
    channelCount_ =
        proTemp.GetBodyValue(EASY_TAG_CHANNEL_COUNT);//��ǰ�豸����������ͷ����
    ...
    if (eAppType == EASY_APP_TYPE_NVR)//����豸������NVR������Ҫ��ȡ����ͷ��Ϣ
    {
      ...
      int size =
          (*proot)[EASY_TAG_ROOT][EASY_TAG_BODY][EASY_TAG_CHANNELS].size(); //�����С

      set<string> channelSetTemp;
      for (auto i = 0; i < size; ++i) {
        auto &json_camera =
            (*proot)[EASY_TAG_ROOT][EASY_TAG_BODY][EASY_TAG_CHANNELS][i];
...
void strDevice::HoldSnapPath(const string &strJpgPath,
                             const string &strChannel)//�������յ�ʱ������
{
