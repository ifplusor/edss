...
std::string EasyUtil::TimeT2String(EasyDarwinTimeFormat whatFormat,
                                   unsigned long time) {
  struct tm local;
  time_t t = time;
...
unsigned long EasyUtil::String2TimeT(EasyDarwinTimeFormat whatFormat,
...
unsigned long
EasyUtil::String2TimeT(const std::string &dateYYMMDD/*2014-11-23*/,
                       const std::string &timeHHMMSS/*08:30:00*/) {
...
bool EasyUtil::Base64Decode(const std::string &sInput, string &sOutput) {
  typedef boost::archive::iterators::transform_width<boost::archive::iterators::binary_from_base64<
      string::const_iterator>, 8, 6> Base64DecodeIterator;
  stringstream result;
  try {
    copy(Base64DecodeIterator(sInput.begin()),
         Base64DecodeIterator(sInput.end()),
...
bool EasyUtil::Base64Encode(const std::string &sInput, string &sOutput) {
  typedef boost::archive::iterators::base64_from_binary<boost::archive::iterators::transform_width<
      string::const_iterator,
      6,
      8> > Base64EncodeIterator;
  stringstream result;
  copy(Base64EncodeIterator(sInput.begin()),
       Base64EncodeIterator(sInput.end()),
       ostream_iterator<char>(result));
...
string EasyUtil::Base64Decode(const string &sInput) {
  ...
  try {
    ...
    copy(Base64DecodeIterator(temp.begin()),
         Base64DecodeIterator(temp.end()),
         ostream_iterator<char>(result));
  }
  catch (...) {
    return string();
...
inline char toHex(const char &x) {
  return (char) (x > 9 ? x - 10 + 'A' : x + '0');
}

inline char fromHex(const char &x) {
  return (char) (isdigit(x) ? x - '0' : x - 'A' + 10);
}
