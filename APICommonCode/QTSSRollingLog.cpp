...
char *QTSSRollingLog::GetLogPath(char *extension) {
  ...
  formatPath.Set(nullptr,
                 0); //don't delete buffer we are returning the path as  a result
...
time_t QTSSRollingLog::ReadLogHeader(FILE *inFile) {
  ...
  if (EOF == ::sscanf(theFirstLine,
                      "#Log File Created On: %d/%d/%d %d:%d:%d\n",
                      &theFileCreateTime.tm_mon,
                      &theFileCreateTime.tm_mday,
                      &theFileCreateTime.tm_year,
                      &theFileCreateTime.tm_hour,
                      &theFileCreateTime.tm_min,
                      &theFileCreateTime.tm_sec))
    return -1;
