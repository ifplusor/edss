...
bool QTAccessFile::HaveGroups(char **groupArray,
                              UInt32 numGroups,
                              void *extraDataPtr) {
  bool result = false;
...
bool QTAccessFile::HaveRealm(char *userName,
                             StrPtrLen *ioRealmNameStr,
                             void *extraData) {
  bool result = false;

  if (ioRealmNameStr != NULL && ioRealmNameStr->Ptr != NULL
      && ioRealmNameStr->Len > 0)
    result = true;

  return result;
}

void QTAccessFile::GetRealm(StrPtrLen *accessRealm,
                            StrPtrLen *ioRealmNameStr,
                            char *userName,
                            void *extraDataPtr) {
...
bool QTAccessFile::TestUser(StrPtrLen *accessUser,
                            char *userName,
                            void *extraDataPtr) {
  bool result = false;
...
bool QTAccessFile::TestGroup(StrPtrLen *accessGroup,
                             char *userName,
                             char **groupArray,
                             UInt32 numGroups,
                             void *extraDataPtr) {
...
bool QTAccessFile::TestExtraData(StrPtrLen *wordPtr,
                                 StringParser *lineParserPtr,
                                 void *extraDataPtr) {
  return false;
}

bool QTAccessFile::AccessAllowed(char *userName,
                                 char **groupArray,
                                 UInt32 numGroups,
                                 StrPtrLen *accessFileBufPtr,
                                 QTSS_ActionFlags inFlags,
                                 StrPtrLen *ioRealmNameStr,
                                 bool *outAllowAnyUserPtr,
                                 void *extraDataPtr) {
  if (NULL == accessFileBufPtr || NULL == accessFileBufPtr->Ptr
      || 0 == accessFileBufPtr->Len)
  ...
  *outAllowAnyUserPtr = false;
  ...
  while (accessFileParser.GetDataRemaining() != 0) {
    ...
    if (word.Equal("<Limit")) // a limit line
    {
      ...
      lineParser.ConsumeUntil(&word,
                              sWhitespaceAndGreaterThanMask); // the flag <limit Read> or <limit Read >
      while (word.Len != 0) // compare each word in the line
      {
        if (word.Equal("WRITE")) {
          currentFlags |= inFlags
              & qtssActionFlagsWrite; // accept following lines if inFlags has write
        }

        if (word.Equal("READ")) {
          currentFlags |= inFlags
              & qtssActionFlagsRead; // accept following lines if inFlags has read
        }
        lineParser.ConsumeWhitespace();
    ...
    if (word.Equal("</Limit>"))
      currentFlags =
          qtssActionFlagsRead; // set the current access state to the default of read access
    ...
    if (word.Equal("require")) {
      ...
      if (haveGroups
          && word.Equal("group")) // check if we have groups for the user
      {
        lineParser.ConsumeWhitespace();
        lineParser.ConsumeUntilWhitespace(&word);
...
char *QTAccessFile::GetAccessFile_Copy(char const *movieRootDir,
  ...
  int maxLen = strlen(dirPath) + strlen(sQTAccessFileName)
      + strlen(kPathDelimiterString) + 1;
  ...
  while (true) { //walk backward up the dir tree.
    ...
    int curLen = strlen(currentDir) + strlen(sQTAccessFileName)
        + strlen(kPathDelimiterString);
    ...
    if (QTSS_OpenFileObject(currentDir, qtssOpenFileNoFlags, &fileObject)
        == QTSS_NoErr) {
    ...
    if ((lastSlash - currentDir)
        < movieRootDirLen) //bail if we start eating our way out of fMovieRootDir
      break;
  }
...
QTSS_AuthScheme QTAccessFile::FindUsersAndGroupsFilesAndAuthScheme(char *inAccessFilePath,
                                                                   QTSS_ActionFlags inAction,
                                                                   char **outUsersFilePath,
                                                                   char **outGroupsFilePath) {
  ...
  *outGroupsFilePath = NULL;
  ...
  while (accessFileParser.GetDataRemaining() != 0) {
    ...
    if (word.Equal("AuthUserFile")) {
      ...
      if (*outUsersFilePath
          != NULL)       // we are encountering the AuthUserFile keyword twice!
    ...
    if (word.Equal("AuthGroupFile")) {
      ...
      if (*outGroupsFilePath
          != NULL)      // we are encountering the AuthGroupFile keyword twice!
...
QTSS_Error QTAccessFile::AuthorizeRequest(QTSS_StandardRTSP_Params *inParams,
                                          bool allowNoAccessFiles,
                                          QTSS_ActionFlags noAction,
                                          QTSS_ActionFlags authorizeAction,
                                          bool *outAuthorizedPtr,
                                          bool *outAllowAnyUserPtr) {
  if ((NULL == inParams) || (NULL == inParams->inRTSPRequest)
      || (NULL == outAllowAnyUserPtr) || (NULL == outAuthorizedPtr))
  ...
  *outAuthorizedPtr = false;
  ...
  bool allowRequest = this->AccessAllowed(username,
                                          groupCharPtrArray,
                                          numGroups,
                                          &accessFileBuf,
                                          authorizeAction,
  ...
  if ((theAuthScheme == qtssAuthBasic)
      && (realmNameStr.Ptr[0] != '\0'))   //set the realm if we have one
