...
void EventContext::Cleanup() {
  ...
  AssertV(err == 0,
          OSThread::GetErrno());//we don't really care if there was an error, but it's nice to know
}
...
void EventContext::RequestEvent(int theMask) {
  ...
  if (fWatchEventCalled) {
    ...
    do {
      ...
      if (!sUniqueID.compare_exchange_weak(topVal,
                                           1))  // Fix 2466667: message IDs above a
        fUniqueID = ++sUniqueID;         // level are ignored, so wrap at 8192
      else
        fUniqueID = 1;
...
void EventThread::Entry() {
  ...
  while (true) {
    ...
    if (theCurrentEvent.er_data != NULL) {
      ...
      StrPtrLen
          idStr((char *) &theCurrentEvent.er_data, sizeof(PointerSizedInt));
