...
class SVector {
 public:
  ...
  UInt32 find(UInt32 searchStart, UInt32 searchEnd, const T &target) {
    return find < EqualOp > (searchStart, searchEnd, target);
  }

  //Allows you to specify an equality comparison functor
  template<class Eq>
  UInt32 find(UInt32 searchStart,
              UInt32 searchEnd,
              const T &target,
              Eq eq = Eq()) {
  ...
  template<class Eq>
  UInt32 find(const T &target, Eq eq = Eq()) {
    return find(0,
                size(),
                target,
                eq);
  ...
  void insert(UInt32 position, const T &newItem) {
    insert(position,
           1,
           newItem);
  }
