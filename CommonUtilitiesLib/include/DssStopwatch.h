...
class DssEggtimer {
 public:
  ...
  DssEggtimer(SInt64 inMilliseconds) {
...
class DssMillisecondStopwatch {
 ...
 public:
  DssMillisecondStopwatch() :
      fIsStarted(false), fTimerDuration(-1) {};
  void Start() {
    fStartedAt = OS::Milliseconds();
    fIsStarted = true;
  }
...
class DssDurationTimer {
 ...
 public:
  ...
  void ResetToDuration(SInt64 inDurationInMsec) {
    fStartedAtMsec = OS::Milliseconds() - inDurationInMsec;
  }
  SInt64 DurationInMilliseconds() {
    return OS::Milliseconds() - fStartedAtMsec;
  }
  SInt64 DurationInSeconds() {
    return (OS::Milliseconds() - fStartedAtMsec) / (SInt64) 1000;
  }
