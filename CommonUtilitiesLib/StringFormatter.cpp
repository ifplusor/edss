...
void StringFormatter::Put(char *buffer, UInt32 bufferSize) {
  ...
  while ((spaceInBuffer < bufferSize)
      || (spaceLeft == 0)) // too big for destination
  {
    if (spaceLeft > 0) {
...
bool StringFormatter::PutFmtStr(const char *fmt, ...) {
  ...
  for (;;) {
    ...
    if (length < 0)
      return false;
    if (static_cast<UInt32>(length)
        >= this->GetSpaceLeft()) //was not able to write all the output
    {
      if (this->BufferIsFull(fStartPut, this->GetCurrentOffset()))
        continue;
      //can only output a portion of the string
      UInt32 bytesWritten = fEndPut - fCurrentPut
          - 1; //We don't want to include the NUL terminator
