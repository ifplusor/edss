...
TimeoutTask::TimeoutTask(Task *inTask, SInt64 inTimeoutInMilSecs)
  ...
  if (nullptr == inTask)
    fTask = (Task *) this;
  Assert(sThread
             != nullptr); // this can happen if RunServer intializes tasks in the wrong order
...
SInt64 TimeoutTaskThread::Run() {
  ...
  SInt64 intervalMilli = kIntervalSeconds
      * 1000;//always default to 60 seconds but adjust to smallest interval > 0
  ...
  for (OSQueueIter iter(&fQueue); !iter.IsDone(); iter.Next()) {
    ...
    if ((theTimeoutTask->fTimeoutAtThisTime > 0)
      ...
      if ((taskInterval > 0) && (theTimeoutTask->fTimeoutInMilSecs > 0)
        ...
        intervalMilli = taskInterval
            + 1000; // set timeout to 1 second past this task's timeout
#if TIMEOUT_DEBUGGING
  ...
  // 注意:在TaskThread::Entry将TimeoutTaskThread项从线程的fTaskQueue里取出处理后,根据Run返回
  // 值,决定是否插入线程的fHeap,而不会再次插入到fTaskQueue里。如果插入fHeap,在
  // TaskThread::WaitForTask里会被得到处理。
