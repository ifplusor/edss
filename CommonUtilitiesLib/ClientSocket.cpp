...
OS_Error ClientSocket::SendSendBuffer(TCPSocket *inSocket) {
  ...
  do {
    ...
    theErr = inSocket->Send(fSendBuffer.Ptr + fSentLength,
                            fSendBuffer.Len - fSentLength,
                            &theLengthSent);
...
void TCPClientSocket::SetOptions(int sndBufSize,
  ...
  err = ::setsockopt(fSocket.GetSocketFD(),
                     SOL_SOCKET,
                     SO_RCVBUF,
                     (char *) &rcvBufSize,
                     sizeof(int));
...
OS_Error TCPClientSocket::SendV(iovec *inVec, UInt32 inNumVecs) {
  if (fSendBuffer.Len == 0) {
    for (UInt32 count = 0; count < inNumVecs; count++) {
      ::memcpy(fSendBuffer.Ptr + fSendBuffer.Len,
               inVec[count].iov_base,
               inVec[count].iov_len);
...
OS_Error TCPClientSocket::Read(void *inBuffer,
...
OS_Error HTTPClientSocket::Read(void *inBuffer,
                                const UInt32 inLength,
                                UInt32 *outRcvLen) {
  //
  ...
  if (fSentLength > 0) {
    *outRcvLen = 0;
    do {
      ...
      theErr = fGetSocket.Read(&fSendBuffer.Ptr[fGetReceived],
                               kSendBufferLen - fGetReceived,
                               outRcvLen);
  ...
  theErr = fGetSocket.Read(&((char *) inBuffer)[fGetReceived],
                           inLength - fGetReceived,
                           outRcvLen);
  if (theErr != OS_NoErr) {
#if CLIENT_SOCKET_DEBUG
...
void HTTPClientSocket::encodeVec(iovec *inVec, UInt32 inNumVecs) {
  for (UInt32 count = 0; count < inNumVecs; count++) {
    fSendBuffer.Len += ::Base64encode(fSendBuffer.Ptr + fSendBuffer.Len,
                                      (char *) inVec[count].iov_base,
                                      inVec[count].iov_len);
    Assert(fSendBuffer.Len < ClientSocket::kSendBufferLen);
    fSendBuffer.Len =
        ::strlen(fSendBuffer.Ptr); //Don't trust what the above function returns for a length
  }
}
