...
std::size_t parseIndex(std::string::const_iterator &itFmt,
...
void parseFlags(std::ostream &str,
                std::string::const_iterator &itFmt,
                const std::string::const_iterator &endFmt) {
  bool isFlag = true;
  while (isFlag && itFmt != endFmt) {
    switch (*itFmt) {
      case '-': str.setf(std::ios::left);
        ++itFmt;
        break;
      case '+': str.setf(std::ios::showpos);
        ++itFmt;
        break;
      case '0': str.fill('0');
        ...
        ++itFmt;
        break;
      case '#': str.setf(std::ios::showpoint | std::ios::showbase);
        ++itFmt;
        break;
      default: isFlag = false;
        break;
    }
  }
}

void parseWidth(std::ostream &str,
                std::string::const_iterator &itFmt,
                const std::string::const_iterator &endFmt) {
  int width = 0;
  while (itFmt != endFmt && isdigit(*itFmt)) {
...
void parsePrec(std::ostream &str,
               std::string::const_iterator &itFmt,
               const std::string::const_iterator &endFmt) {
...
char parseMod(std::string::const_iterator &itFmt,
              const std::string::const_iterator &endFmt) {
  char mod = 0;
  if (itFmt != endFmt) {
    switch (*itFmt) {
...
void prepareFormat(std::ostream &str, char type) {
  switch (type) {
    ...
    case 'x': str << std::hex;
      break;
    case 'X': str << std::hex << std::uppercase;
      break;
    case 'e': str << std::scientific;
      break;
    case 'E': str << std::scientific << std::uppercase;
      break;
    case 'f': str << std::fixed;
      break;
  }
}

void FormatOne(std::string &result,
               std::string::const_iterator &itFmt,
               const std::string::const_iterator &endFmt,
  ...
  try {
    ...
    if (itFmt != endFmt) {
      ...
      switch (type) {
        case 'b': str << boost::any_cast<bool>(*itVal++);
          break;
        case 'c': str << boost::any_cast<char>(*itVal++);
          break;
        case 'd':
        case 'i':
          switch (mod) {
            case 'l': str << boost::any_cast<long>(*itVal++);
              break;
            case 'L': str << boost::any_cast<long long>(*itVal++);
              break;
            case 'h': str << boost::any_cast<short>(*itVal++);
              break;
            case '?': writeAnyInt(str, *itVal++);
              break;
            default: str << boost::any_cast<int>(*itVal++);
              break;
          }
          break;
        case 'o':
        ...
        case 'X':
          switch (mod) {
            case 'l': str << boost::any_cast<unsigned long>(*itVal++);
              break;
            case 'L': str << boost::any_cast<unsigned long long>(*itVal++);
              break;
            case 'h': str << boost::any_cast<unsigned short>(*itVal++);
              break;
            case '?': writeAnyInt(str, *itVal++);
              break;
            default: str << boost::any_cast<unsigned>(*itVal++);
              break;
          }
          break;
        case 'e':
        ...
        case 'f':
          switch (mod) {
            case 'l': str << boost::any_cast<long double>(*itVal++);
              break;
            case 'L': str << boost::any_cast<long double>(*itVal++);
              break;
            case 'h': str << boost::any_cast<float>(*itVal++);
              break;
            default: str << boost::any_cast<double>(*itVal++);
              break;
          }
          break;
        case 's': str << boost::any_cast<std::string>(*itVal++);
          break;
        case 'z': str << boost::any_cast<std::size_t>(*itVal++);
          break;
        case 'I':
        case 'D':
        default: str << type;
        ...
        if (itFmt != endFmt && (itVal != endVal || *itFmt == '[')) {
          if (*itFmt == '[') {
            ...
            if (index < values.size()) {
              std::vector<boost::any>::const_iterator
                  it = values.begin() + index;
              FormatOne(result, itFmt, endFmt, it);
            } else
              throw std::invalid_argument("Format argument index out of range");
          } else {
