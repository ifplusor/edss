...
void FileBlockPool::MarkUsed(FileBlockBuffer *inBuffPtr) {
  ...
  if (fQueue.GetTail()
      != inBuffPtr->GetQElem()) // Least Recently Used tail is last accessed
  {
...
void FileMap::AllocateBufferMap(UInt32 inUnitSizeInK,
  ...
  if (fFileMapArray != nullptr && fNumBuffSizeUnits == inNumBuffSizeUnits
      && inBufferIncCount == fBlockPool.GetMaxBuffers())
    return;
  ...
  if (inBitRate == 0) // just use the maximum possible size
    inBitRate =
        inMaxBitRateBuffSizeInBlocks * fBlockPool.GetBufferUnitSizeBytes();
  ...
  if (inNumBuffSizeUnits == 0) // calculate the buffer size ourselves
  {
    ...
    if (inNumBuffSizeUnits
        > inMaxBitRateBuffSizeInBlocks) // max is 8 * buffUnit Size (32k) = 256K
    {
...
void FileMap::DeleteOldBuffs() {
  while (fBlockPool.GetNumCurrentBuffers()
      > fBlockPool.GetMaxBuffers()) // delete any old buffers
  {
...
void FileMap::Clean() {
  if (fFileMapArray != nullptr)
    ::memset((char *) fFileMapArray,
             0,
             (SInt32) (sizeof(FileBlockBuffer *) * fMapArraySize));
}
...
OS_Error OSFileSource::ReadFromCache(UInt64 inPosition,
  ...
  *outRcvLen = 0;
  ...
  while (buffIndex <= endIndex && buffIndex <= maxIndex) {
    ...
    if (fillBuff) {
      OS_Error theErr = this->FillBuffer((char *) inBuffer,
                                         (char *) buffStart,
                                         (SInt32) buffIndex);
      if (theErr != OS_NoErr)
        return theErr;
