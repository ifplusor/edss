...
bool RTCPPacket::ParsePacket(UInt8 *inPacketBuffer, UInt32 inPacketLen) {
  ...
  if (RTCP_PACKET_DEBUG)
    qtss_printf("RTCPPacket::ParsePacket first 4 bytes of packet=%x \n",
  ...
  if (RTCP_PACKET_DEBUG)
    qtss_printf("RTCPPacket::ParsePacket len=%"   _U32BITARG_   " min allowed=%"   _U32BITARG_   "\n",
                inPacketLen,
  ...
  if (inPacketLen
    ...
    if (RTCP_PACKET_DEBUG)
      qtss_printf("RTCPPacket::ParsePacket invalid len=%"   _U32BITARG_   "\n",
                  inPacketLen);
    return false;
  }
  ...
  if (this->GetVersion() != kSupportedRTCPVersion) {
    if (RTCP_PACKET_DEBUG)
      qtss_printf("RTCPPacket::ParsePacket unsupported version\n");
    return false;
  }
...
bool RTCPReceiverPacket::ParseReport(UInt8 *inPacketBuffer,
  ...
  int theMaxReports =
      (inPacketLength - kRTCPPacketSizeInBytes) / kReportBlockOffsetSizeInBytes;
  ...
  if (this->GetReportCount() > theMaxReports) {
    if (RTCP_PACKET_DEBUG)
      printf(
          "RTCPReceiverPacket::ParseReport this rtcp report count=%d > max reports=%d\n",
          this->GetReportCount(),
          theMaxReports);
    return false;
  }
...
bool RTCPSenderReportPacket::ParseReport(UInt8 *inPacketBuffer,
                                         UInt32 inPacketLength) {
  bool ok = this->ParsePacket(inPacketBuffer, inPacketLength);
  if (!ok)
    return false;
  ...
  fRTCPReceiverReportArray =
      inPacketBuffer + kRTCPPacketSizeInBytes + kRTCPSRPacketSenderInfoInBytes;

  //this is the maximum number of reports there could possibly be
  int theNumReports =
      (inPacketLength - kRTCPPacketSizeInBytes - kRTCPSRPacketSenderInfoInBytes)
          / kReportBlockOffsetSizeInBytes;
...
void RTCPPacket::Dump() {
  qtss_printf(
      "H_vers=%d, H_pad=%d, H_rprt_count=%d, H_type=%d, H_length=%d, H_ssrc=%" _S32BITARG_ "",
      this->GetVersion(),
