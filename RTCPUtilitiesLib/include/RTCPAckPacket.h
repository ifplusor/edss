...
class RTCPAckPacket : public RTCPAPPPacket {
 ...
 private:
  ...
  inline bool IsAckType(UInt32 theAppType) {
    return ((theAppType == kAckPacketAlternateName)
        || (theAppType == kAckPacketName));
  }
};
...
bool RTCPAckPacket::IsNthBitEnabled(UInt32 inBitNumber) {
  ...
  UInt8 bitMask = 128;
  return *(fRTCPAckBuffer + kAckMaskOffset + (inBitNumber >> 3))
      & (bitMask >>= inBitNumber & 7);
}
