...
class RTCPAckPacketFmt {
 ...
 public:
  ...
  RTCPAckPacketFmt(StrPtrLen &newBuffer)
      : fBitMaskSize(0) { SetBuffer(newBuffer); }
  RTCPAckPacketFmt(char *newBuffer, UInt32 bufLen)
      : fBitMaskSize(0) { SetBuffer(newBuffer, bufLen); }
  ...
  void SetAcks(SVector <UInt32> AckList, UInt32 serverSSRC) {
    ...
    fBitMaskSize = slotsInMaskNeeded % 32 == 0 ? (slotsInMaskNeeded / 32) * 4 :
                   (slotsInMaskNeeded / 32 + 1) * 4;
