...
RTPStream *RTPSession::FindRTPStreamForChannelNum(UInt8 inChannelNum) {
  ...
  for (int x = 0; this->GetValuePtr(qtssCliSesStreamObjects,
    ...
    if (*theStream != NULL)
      if (((*theStream)->GetRTPChannelNum() == inChannelNum) ||
          ((*theStream)->GetRTCPChannelNum() == inChannelNum))
        return *theStream;
  }
...
QTSS_Error RTPSession::Play(RTSPRequestInterface *request,
  ...
  if ((bitRate == 0) || (bitRate
      > QTSServerInterface::GetServer()->GetPrefs()->GetWindowSizeMaxThreshold()
          * 1024))
    theWindowSize = 1024
        * QTSServerInterface::GetServer()->GetPrefs()->GetLargeWindowSizeInK();
  else if (bitRate
      > QTSServerInterface::GetServer()->GetPrefs()->GetWindowSizeThreshold()
          * 1024)
    theWindowSize = 1024
        * QTSServerInterface::GetServer()->GetPrefs()->GetMediumWindowSizeInK();
  else
    theWindowSize = 1024
        * QTSServerInterface::GetServer()->GetPrefs()->GetSmallWindowSizeInK();
...
#endif
  this->Signal(Task::kStartEvent);  // 发送信号,RTPSession::Run 将会被运行。
...
void RTPSession::SendDescribeResponse(RTSPRequestInterface *inRequest) {
  ...
  inRequest->AppendHeader(qtssXAcceptRetransmitHeader,
                          &sRetransmitProtocolName);
...
SInt64 RTPSession::Run() {
  ...
  if ((events & Task::kKillEvent) || (events & Task::kTimeoutEvent)
    ...
    if (!fModuleDoingAsyncStuff) {
      ...
      OSRefTable
          *sessionTable = QTSServerInterface::GetServer()->GetRTPSessionMap();
