...
class RTPBandwidthTracker {
 public:
  ...
  void FillWindow(UInt32 inNumBytes) {
    fBytesInList += inNumBytes;
    fIsRetransmitting = false;
  }
  ...
  //
  // ACCESSORS
  const bool ReadyForAckProcessing() {
    return (fClientWindow > 0 && fCongestionWindow > 0);
  } // see RTPBandwidthTracker::EmptyWindow for requirements
  const bool IsFlowControlled() {
    return ((SInt32) fBytesInList >= fCongestionWindow);
  }
  ...
  const SInt32 RunningAverageMSecs() {
    return fRunningAverageMSecs / 8;
  }  // fRunningAverageMSecs is stored scaled up 8x
  const SInt32 RunningMeanDevationMSecs() {
    return fRunningMeanDevationMSecs / 4;
  } // fRunningMeanDevationMSecs is stored scaled up 4x
  const SInt32 CurRetransmitTimeout() { return fCurRetransmitTimeout; }
  const SInt32 GetCurrentBandwidthInBps() {
    return (fUnadjustedRTO > 0) ? (fCongestionWindow * 1000) / fUnadjustedRTO
                                : 0;
  }
  ...
  SInt32 GetAvgCongestionWindowSize() {
    return (SInt32) (fTotalCongestionWindowSize / (SInt64) fNumStatsSamples);
  }
  SInt32 GetMaxRTO() { return fMaxRTO; }
  SInt32 GetMinRTO() { return fMinRTO; }
  SInt32 GetAvgRTO() {
    return (SInt32) (fTotalRTO / (SInt64) fNumStatsSamples);
  }
 ...
 private:
  ...
  SInt32
      fSlowStartThreshold;    // point at which we stop adding to the window for each ack, and add to the window for each window full of acks
  SInt32
      fSlowStartByteCount;            // counts window a full of acks when past ss thresh
  ...
  bool fUseSlowStart;
  bool
      fIsRetransmitting;      // are we in the re-transmit 'state' ( started resending, but have yet to send 'new' data
