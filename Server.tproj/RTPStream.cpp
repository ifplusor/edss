...
QTSS_Error RTPStream::Setup(RTSPRequestInterface *request,
  ...
  if (fTransportType == qtssRTPTransportTypeTCP) {
    ...
    fRTPChannel =
        request->GetSession()->GetTwoChannelNumbers(fSession->GetValue(
  ...
  if ((request->GetSourceAddr() != INADDR_ANY)
      && (SocketUtils::IsLocalIPAddr(request->GetSourceAddr())))
  ...
  if (SocketUtils::IsMulticastIPAddr(fRemoteAddr)) {
    ...
    if (fSockets != NULL) {
      ...
      if (err == QTSS_NoErr)
        err =
            fSockets->GetSocketA()->SetMulticastInterface(fSockets->GetSocketA()->GetLocalAddr());
      if (err == QTSS_NoErr)
        err =
            fSockets->GetSocketB()->SetMulticastInterface(fSockets->GetSocketB()->GetLocalAddr());
      if (err != QTSS_NoErr)
        return QTSSModuleUtils::SendErrorResponse(request,
                                                  qtssServerInternal,
...
QTSS_Error RTPStream::InterleavedWrite(void *inBuffer,
  ...
  if (fSession->GetRTSPSession()
      == NULL) // RTSPSession required for interleaved write
  {
    return EAGAIN;
  }
...
void RTPStream::SendRTCPSR(const SInt64 &inTime, bool inAppendBye) {
  ...
  fLastNTPTimeStamp = OS::TimeMilli_To_1900Fixed64Secs(
      OS::Milliseconds()); //The time value should be filled in as late as possible.
    ...
    switch (rtcpPacket.GetPacketType()) {
      case RTCPPacket::kReceiverPacketType: {
        ...
        if (receiverPacket.GetReportCount() > 0) {
          ...
          if (lsr != 0) {
            UInt32 diff =
                static_cast<UInt32>(OS::TimeMilli_To_1900Fixed64Secs(curTime)
                    >> 16) - lsr - dlsr;
            UInt32
                measuredRTT = static_cast<UInt32>(OS::Fixed64Secs_To_TimeMilli(
                static_cast<SInt64>(diff) << 16));
...
void RTPStream::PrintRTCPSenderReport(char *packetBuff, UInt32 inLen) {
  ...
  qtss_printf(
      ...
      ::qtss_ctime(&theTime, timebuffer, sizeof(timebuffer)));
}
  ...
  switch (inType) {
    case RTPStream::rtp:
      if (QTSServerInterface::GetServer()->GetPrefs()->PrintRTPHeaders()) {
        qtss_printf("\n");
        qtss_printf("<send sess=%"   _U32BITARG_   ": RTP %s xmit_sec=%.3f %s size=%"   _U32BITARG_   " ",
    ...
    case RTPStream::rtcpSR:
      if (QTSServerInterface::GetServer()->GetPrefs()->PrintSRHeaders()) {
        qtss_printf("\n");
        qtss_printf("<send sess=%"   _U32BITARG_   ": SR %s xmit_sec=%.3f %s size=%"   _U32BITARG_   " ",
                    this->fSession->GetUniqueID(),
                    this->GetStreamTypeStr(),
                    this->GetStreamStartTimeSecs(),
                    theType,
    ...
    case RTPStream::rtcpRR:
      if (QTSServerInterface::GetServer()->GetPrefs()->PrintRRHeaders()) {
        ...
        if (rtcpRR.ParseReport((UInt8 *) inBuffer, inLen)) {
          qtss_printf("\n");
          qtss_printf(">recv sess=%"   _U32BITARG_   ": RTCP %s recv_sec=%.3f %s size=%"   _U32BITARG_   " ",
                      this->fSession->GetUniqueID(),
                      rr,
                      this->GetStreamStartTimeSecs(),
                      theType,
