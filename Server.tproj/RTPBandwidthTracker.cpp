...
void RTPBandwidthTracker::SetWindowSize(SInt32 clientWindowSize) {
  ...
  //if ( initSlowStart )
  //  s_printf( "ack list initializing with slow start.\n" );
  //else
  //  s_printf( "ack list initializing at full speed.\n" );
#endif
...
void RTPBandwidthTracker::EmptyWindow(UInt32 bytesIncreased,
                                      bool updateBytesInList) {
  if (bytesIncreased == 0)
    return;
  ...
  // this assert hits
  Assert(fBytesInList < ((UInt32) fClientWindow
      + 2000)); //mainly just to catch fBytesInList wrapping below 0
...
//  fCongestionWindow = fCongestionWindow / 2;  // half the congestion window size
  SInt64 theTime = CF::Core::Time::Milliseconds();
...
void RTPBandwidthTracker::AddToRTTEstimate(SInt32 rttSampleMSecs) {
  ...
  if (fRunningAverageMSecs == 0)
    fRunningAverageMSecs =
        rttSampleMSecs * 8;  // init avg to cur sample, scaled by 2**3

  SInt32 delta = rttSampleMSecs - fRunningAverageMSecs
      / 8; // scale average back to get cur delta from sample

  // add 1/8 the delta back to the smooth running average
  fRunningAverageMSecs = fRunningAverageMSecs
      + delta; // same as: rt avg = rt avg + delta / 8, but scaled
  ...
  */
  ...
  fUnadjustedRTO = fCurRetransmitTimeout =
      fRunningAverageMSecs / 8 + fRunningMeanDevationMSecs;
...
void RTPBandwidthTracker::UpdateAckTimeout(UInt32 bitsSentInInterval,
                                           SInt64 intervalLengthInMsec) {
  //
  ...
  if (bitsSentInInterval > 0)
    unadjustedTimeout = (UInt32) ((intervalLengthInMsec * fCongestionWindow)
        / bitsSentInInterval);
  ...
  //s_printf("UnadjustedTimeout = %"   _U32BITARG_   ". rto: %" _S32BITARG_ ". Last ack timeout: %"   _U32BITARG_   ". Adjustment = %"   _U32BITARG_   ".", unadjustedTimeout, fUnadjustedRTO, fAckTimeout, adjustment);
  if (adjustment > unadjustedTimeout)
    adjustment = unadjustedTimeout;
  fAckTimeout = unadjustedTimeout - adjustment;

  //s_printf("AckTimeout: %"   _U32BITARG_   "\n",fAckTimeout);
