...
QTSS_Error QTSSCallbacks::QTSS_AddRole(QTSS_Role inRole) {
  QTSS_ModuleState
      *theState = (QTSS_ModuleState *) OSThread::GetMainThreadData();
...
QTSS_Error QTSSCallbacks::QTSS_AddStaticAttribute(QTSS_ObjectType inObjectType,
...
QTSS_Error QTSSCallbacks::QTSS_AddInstanceAttribute(QTSS_Object inObject,
  ...
  return ((QTSSDictionary *) inObject)->AddInstanceAttribute(inAttrName,
                                                             NULL,
                                                             inAttrDataType,
                                                             qtssAttrModeRead
                                                                 | qtssAttrModeWrite
...
QTSS_Error QTSSCallbacks::QTSS_IDForAttr(QTSS_ObjectType inType,
  ...
  return QTSSDictionaryMap::GetMap(theDictionaryIndex)->GetAttrID(inName,
                                                                  outID);
}
...
QTSS_Error QTSSCallbacks::QTSS_GetValue(QTSS_Object inDictionary,
                                        QTSS_AttributeID inID,
                                        UInt32 inIndex,
                                        void *ioBuffer,
                                        UInt32 *ioLen) {
  if (inDictionary == NULL || (inID == qtssIllegalAttrID))
    return QTSS_BadArgument;
  return ((QTSSDictionary *) inDictionary)->GetValue(inID,
                                                     inIndex,
                                                     ioBuffer,
                                                     ioLen);
}

QTSS_Error QTSSCallbacks::QTSS_GetValueAsString(QTSS_Object inDictionary,
                                                QTSS_AttributeID inID,
                                                UInt32 inIndex,
                                                char **outString) {
  if (inDictionary == NULL)
    return QTSS_BadArgument;
  return ((QTSSDictionary *) inDictionary)->GetValueAsString(inID,
                                                             inIndex,
                                                             outString);
}

QTSS_Error QTSSCallbacks::QTSS_TypeToTypeString(const QTSS_AttrDataType inType,
                                                char **outTypeString) {
...
QTSS_Error QTSSCallbacks::QTSS_SetValue(QTSS_Object inDictionary,
                                        QTSS_AttributeID inID,
                                        UInt32 inIndex,
                                        const void *inBuffer,
                                        UInt32 inLen) {
  if ((inDictionary == NULL) || ((inBuffer == NULL) && (inLen > 0))
      || (inID == qtssIllegalAttrID))
    return QTSS_BadArgument;
  return ((QTSSDictionary *) inDictionary)->SetValue(inID,
                                                     inIndex,
                                                     inBuffer,
                                                     inLen);
}
...
QTSS_Error QTSSCallbacks::QTSS_Read(QTSS_StreamRef inStream,
                                    void *ioBuffer,
                                    UInt32 inBufLen,
  ...
  if ((inStream == NULL) || (ioBuffer == NULL))
    return QTSS_BadArgument;
  QTSS_Error theErr =
      ((QTSSStream *) inStream)->Read(ioBuffer, inBufLen, outLengthRead);
...
QTSS_Error QTSSCallbacks::QTSS_Seek(QTSS_StreamRef inStream,
...
QTSS_Error QTSSCallbacks::QTSS_Advise(QTSS_StreamRef inStream,
                                      UInt64 inPosition,
                                      UInt32 inAdviseSize) {
  if (inStream == NULL)
    return QTSS_BadArgument;
...
QTSS_Error QTSSCallbacks::QTSS_OpenFileObject(char *inPath,
...
QTSS_Error QTSSCallbacks::QTSS_CreateStreamFromSocket(int inFileDesc,
...
QTSS_Error QTSSCallbacks::QTSS_SendStandardRTSPResponse(QTSS_RTSPRequestObject inRTSPRequest,
  ...
  switch (((RTSPRequestInterface *) inRTSPRequest)->GetMethod()) {
    ...
    case qtssPauseMethod:((RTPSession *) inRTPInfo)->SendPauseResponse((RTSPRequestInterface *) inRTSPRequest);
      return QTSS_NoErr;
    case qtssTeardownMethod:((RTPSession *) inRTPInfo)->SendTeardownResponse((RTSPRequestInterface *) inRTSPRequest);
      return QTSS_NoErr;
    case qtssAnnounceMethod:((RTPSession *) inRTPInfo)->SendAnnounceResponse((RTSPRequestInterface *) inRTSPRequest);
      return QTSS_NoErr;
  }
  return QTSS_BadArgument;
}
...
QTSS_Error QTSSCallbacks::QTSS_RequestEvent(QTSS_StreamRef inStream,
...
QTSS_Error QTSSCallbacks::QTSS_Authorize(QTSS_RTSPRequestObject inAuthRequestObject,
  ...
  *outAuthUserAllowed = allowedDefault;
  ...
  for (; x < numModules; x++) {
    ...
    debug_printf(
          " QTSSCallbacks::QTSS_Authorize calling module module = %lu numModules=%lu\n",
          x,
          numModules);
    ...
    if (theModulePtr) {
      if (__QTSSCALLBACKS_DEBUG__)
        theModulePtr->GetValue(qtssModName)->PrintStr(
    ...
    handled = request->GetAuthHandled();
    debug_printf(
          "QTSSCallbacks::QTSS_Authorize allowedDefault =%d allowed= %d hasUser = %d handled=%d \n",
          allowedDefault,
          allowed,
          hasUser,
          handled);
...
void *QTSSCallbacks::Easy_GetRTSPPushSessions() {
  OSRefTable *reflectorSessionMap =
      QTSServerInterface::GetServer()->GetReflectorSessionMap();
  ...
  for (OSRefHashTableIter theIter(reflectorSessionMap->GetHashTable());
    ...
    RTPSession
        *clientSession = (RTPSession *) theSession->GetBroadcasterSession();
