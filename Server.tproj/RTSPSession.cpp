...
StrPtrLenRTSPSession::sHTTPResponseHeaderPtr
(sHTTPResponseHeaderBuf, kMaxHTTPResponseLen
);

char        RTSPSession::sHTTPResponseNoServerHeaderBuf[kMaxHTTPResponseLen];
StrPtrLenRTSPSession::sHTTPResponseNoServerHeaderPtr
(sHTTPResponseNoServerHeaderBuf, kMaxHTTPResponseLen
);
...
char *RTSPSession::sHTTPResponseFormatStr =
    "HTTP/1.0 200 OK\r\n%s%s%s%s\r\nConnection: close\r\nDate: Thu, 19 Aug 1982 18:30:00 GMT\r\nCache-Control: no-store\r\nPragma: no-cache\r\nContent-Type: application/x-rtsp-tunnelled\r\n\r\n";
char *RTSPSession::sHTTPNoServerResponseFormatStr =
...
void RTSPSession::Initialize() {
  ...
  qtss_sprintf(sHTTPResponseHeaderBuf,
  ...
  qtss_sprintf(sHTTPResponseNoServerHeaderBuf,
...
RTSPSession::~RTSPSession() {
  ...
  // Invoke modules
  for (UInt32 x = 0; x
      < QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPSessionClosingRole);
       x++)
    (void) QTSServerInterface::GetModule(QTSSModule::kRTSPSessionClosingRole,
                                         x)->CallDispatch(
        QTSS_RTSPSessionClosing_Role, &theParams);
  ...
  while (this->IsLiveSession()) {
    ...
    switch (fState) {
      ...
      case kHTTPFilteringRequest: {
        ...
        fState =
            kHaveNonTunnelMessage; // assume it's not a tunnel setup message
        ...
        if (preFilterErr == QTSS_NoErr) { HTTP_TRACE(
              "RTSPSession::Run kHTTPFilteringRequest\n")
          continue;
        } else {
          // pre filter error indicates a tunnelling message that could
          // not join to a session.
          HTTP_TRACE(
              "RTSPSession::Run kHTTPFilteringRequest Tunnel protocol ERROR.\n")
          return -1;
        }
      }
      ...
      case kHaveNonTunnelMessage:  // 说明请求报文格式是正确的，请求已进入受理状态
      {
        ...
        fRoleParams.rtspRequestParams.inRTSPHeaders =
            fRequest->GetHeaderDictionary();
        ...
        if (err == E2BIG) {
          (void) QTSSModuleUtils::SendErrorResponse(fRequest,
      ...
      case kFilteringRequest: {
        ...
        numModules =
            QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPFilterRole);
        for (; (fCurrentModule < numModules) &&
          ...
          theModule = QTSServerInterface::GetModule(QTSSModule::kRTSPFilterRole,
                                                    fCurrentModule);
          (void) theModule->CallDispatch(QTSS_RTSPFilter_Role,
                                         &theFilterParams);
          ...
          if (theReplacedRequest != nullptr) {
            ...
            fRequest->SetVal(qtssRTSPReqFullRequest,
                             theReplacedRequest,
                             ::strlen(theReplacedRequest));
        ...
        if (fSentOptionsRequest && this->ParseOptionsResponse()) {
          fRoundTripTime =
              (SInt32) (OS::Milliseconds() - fOptionsRequestSendTime);
          ...
          // 建立用于管理数据传输的RTPSession类对象,根据请求消息中的方法做处理。
          this->SetupRequest();
        }
      ...
      case kRoutingRequest: {
        ...
        numModules =
            QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPRouteRole);
        {
          ...
          for (; (fCurrentModule < numModules) &&
            ...
            theModule =
                QTSServerInterface::GetModule(QTSSModule::kRTSPRouteRole,
                                              fCurrentModule);
      ...
      case kAuthenticatingRequest: {
        ...
        StrPtrLenDel prefRealm
            (QTSServerInterface::GetServer()->GetPrefs()->GetAuthorizationRealm());
        ...
        if (method != qtssIllegalMethod)
          do {   //Set the request action before calling the authentication module
            ...
            if ((method == qtssAnnounceMethod) ||
                ((method == qtssSetupMethod) && fRequest->IsPushRequest())) {
            ...
            if (QTSS_NoErr == fRTPSession->GetValue(sClientBroadcastSessionAttr,
                                                    0,
                                                    &theSession,
                                                    &theLen)) {
        ...
        if (fRequest->GetAuthScheme() == qtssAuthNone) {
          QTSS_AuthScheme scheme =
              QTSServerInterface::GetServer()->GetPrefs()->GetAuthScheme();
        ...
        StrPtrLen *lastUsedDigestChallengePtr =
            this->GetValue(qtssRTSPSesLastDigestChallenge);
        if (lastUsedDigestChallengePtr != nullptr)
          (void) fRequest->SetValue(qtssRTSPReqDigestChallenge,
                                    (UInt32) 0,
                                    (void *) lastUsedDigestChallengePtr->Ptr,
                                    lastUsedDigestChallengePtr->Len,
                                    QTSSDictionary::kDontObeyReadOnly);

        numModules =
            QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPAthnRole);
        for (fCurrentModule = 0; (fCurrentModule < numModules)
            && ((!fRequest->HasResponseBeenSent()) ||
                fModuleState.eventRequested); fCurrentModule++) {
          ...
          debug_printf(
                "RTSPSession.cpp:kAuthenticatingRequest  fCurrentModule = %lu numModules=%lu\n",
                fCurrentModule,
                numModules);
          ...
          theModule = QTSServerInterface::GetModule(QTSSModule::kRTSPAthnRole,
                                                    fCurrentModule);
          if (nullptr == theModule)
            continue;

          if (__RTSP_AUTH_DEBUG__) {
            theModule->GetValue(qtssModName)->PrintStr(
                "QTSSModule::CallDispatch ENTER module=",
                "\n");
          }

          (void) theModule->CallDispatch(QTSS_RTSPAuthenticate_Role,
          ...
          debug_printf(
                "RTSPSession::Run Role(kAuthenticatingRequest) allowedDefault =%d allowed= %d hasUser = %d handled=%d \n",
                allowedDefault,
                allowed,
                hasUser,
                handled);
          ...
          if (hasUser || handled) {
            debug_printf(
                  "RTSPSession.cpp::Run(kAuthenticatingRequest)  skipping other modules fCurrentModule = %lu numModules=%lu\n",
                  fCurrentModule,
                  numModules);
            break;
          }
      ...
      case kAuthorizingRequest: {
        ...
        numModules =
            QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPAuthRole);
        bool allowedDefault =
            QTSServerInterface::GetServer()->GetPrefs()->GetAllowGuestDefault();
        bool allowed = true;
        bool hasUser = false;
        ...
        for (; (fCurrentModule < numModules) &&
          ...
          fRequest->SetAuthHandled(false);
          debug_printf(
                "RTSPSession.cpp:kAuthorizingRequest  BEFORE DISPATCH fCurrentModule = %lu numModules=%lu\n",
                fCurrentModule,
                numModules);
          ...
          theModule = QTSServerInterface::GetModule(QTSSModule::kRTSPAuthRole,
                                                    fCurrentModule);
          if (nullptr == theModule)
            continue;
          ...
          handled = fRequest->GetAuthHandled();
          debug_printf(
                "RTSPSession::Run Role(kAuthorizingRequest) allowedDefault =%d allowed= %d hasUser = %d handled=%d \n",
                allowedDefault,
                allowed,
                hasUser,
                handled);
        ...
        if (!allowed) {
          if (false == fRequest->HasResponseBeenSent()) {
            ...
            if (challengeScheme == qtssAuthDigest) {
              ...
              debug_printf(
                    "RTSPSession.cpp:kAuthorizingRequest  scheme == qtssAuthBasic)\n");
            }
            ...
            if (challengeScheme == qtssAuthBasic) {
              ...
              fRTPSession->UpdateDigestAuthChallengeParams(false,
                                                           false,
                                                           RTSPSessionInterface::kNoQop);
              theErr = fRequest->SendDigestChallenge(fRTPSession->GetAuthQop(),
                                                     fRTPSession->GetAuthNonce(),
                                                     fRTPSession->GetAuthOpaque());
            } else {
            ...
            if (QTSS_NoErr !=
                theErr) // We had an error so bail on the request quit the session and post process the request.
            {
      ...
      case kPreprocessingRequest: {
        ...
        numModules =
            QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPPreProcessorRole);
        {
          // Manipulation of the RTPSession from the point of view of
          ...
          for (; (fCurrentModule < numModules) &&
            ...
            theModule =
                QTSServerInterface::GetModule(QTSSModule::kRTSPPreProcessorRole,
                                              fCurrentModule);
            (void) theModule->CallDispatch(QTSS_RTSPPreProcessor_Role,
                                           &fRoleParams);
            fModuleState.isGlobalLocked = false;
      ...
      case kProcessingRequest: {
        ...
        fModuleState.eventRequested = false;
        fModuleState.idleTime = 0;
        if (QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPRequestRole)
            > 0) {
          // Manipulation of the RTPSession from the point of view of
          // a module is guarenteed to be atomic by the API.
          Assert(fRTPSession != nullptr);
          ...
          theModule =
              QTSServerInterface::GetModule(QTSSModule::kRTSPRequestRole, 0);
        ...
        if (!fRequest->HasResponseBeenSent()) {
          ...
          {
            QTSS_RTSPStatusCode
                statusCode = qtssSuccessOK; //qtssClientParameterNotUnderstood;
            fRequest->SetValue(qtssRTSPReqStatusCode,
                               0,
                               &statusCode,
                               sizeof(statusCode));
            fRequest->SendHeader();
      ...
      case kPostProcessingRequest: {
        ...
        if (fRTPSession != nullptr) {
          ...
          numModules =
              QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPPostProcessorRole);
          {
            // Manipulation of the RTPSession from the point of view of
            // a module is guarenteed to be atomic by the API.
            OSMutexLocker locker(fRTPSession->GetSessionMutex());
...
QTSS_Error RTSPSession::PreFilterForHTTPProxyTunnel() {
  ...
  { HTTP_TRACE("RTSPSession is a GET request.\n")
    ...
    bool showServerInfo =
        QTSServerInterface::GetServer()->GetPrefs()->GetRTSPServerInfoEnabled();
    if (fDoReportHTTPConnectionAddress) {
      ...
      this->GetValue(qtssRTSPSesLocalAddrStr,
                     0,
                     localIPAddr.Ptr,
                     &localIPAddr.Len);
      ...
      if (showServerInfo) {
        ...
        qtss_sprintf(responseHeaderBuf,
                     sHTTPResponseFormatStr,
                     "X-server-ip-address: ",
                     localIPAddrBuf,
                     "\r\n",
  ...
  OSRef *rtspSessionRef =
      this->RegisterRTSPSessionIntoHTTPProxyTunnelMap(theOtherSessionType);
  ...
  if (rtspSessionRef == nullptr) { HTTP_TRACE(
        "RegisterRTSPSessionIntoHTTPProxyTunnelMap returned nullptr. Abort.\n");
    return QTSS_RequestFailed;
  }

  // We registered ourselves into the map (we are the first half), so wait for our other half
  if (rtspSessionRef == &fProxyRef) { HTTP_TRACE(
        "Registered this session into map. Waiting to bind\n");
    return QTSS_NoErr;
  }

  OSRefReleaser theRefReleaser
      (sHTTPProxyTunnelMap, rtspSessionRef); // auto release this ref
  RTSPSession
      *theOtherSession = (RTSPSession *) theRefReleaser.GetRef()->GetObject();
  ...
  if (fHTTPMethod == kHTTPMethodPost) {
    ...
    HTTP_TRACE_ONE(
        "RTSPSession POST snarfed a donor session successfuly (%s).\n",
        fProxySessionID)
    ...
    Assert(theOtherSession->fState == kWaitingToBindHTTPTunnel);HTTP_TRACE_ONE(
        "RTSPSession GET snarfed a donor session successfuly (%s).\n",
        fProxySessionID)
...
OSRef *RTSPSession::RegisterRTSPSessionIntoHTTPProxyTunnelMap(
  ...
  if (scheme != (fRTPSession->GetAuthScheme())) {
    ...
    if (userPassword->Len == 0) {
      ...
      if (::strcmp(userPasswdStr, md5EncodeResult) != 0)
          authenticated = false;
#else
      if (::strcmp(userPasswdStr, (char *) crypt(reqPasswdStr, userPasswdStr))
          != 0)
        authenticated = false;
#endif
    }
    ...
    do {
      ...
      if (qop == RTSPSessionInterface::kAuthQop) {
        ...
        CalcRequestDigest(userPassword,
                          nonce,
                          nonceCount,
                          cNonce,
                          &sAuthQop,
  ...
  if (!fRequest->GetAuthHandled()) {
    if ((!authenticated) || (authenticated && (fRequest->GetStale()))) {
      ...
      (void) profile->SetValue(qtssUserName,
                               ...
                               sEmptyStr.Len,
                               QTSSDictionary::kDontObeyReadOnly);
      (void) profile->SetValue(qtssUserPassword,
                               0,
                               sEmptyStr.Ptr,
                               sEmptyStr.Len,
                               QTSSDictionary::kDontObeyReadOnly);
...
void RTSPSession::SetupRequest() {
  ...
  if (fRTPSession != nullptr) {
    ...
    if (headerBits != 0)
      (void) fRTPSession->SetValue(qtssCliSessLastRTSPBandwidth,
                                   (UInt32) 0,
                                   &headerBits,
                                   sizeof(headerBits),
                                   QTSSDictionary::kDontObeyReadOnly);
  ...
  {
    StrPtrLen
        *cSeqPtr = fRequest->GetHeaderDictionary()->GetValue(qtssCSeqHeader);
    ...
    fRequest->AppendHeader(qtssPublicHeader,
                           QTSServerInterface::GetPublicHeader());

    // DJM PROTOTYPE
    StrPtrLen *requirePtr =
        fRequest->GetHeaderDictionary()->GetValue(qtssRequireHeader);
    if (requirePtr && requirePtr->EqualIgnoreCase(RTSPProtocol::GetHeaderString(
        qtssXRandomDataSizeHeader))) {
      body = (char *) RTSPSessionInterface::sOptionsRequestBody;
      bodySizeBytes = fRequest->GetRandomDataSize();
      Assert(
          bodySizeBytes <= sizeof(RTSPSessionInterface::sOptionsRequestBody));
  ...
  if (fRequest->GetMethod() == qtssDescribeMethod) {
    if (fRequest->GetHeaderDictionary()->GetValue(qtssSessionHeader)->Len > 0) {
      (void) QTSSModuleUtils::SendErrorResponse(fRequest,
                                                qtssClientHeaderFieldNotValid,
                                                qtssMsgNoSesIDOnDescribe);
      return;
    }
  }
  ...
  if ((fRequest->GetMethod() == qtssPlayMethod)
      && (fRequest->GetLateToleranceInSec() != -1))
    fRTPSession->SetStreamThinningParams(fRequest->GetLateToleranceInSec());
  ...
  if ((fRequest->GetMethod() == qtssPlayMethod)
      && (fRTPSession->GetSessionState() == qtssPlayingState)
...
void RTSPSession::CleanupRequest() {
  ...
  if (fRequest != nullptr) {
    ...
    // their request now.
    if (fRequest->GetValue(qtssRTSPReqFullRequest)
        && fInputStream.GetRequestBuffer()) {
      if (fRequest->GetValue(qtssRTSPReqFullRequest)->Ptr
          != fInputStream.GetRequestBuffer()->Ptr)
...
void RTSPSession::SetupClientSessionAttrs() {
  ...
  (void) fRTPSession->SetValue(qtssCliSesPresentationURL,
                               0,
                               theValue->Ptr,
                               theValue->Len,
                               QTSSDictionary::kDontObeyReadOnly);
  ...
  Assert(theValue != nullptr);
  (void) fRTPSession->SetValue(qtssCliSesFullURL,
                               0,
                               theValue->Ptr,
                               theValue->Len,
                               QTSSDictionary::kDontObeyReadOnly);
  ...
  Assert(theValue != nullptr);
  (void) fRTPSession->SetValue(qtssCliSesHostName,
                               0,
                               theValue->Ptr,
                               theValue->Len,
                               QTSSDictionary::kDontObeyReadOnly);
  ...
  Assert(theValue != nullptr);
  (void) fRTPSession->SetValue(qtssCliSesFirstUserAgent,
                               0,
                               theValue->Ptr,
                               theValue->Len,
                               QTSSDictionary::kDontObeyReadOnly);
  ...
  if (fRequest->GetMethod() == qtssDescribeMethod) {
    ...
    Assert(theValue != nullptr);
    (void) fRTPSession->SetValue(qtssCliSesReqQueryString,
                                 0,
                                 theValue->Ptr,
                                 theValue->Len,
                                 QTSSDictionary::kDontObeyReadOnly);
  ...
  StrPtrLen tempStr;
  tempStr.Len = 0;
  (void) this->GetValuePtr(qtssRTSPSesRemoteAddrStr,
                           (UInt32) 0,
                           (void **) &tempStr.Ptr,
                           &tempStr.Len);
  Assert(tempStr.Len != 0);
  (void) fRTPSession->SetValue(qtssCliRTSPSessRemoteAddrStr,
                               (UInt32) 0,
                               tempStr.Ptr,
                               tempStr.Len,
                               QTSSDictionary::kDontObeyReadOnly);
  ...
  tempStr.Len = 0;
  (void) this->GetValuePtr(qtssRTSPSesLocalDNS,
                           (UInt32) 0,
                           (void **) &tempStr.Ptr,
                           &tempStr.Len);
  Assert(tempStr.Len != 0);
  (void) fRTPSession->SetValue(qtssCliRTSPSessLocalDNS,
                               (UInt32) 0,
                               (void **) tempStr.Ptr,
                               tempStr.Len,
                               QTSSDictionary::kDontObeyReadOnly);
  ...
  tempStr.Len = 0;
  (void) this->GetValuePtr(qtssRTSPSesLocalAddrStr,
                           (UInt32) 0,
                           (void **) &tempStr.Ptr,
                           &tempStr.Len);
  Assert(tempStr.Len != 0);
  (void) fRTPSession->SetValue(qtssCliRTSPSessLocalAddrStr,
                               (UInt32) 0,
                               tempStr.Ptr,
                               tempStr.Len,
                               QTSSDictionary::kDontObeyReadOnly);
...
UInt32 RTSPSession::GenerateNewSessionID(char *ioBuffer) {
...

  {
    ...
    OSRefHashTable
        *theHashTable = theServer->GetRTPSessionMap()->GetHashTable();
...
QTSS_Error RTSPSession::IsOkToAddNewRTPSession() {
  ...
  if (this->OverMaxConnections(0))
    return QTSSModuleUtils::SendErrorResponse(fRequest,
                                              qtssClientNotEnoughBandwidth,
                                              qtssMsgTooManyClients);

  //if the max bandwidth limit has been hit
  SInt32 maxKBits = theServer->GetPrefs()->GetMaxKBitsBandwidth();
  if ((maxKBits > -1)
      && (theServer->GetCurBandwidthInBits() >= ((UInt32) maxKBits * 1024)))
    return QTSSModuleUtils::SendErrorResponse(fRequest,
                                              qtssClientNotEnoughBandwidth,
...
void RTSPSession::SaveRequestAuthorizationParams(RTSPRequest *theRTSPRequest) {
  ...
  if (tempPtr) {
    if (tempPtr->Len == 0) {
      ...
      OSCharArrayDeleter theDefaultRealm
          (QTSServerInterface::GetServer()->GetPrefs()->GetAuthorizationRealm());
...
void RTSPSession::HandleIncomingDataPacket() {
  ...
  packetParams.rtspIncomingDataParams.inPacketData =
      fInputStream.GetRequestBuffer()->Ptr;
  packetParams.rtspIncomingDataParams.inPacketLen =
      fInputStream.GetRequestBuffer()->Len;

  UInt32 numModules =
      QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPIncomingDataRole);
  for (; fCurrentModule < numModules; fCurrentModule++) {
    QTSSModule *theModule =
        QTSServerInterface::GetModule(QTSSModule::kRTSPIncomingDataRole,
                                      fCurrentModule);
