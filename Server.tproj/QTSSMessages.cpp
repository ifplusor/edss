...
void QTSSMessages::Initialize() {
  ...
  for (UInt32 x = 0; x < qtssMsgNumParams; x++)
    theMap->SetAttribute(x, sMessagesKeyStrings[x], nullptr, qtssAttrDataTypeCharArray,
...
QTSSMessages::QTSSMessages(PrefsSource *inMessages)
  ...
  for (UInt32 x = 0; x < numAttrs; x++) {
    ...
    if (theMessage[0] != '\0') {
      ...
      this->SetVal(this->GetDictionaryMap()->GetAttrID(x), attrBuffer[x], ::strlen(attrBuffer[x]));
    }
  }
}
