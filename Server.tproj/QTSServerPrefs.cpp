...
char *QTSServerPrefs::sAdditionalDefaultPorts[] = { "554", NULL };

char *QTSServerPrefs::sRTP_Header_Players[] = { "EasyPlayer", NULL };

char *QTSServerPrefs::sAdjust_Bandwidth_Players[] = { "EasyPlayer", NULL };

char *QTSServerPrefs::sNo_Pause_Time_Adjustment_Players[] = { "EasyPlayer", NULL };

char *QTSServerPrefs::sRTP_Start_Time_Players[] = { NULL };
  ...
  for (UInt32 x = 0; x < theMap->GetNumAttrs(); x++) {
    ...
    if ((thePrefValue == nullptr) && (x < qtssPrefsNumParams)) { // Only generate errors for server prefs
      ...
      if (::strlen(sPrefInfo[x].fDefaultValue) > 0) {
        ...
        QTSSModuleUtils::LogError(QTSSModuleUtils::GetMisingPrefLogVerbosity(), qtssServerPrefMissing, 0,
                                  sAttributes[x].fAttrName, sPrefInfo[x].fDefaultValue);
      }
    ...
    if ((x < qtssPrefsNumParams) && (theType != sAttributes[x].fAttrDataType)) { // Only generate errors for server prefs
      ...
      if (::strlen(sPrefInfo[x].fDefaultValue) > 0) {
        ...
        // doesn't count). If there is no default, we will constantly print
        // out an error message...
        QTSSModuleUtils::LogError(qtssWarningVerbosity, qtssServerPrefWrongType, 0,
                                  sAttributes[x].fAttrName, sPrefInfo[x].fDefaultValue);
      }
