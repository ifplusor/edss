...
RTSPRequestStream::RTSPRequestStream(CF::Net::TCPSocket *sock)
    : fSocket(sock),
      fRetreatBytes(0),
      fRetreatBytesRead(0),
      fCurOffset(0),
...
void RTSPRequestStream::SnarfRetreat(RTSPRequestStream &fromRequest) {
  ...
  ::memcpy(&fRequestBuffer[0],
           fromRequest.fRequest.Ptr + fromRequest.fRequest.Len,
  ...
  while (true) {
    ...
    if (newOffset == 0) {
      ...
      if (fDecode) {
        ...
        QTSS_Error decodeErr = this->DecodeIncomingData(&fRequestBuffer[fCurOffset - fEncodedBytesRemaining], newOffset + fEncodedBytesRemaining);
        ...
        // be ignored.
        if (decodeErr == QTSS_NoErr) Assert(fEncodedBytesRemaining < 4);
      } else
    ...
    if (fPrintRTSP) {
      ...
      if (fSocket != NULL) {
        ...
        CF::StrPtrLen *theLocalAddrStr = fSocket->GetLocalAddrStr();
        CF::StrPtrLen *theRemoteAddrStr = fSocket->GetRemoteAddrStr();
...
QTSS_Error RTSPRequestStream::DecodeIncomingData(char *inSrcData,
                                                 UInt32 inSrcDataLen) {
