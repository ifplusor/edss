...
bool QTSServer::Initialize(XMLPrefsParser *inPrefsSource,
  ...
  fSrvrPrefs = new QTSServerPrefs(inPrefsSource,
                                  false); // First time, don't write changes to the prefs file
...
bool QTSServer::CreateListeners(bool startListeningNow,
  ...
  TCPListenerSocket **newListenerArray =
      new TCPListenerSocket *[theTotalRTSPPortTrackers
          + theTotalHTTPPortTrackers];
  ...
  for (UInt32 count = 0; count < theTotalRTSPPortTrackers; count++) {
    for (UInt32 count2 = 0; count2 < fNumListeners; count2++) {
      if ((fListeners[count2]->GetLocalPort()
          == theRTSPPortTrackers[count].fPort) &&
          (fListeners[count2]->GetLocalAddr()
              == theRTSPPortTrackers[count].fIPAddr)) {
  ...
  for (UInt32 count = 0; count < theTotalHTTPPortTrackers; count++) {
    for (UInt32 count2 = 0; count2 < fNumListeners; count2++) {
      if ((fListeners[count2]->GetLocalPort()
          == theHTTPPortTrackers[count].fPort) &&
          (fListeners[count2]->GetLocalAddr()
              == theHTTPPortTrackers[count].fIPAddr)) {
        theHTTPPortTrackers[count].fNeedsCreating = false;
        newListenerArray[curPortIndex++] = fListeners[count2];
        Assert(curPortIndex
                   <= theTotalRTSPPortTrackers + theTotalHTTPPortTrackers);
        break;
      }
    }
  }
  ...
  for (UInt32 count3 = 0; count3 < theTotalRTSPPortTrackers; count3++) {
    if (theRTSPPortTrackers[count3].fNeedsCreating) {
      ...
      QTSS_Error err =
          newListenerArray[curPortIndex]->Initialize(theRTSPPortTrackers[count3].fIPAddr,
  ...
  for (UInt32 count3 = 0; count3 < theTotalHTTPPortTrackers; count3++) {
    if (theHTTPPortTrackers[count3].fNeedsCreating) {
      ...
      QTSS_Error err =
          newListenerArray[curPortIndex]->Initialize(theHTTPPortTrackers[count3].fIPAddr,
                                                     theHTTPPortTrackers[count3].fPort);
...
bool QTSServer::SetupUDPSockets() {
  ...
  for (UInt32 theNumPairs = 0; theNumPairs < SocketUtils::GetNumIPAddrs();
    ...
    UDPSocketPair *thePair =
        fSocketPool->CreateUDPSocketPair(SocketUtils::GetIPAddr(theNumPairs),
                                         0);
    if (thePair != nullptr) {
  ...
  if (::strlen(runGroupName.GetObject()) > 0) {
    ...
    if (gr == nullptr || ::setgid(gr->gr_gid) == -1) {
      ...
      QTSSModuleUtils::LogError(qtssFatalVerbosity, qtssMsgCannotSetRunGroup, 0,
                                ...
                                qtss_strerror(OSThread::GetErrno(),
                                              buffer,
                                              sizeof(buffer)));
      return false;
    }
...
void QTSServer::InitCallbacks() {
  ...
  sCallbacks.addr[kGetRTSPPushSessionsCallback] =
      (QTSS_CallbackProcPtr) QTSSCallbacks::Easy_GetRTSPPushSessions;
...
UDPSocketPair *RTPSocketPool::ConstructUDPSocketPair() {
  ...
  return new
      UDPSocketPair(new UDPSocket(theTask, Socket::kNonBlockingSocketType),
                    new UDPSocket(theTask,
                                  UDPSocket::kWantsDemuxer
                                      | Socket::kNonBlockingSocketType));
}
...
QTSS_Error QTSServer::RereadPrefsService(QTSS_ServiceFunctionArgsPtr /*inArgs*/) {
  ...
  for (UInt32 x = 0; x
      < QTSServerInterface::GetNumModulesInRole(QTSSModule::kRereadPrefsRole);
       x++) {
    QTSSModule *theModule =
