...
QTSSDictionary *QTSSDictionary::CreateNewDictionary(QTSSDictionaryMap *inMap,
                                                    OSMutex *inMutex) {
  return new QTSSDictionary(inMap, inMutex);
}

QTSS_Error QTSSDictionary::GetValuePtr(QTSS_AttributeID inAttrID,
                                       UInt32 inIndex,
                                       void **outValueBuffer,
                                       UInt32 *outValueLen,
                                       bool isInternal) {
  ...
  *outValueLen = theAttrs[theMapIndex].fAttributeData.Len;
  ...
  if ((theMap->GetAttrFunction(theMapIndex) != NULL)
      && ((cacheable && (*outValueLen == 0)) || !cacheable)) {
    ...
    theBuffer =
        (char *) theMap->GetAttrFunction(theMapIndex)(this, outValueLen);
  ...
  *outValueBuffer = theBuffer;
  ...
  if ((theMap->GetAttrType(theMapIndex) == qtssAttrDataTypeCharArray)
...
QTSS_Error QTSSDictionary::GetValueAsString(QTSS_AttributeID inAttrID,
                                            UInt32 inIndex,
                                            char **outString) {
  ...
  *outString = QTSSDataConverter::ValueToString(tempValueBuffer,
                                                tempValueLen,
                                                theMap->GetAttrType(theMapIndex));
  return QTSS_NoErr;
}

QTSS_Error QTSSDictionary::CreateObjectValue(QTSS_AttributeID inAttrID,
                                             UInt32 *outIndex,
                                             QTSSDictionary **newObject,
 ...
 */
QTSS_Error QTSSDictionary::SetValue(QTSS_AttributeID inAttrID,
                                    UInt32 inIndex,
                                    const void *inBuffer,
                                    UInt32 inLen,
                                    UInt32 inFlags) {
  ...
  if (dataType == qtssAttrDataTypeCharArray) {
    ...
    if ((numValues == 1) && (inIndex == 1)) {
      ...
      theAttrs[theMapIndex].fAttributeData.Ptr =
          new char[theAttrs[theMapIndex].fAllocatedLen];
  ...
  if ((attrLen * (inIndex + 1)) > theAttrs[theMapIndex].fAllocatedLen) {
    ...
    if (inIndex == 0)
      theLen =
          attrLen;   // most attributes are single valued, so allocate just enough space
    else
      theLen =
          2 * (attrLen * (inIndex + 1));// Allocate twice as much as we need
  ...
  if ((dataType != qtssAttrDataTypeCharArray)
    ...
    char **valuePtr = (char **) (theAttrs[theMapIndex].fAttributeData.Ptr
        + (attrLen * inIndex));
  ...
  if (((fMap == NULL) || fMap->CompleteFunctionsAllowed())
      && !(inFlags & kDontCallCompletionRoutine)) {
    // SetValueComplete是虚函数,不同的继承类有不同的实现
    this->SetValueComplete(theMapIndex,
                           theMap,
                           inIndex,
                           attributeBufferPtr,
                           inLen);
...
QTSS_Error QTSSDictionary::RemoveValue(QTSS_AttributeID inAttrID,
                                       UInt32 inIndex,
                                       UInt32 inFlags) {
  // Check first to see if this is a static attribute or an instance attribute
  QTSSDictionaryMap *theMap = fMap;
  ...
  if (theAttrs[theMapIndex].fIsDynamicDictionary) {
    ...
    QTSSDictionary
        *dict = *(QTSSDictionary **) (theAttrs[theMapIndex].fAttributeData.Ptr +
        (theValueLen * inIndex));
    delete dict;
  }
  ...
  if (inIndex + 1 < theAttrs[theMapIndex].fNumAttributes) {
    ::memmove(
        theAttrs[theMapIndex].fAttributeData.Ptr + (theValueLen * inIndex),
        theAttrs[theMapIndex].fAttributeData.Ptr
            + (theValueLen * (inIndex + 1)),
...
void QTSSDictionary::SetNumValues(QTSS_AttributeID inAttrID,
                                  UInt32 inNumValues) {
  // Check first to see if this is a static attribute or an instance attribute
  QTSSDictionaryMap *theMap = fMap;
  DictValueElement *theAttrs = fAttributes;
  ...
  QTSS_AttrDataType dataType = theMap->GetAttrType(theMapIndex);
  if (theAttrs[theMapIndex].fIsDynamicDictionary
      || (dataType == qtssAttrDataTypeCharArray)) {
    ...
    for (UInt32 removeCount = numAttributes - inNumValues;
         removeCount >
             0;
         removeCount--) {  // the delete index passed to RemoveValue is always the last in the array.
      this->RemoveValue(inAttrID,
                        theAttrs[theMapIndex].fNumAttributes - 1,
                        kDontObeyReadOnly);
    }
  } else {
...
void QTSSDictionary::SetEmptyVal(QTSS_AttributeID inAttrID,
                                 void *inBuf,
                                 UInt32 inBufLen) {
  Assert(inAttrID >= 0);
  Assert(fMap);
...
QTSS_Error QTSSDictionary::AddInstanceAttribute(const char *inAttrName,
  ...
  if (fInstanceMap == NULL) {
    UInt32 theFlags =
        QTSSDictionaryMap::kAllowRemoval | QTSSDictionaryMap::kIsInstanceMap;
    if ((fMap == NULL) || fMap->CompleteFunctionsAllowed())
  ...
  theErr = fInstanceMap->AddAttribute(inAttrName,
                                      inFuncPtr,
                                      inDataType,
                                      inPermission);
  if (theErr != QTSS_NoErr)
  ...
  if (fInstanceMap->GetNumAttrs() >= fInstanceArraySize) {
    ...
    if (fInstanceAttrs != NULL) {
      ::memcpy(theNewArray,
               fInstanceAttrs,
               sizeof(DictValueElement) * fInstanceArraySize);
...
QTSS_Error QTSSDictionary::GetAttrInfoByIndex(UInt32 inIndex,
  ...
  return fInstanceMap->GetAttrInfoByIndex(inIndex - numStaticValues,
                                          outAttrInfoDict);

}

QTSS_Error QTSSDictionary::GetAttrInfoByID(QTSS_AttributeID inAttrID,
                                           QTSSAttrInfoDict **outAttrInfoDict) {
  if (outAttrInfoDict == NULL)
    return QTSS_BadArgument;
...
QTSS_Error QTSSDictionary::GetAttrInfoByName(const char *inAttrName,
                                             QTSSAttrInfoDict **outAttrInfoDict) {
...
QTSSAttrInfoDict::AttrInfo  QTSSAttrInfoDict::sAttributes[] =
    {
        /* 0 */ {"qtssAttrName", NULL, qtssAttrDataTypeCharArray,
                 qtssAttrModeRead |
                     qtssAttrModePreempSafe},
        /* 1 */
                {"qtssAttrID", NULL, qtssAttrDataTypeUInt32, qtssAttrModeRead |
                    qtssAttrModePreempSafe},
        /* 2 */
                {"qtssAttrDataType", NULL, qtssAttrDataTypeUInt32,
                 qtssAttrModeRead |
                     qtssAttrModePreempSafe},
        /* 3 */
                {"qtssAttrPermissions", NULL, qtssAttrDataTypeUInt32,
...
QTSSDictionaryMap *QTSSDictionaryMap::sDictionaryMaps[
    kNumDictionaries + kNumDynamicDictionaryTypes];
...
void QTSSDictionaryMap::Initialize() {
  ...
  // dict maps.
  sDictionaryMaps[kAttrInfoDictIndex] =
      new QTSSDictionaryMap(qtssAttrInfoNumParams);
  ...
  sDictionaryMaps[kPrefsDictIndex] = new QTSSDictionaryMap(qtssPrefsNumParams,
                                                           QTSSDictionaryMap::kInstanceAttrsAllowed
                                                               |
                                                                   QTSSDictionaryMap::kCompleteFunctionsAllowed);
  sDictionaryMaps[kTextMessagesDictIndex] =
      new QTSSDictionaryMap(qtssMsgNumParams);
  sDictionaryMaps[kServiceDictIndex] = new QTSSDictionaryMap(0);
  sDictionaryMaps[kRTPStreamDictIndex] =
      new QTSSDictionaryMap(qtssRTPStrNumParams);
  sDictionaryMaps[kClientSessionDictIndex] = new QTSSDictionaryMap(
      qtssCliSesNumParams,
      QTSSDictionaryMap::kCompleteFunctionsAllowed);
  sDictionaryMaps[kRTSPSessionDictIndex] =
      new QTSSDictionaryMap(qtssRTSPSesNumParams);
  sDictionaryMaps[kRTSPRequestDictIndex] =
      new QTSSDictionaryMap(qtssRTSPReqNumParams);
  ...
  sDictionaryMaps[kModulePrefsDictIndex] =
      new QTSSDictionaryMap(0, QTSSDictionaryMap::kInstanceAttrsAllowed |
          QTSSDictionaryMap::kCompleteFunctionsAllowed);
  sDictionaryMaps[kQTSSUserProfileDictIndex] =
      new QTSSDictionaryMap(qtssUserNumParams);
  sDictionaryMaps[kQTSSConnectedUserDictIndex] =
      new QTSSDictionaryMap(qtssConnectionNumParams);
  sDictionaryMaps[kHTTPSessionDictIndex] =
      new QTSSDictionaryMap(qtssRTSPSesNumParams);
}
...
void QTSSDictionaryMap::SetAttribute(QTSS_AttributeID inID,
  ...
  // 创建描述属性的QTSSAttrInfoDict对象,将属性包含的信息(ID、名称、函数指针、数据类型、权限)保存下来。
  ...
  fAttrArray[theIndex]->SetVal(qtssAttrName,
                               &fAttrArray[theIndex]->fAttrInfo.fAttrName[0],
                               theNameLen);
  fAttrArray[theIndex]->SetVal(qtssAttrID,
                               &fAttrArray[theIndex]->fID,
...
QTSS_Error QTSSDictionaryMap::CheckRemovePermission(QTSS_AttributeID inAttrID) {
  ...
  if (0
      == (fAttrArray[theIndex]->fAttrInfo.fAttrPermission & qtssAttrModeDelete))
    return QTSS_BadArgument;
...
QTSS_Error QTSSDictionaryMap::GetAttrInfoByName(const char *inAttrName,
  ...
  for (UInt32 count = 0; count < fNextAvailableID; count++) {
    if (::strcmp(&fAttrArray[count]->fAttrInfo.fAttrName[0], inAttrName) == 0) {
      if ((fAttrArray[count]->fAttrInfo.fAttrPermission
          & qtssPrivateAttrModeRemoved) && (!returnRemovedAttr))
        continue;
...
QTSS_Error QTSSDictionaryMap::GetAttrID(const char *inAttrName,
