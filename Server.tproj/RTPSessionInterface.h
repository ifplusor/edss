...
class RTPSessionInterface : public QTSSDictionary, public CF::Thread::Task {
 public:
  ...
  void RefreshRTSPTimeout() {
    if (fRTSPSession != NULL)
      fRTSPSession->RefreshTimeout();
  }
  ...
  CF::Core::Mutex *GetSessionMutex() { return &fSessionMutex; }

  CF::Core::Mutex *GetRTSPSessionMutex() { return &fRTSPSessionMutex; }

  UInt32 GetPacketsSent() { return fPacketsSent; }

  UInt32 GetBytesSent() { return fBytesSent; }

  CF::Ref *GetRef() { return &fRTPMapElem; }
  ...
  void UpdateCurrentBitRate(const SInt64 &curTime) {
    if (curTime > (fLastBitRateUpdateTime + 3000))
      this->UpdateBitRateInternal(curTime);
  }

  void SetAllTracksInterleaved(bool newValue) {
    fAllTracksInterleaved = newValue;
  }
  //
  // RTSP RESPONSES
  ...
  void SetQueryString(CF::StrPtrLen *queryString);
  ...
  CF::StrPtrLen *GetAuthNonce() { return &fAuthNonce; }

  UInt32 GetAuthQop() { return fAuthQop; }

  UInt32 GetAuthNonceCount() { return fAuthNonceCount; }

  CF::StrPtrLen *GetAuthOpaque() { return &fAuthOpaque; }
  ...
  Float32 *GetPacketLossPercent() {
    UInt32 outLen;
    return (Float32 *) this->PacketLossPercent(this, &outLen);
  }
  ...
  UInt32 GetMaxBandwidthBits() {
    UInt32 maxRTSP = GetLastRTSPBandwithBits();
    return maxRTSP;
  }
 ...
 protected:
  ...
  CF::Core::Mutex fSessionMutex;
  CF::Core::Mutex fRTSPSessionMutex;
 ...
 private:
  ...
  CF::Thread::TimeoutTask fTimeoutTask;
  UInt32 fTimeout;
  ...
  CF::StrPtrLen fAuthNonce;
  UInt32 fAuthQop;
  UInt32 fAuthNonceCount;
  CF::StrPtrLen fAuthOpaque;
