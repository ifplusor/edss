...
RTSPSessionInterface::RTSPSessionInterface()
    ...
    // 对应于配置文件中的real_rtsp_timeout,缺省值为 180
      ...
      fSocket(nullptr, CF::Net::Socket::kNonBlockingSocketType),
...
UInt8 RTSPSessionInterface::GetTwoChannelNumbers(CF::StrPtrLen *inRTSPSessionID) {
  //
  ...
  CF::StrPtrLen *newMap = new CF::StrPtrLen[numChannelEntries];
  if (fChNumToSessIDMap != nullptr) {
    Assert(numChannelEntries > 1);
    ::memcpy(newMap, fChNumToSessIDMap, sizeof(CF::StrPtrLen) * (numChannelEntries - 1));
    delete[] fChNumToSessIDMap;
  }
...
CF::StrPtrLen *RTSPSessionInterface::GetSessionIDForChannelNum(UInt8 inChannelNum) {
 ...
 */
QTSS_Error
RTSPSessionInterface::InterleavedWrite(void *inBuffer, UInt32 inLen, UInt32 *outLenWritten, unsigned char channel) {
  ...
  // flush rules
  if (fNumInCoalesceBuffer > 0 && (inLen == 0 || inLen > kTCPCoalesceDirectWriteSize ||
      inLen + fNumInCoalesceBuffer + kInteleaveHeaderSize > kTCPCoalesceBufferSize)) {
    ...
    s_printf("InterleavedWrite: flushing %li\n", fNumInCoalesceBuffer);
#endif
  ...
  if (err == QTSS_NoErr) {
    ...
    if (inLen > kTCPCoalesceDirectWriteSize) {
      ...
      RTPInterleaveHeader rih = {
          .header = '$',
          .channel = channel,
          .len = htons((UInt16) inLen)
      };
...
#if RTSP_SESSION_INTERFACE_DEBUGGING
      s_printf("InterleavedWrite: bypass %li\n", inLen);
#endif
...
#if RTSP_SESSION_INTERFACE_DEBUGGING
      s_printf("InterleavedWrite: coalesce %li, total bufff %li\n", inLen, fNumInCoalesceBuffer);
#endif
    }
  }
...
void RTSPSessionInterface::SnarfInputSocket(RTSPSessionInterface *fromRTSPSession) {
  ...
  if (fInputSocketP == fOutputSocketP)
    fInputSocketP = new CF::Net::TCPSocket(this, CF::Net::Socket::kNonBlockingSocketType);
  else
...
void *RTSPSessionInterface::SetupParams(QTSSDictionary *inSession, UInt32 * /*outLen*/) {
  ...
  CF::StrPtrLen *theLocalAddrStr = theSession->fSocket.GetLocalAddrStr();
  CF::StrPtrLen *theLocalDNSStr = theSession->fSocket.GetLocalDNSStr();
  CF::StrPtrLen *theRemoteAddrStr = theSession->fSocket.GetRemoteAddrStr();
  if (theLocalAddrStr == nullptr || theLocalDNSStr == nullptr || theRemoteAddrStr == nullptr) {    //the socket is bad most likely values are all 0. If the socket had an error we shouldn't even be here.
...
void RTSPSessionInterface::RevertOutputStream() {
  ...
  static CF::StrPtrLen theRTTStr(";rtt=", 5);

  if (fOldOutputStreamBuffer.Ptr != nullptr) {
    //fOutputStream.Put(fOldOutputStreamBuffer);
    CF::StringParser theStreamParser(&fOldOutputStreamBuffer);
    CF::StrPtrLen theHeader;
    CF::StrPtrLen theEOL;
    CF::StrPtrLen theField;
    CF::StrPtrLen theValue;
    while (theStreamParser.GetDataRemaining() != 0) {
      theStreamParser.ConsumeUntil(&theHeader, CF::StringParser::sEOLMask);
      if (theHeader.Len != 0) {
        fOutputStream.Put(theHeader);

        CF::StringParser theHeaderParser(&theHeader);
...
void RTSPSessionInterface::SendOptionsRequest() {
  static CF::StrPtrLen sOptionsRequestHeader(
  ...
  fOptionsRequestSendTime = CF::Core::Time::Milliseconds();
