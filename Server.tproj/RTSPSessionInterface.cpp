...
UInt32RTSPSessionInterface::sOptionsRequestBody[
kMaxRandomDataSize / sizeof(UInt32)];
...
QTSSAttrInfoDict::AttrInfo  RTSPSessionInterface::sAttributes[] =
    {   /*fields:   fAttrName, fFuncPtr, fAttrDataType, fAttrPermission */
        ...
        /* 1 */
        {"qtssRTSPSesLocalAddr", SetupParams, qtssAttrDataTypeUInt32,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 2 */
        {"qtssRTSPSesLocalAddrStr", SetupParams, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 3 */
        {"qtssRTSPSesLocalDNS", SetupParams, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 4 */
        {"qtssRTSPSesRemoteAddr", SetupParams, qtssAttrDataTypeUInt32,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 5 */
        {"qtssRTSPSesRemoteAddrStr", SetupParams, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 6 */
        {"qtssRTSPSesEventCntxt", nullptr, qtssAttrDataTypeUInt32,
         qtssAttrModeRead |
             qtssAttrModePreempSafe},
        /* 7 */
        {"qtssRTSPSesType", nullptr, qtssAttrDataTypeUInt32, qtssAttrModeRead |
            qtssAttrModePreempSafe},
        /* 8 */
        {"qtssRTSPSesStreamRef", nullptr, qtssAttrDataTypeQTSS_StreamRef,
         qtssAttrModeRead |
             qtssAttrModePreempSafe},
        /* 9 */
        {"qtssRTSPSesLastUserName", nullptr, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe},
        /* 10 */
        {"qtssRTSPSesLastUserPassword", nullptr, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe},
        /* 11 */
        {"qtssRTSPSesLastURLRealm", nullptr, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe},
        /* 12 */
        {"qtssRTSPSesLocalPort", SetupParams, qtssAttrDataTypeUInt16,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 13 */
        {"qtssRTSPSesRemotePort", SetupParams, qtssAttrDataTypeUInt16,
         qtssAttrModeRead |
             qtssAttrModePreempSafe |
             qtssAttrModeCacheable},
        /* 14 */
        {"qtssRTSPSesLastDigestChallenge", nullptr, qtssAttrDataTypeCharArray,
         qtssAttrModeRead |
             qtssAttrModePreempSafe}
...
void RTSPSessionInterface::Initialize() {
  ...
  ((char *) RTSPSessionInterface::sOptionsRequestBody)[0] =
      0; //always set first byte so it doesn't hit any client parser bugs for \r or \n.
...
RTSPSessionInterface::RTSPSessionInterface()
    ...
    // 对应于配置文件中的real_rtsp_timeout,缺省值为 180
      fTimeoutTask(nullptr,
                   QTSServerInterface::GetServer()->GetPrefs()->GetRTSPSessionTimeoutInSecs()
                       * 1000),
  ...
  this->SetEmptyVal(qtssRTSPSesLastUserPassword,
                    &fUserPasswordBuf[0],
                    kMaxUserPasswordLen);
  this->SetEmptyVal(qtssRTSPSesLastURLRealm,
                    &fUserRealmBuf[0],
                    kMaxUserRealmLen);
...
QTSS_Error RTSPSessionInterface::Read(void *ioBuffer,
                                      UInt32 inLength,
                                      UInt32 *outLenRead) {
  //
...
UInt8 RTSPSessionInterface::GetTwoChannelNumbers(StrPtrLen *inRTSPSessionID) {
  ...
  if (fChNumToSessIDMap != nullptr) {
    ...
    ::memcpy(newMap,
             fChNumToSessIDMap,
             sizeof(StrPtrLen) * (numChannelEntries - 1));
    delete[] fChNumToSessIDMap;
  }
...
RTSPSessionInterface::InterleavedWrite(void *inBuffer,
  ...
  if ((inLen > kTCPCoalesceDirectWriteSize || inLen == 0)
      && fNumInCoalesceBuffer > 0
      || (inLen + fNumInCoalesceBuffer + kInteleaveHeaderSize
          > kTCPCoalesceBufferSize) && fNumInCoalesceBuffer > 0
      ) {
    ...
    err = this->GetOutputStream()->WriteV(iov,
                                          2,
                                          fNumInCoalesceBuffer,
                                          &buffLenWritten,
                                          RTSPResponseStream::kAllOrNothing);
...
void *RTSPSessionInterface::SetupParams(QTSSDictionary *inSession,
                                        UInt32 * /*outLen*/) {
  ...
  if (theLocalAddrStr == nullptr || theLocalDNSStr == nullptr
      || theRemoteAddrStr ==
          nullptr) {    //the socket is bad most likely values are all 0. If the socket had an error we shouldn't even be here.
  ...
  theSession->SetVal(qtssRTSPSesLocalAddr,
                     &theSession->fLocalAddr,
                     sizeof(theSession->fLocalAddr));
  theSession->SetVal(qtssRTSPSesLocalAddrStr,
                     theLocalAddrStr->Ptr,
                     theLocalAddrStr->Len);
  theSession->SetVal(qtssRTSPSesLocalDNS,
                     theLocalDNSStr->Ptr,
                     theLocalDNSStr->Len);
  theSession->SetVal(qtssRTSPSesRemoteAddr,
                     &theSession->fRemoteAddr,
                     sizeof(theSession->fRemoteAddr));
  theSession->SetVal(qtssRTSPSesRemoteAddrStr,
                     theRemoteAddrStr->Ptr,
                     theRemoteAddrStr->Len);

  theSession->SetVal(qtssRTSPSesLocalPort,
                     &theSession->fLocalPort,
                     sizeof(theSession->fLocalPort));
  theSession->SetVal(qtssRTSPSesRemotePort,
    ...
    while (theStreamParser.GetDataRemaining() != 0) {
      ...
      if (theHeader.Len != 0) {
        ...
        if (theHeaderParser.PeekFast() == ':') {
          if (theField.Equal(RTSPProtocol::GetHeaderString(
              qtssXDynamicRateHeader))) {
...
void RTSPSessionInterface::SendOptionsRequest() {
  static StrPtrLen sOptionsRequestHeader(
