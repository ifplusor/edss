...
QTSS_ServerState StartServer(XMLPrefsParser *inPrefsSource,
  ...
  CF::Net::Socket::Initialize();
  CF::Net::SocketUtils::Initialize(false);
  ...
  CF::Utils::SetPersonality(runUserName.GetObject(), runGroupName.GetObject());
...
#if DEBUG
    s_printf("Number of task threads: %"   _U32BITARG_   "\n", numThreads);
#endif

    // Start up the server's global tasks, and start listening
    CF::Thread::TimeoutTask::Initialize();     // The TimeoutTask mechanism is task based,
  ...
  if (sServer->GetServerState() != qtssFatalErrorState) {
    CF::Thread::IdleTask::Initialize();
    CF::Net::Socket::StartThread();
...
void LogStatus(QTSS_ServerState theServerState) {
  ...
  if (statusFile != NULL) {
    ...
    for (i = 0; i < numAttributes; i++) {
      (void) QTSS_GetValueAsString(sServer,
                                   QTSSModuleUtils::GetAttrID(sServer,
                                                              sAttributes[i]),
                                   0,
                                   &theAttributeValue);
...
void DebugLevel_1(FILE *statusFile, FILE *stdOut, bool printHeader) {
  ...
  if (deltaPackets > 0)
    s_snprintf(numStr, sizeof(numStr) - 1, "%" _S32BITARG_ "",
                  (SInt32) ((SInt64) totalLate / (SInt64) deltaPackets));
  print_status(statusFile, stdOut, "%11s", numStr);

  s_snprintf(numStr,
                sizeof(numStr) - 1,
                "%" _S32BITARG_ "",
                (SInt32) currentMaxLate);
  print_status(statusFile, stdOut, "%11s", numStr);

  s_snprintf(numStr,
                sizeof(numStr) - 1,
                "%" _S32BITARG_ "",
                (SInt32) sServer->GetMaxLate());
  print_status(statusFile, stdOut, "%11s", numStr);

  s_snprintf(numStr, sizeof(numStr) - 1, "%s", "0");
  if (deltaPackets > 0)
    s_snprintf(numStr, sizeof(numStr) - 1, "%" _S32BITARG_ "",
                  (SInt32) ((SInt64) deltaQuality / (SInt64) deltaPackets));
  print_status(statusFile, stdOut, "%11s", numStr);

  s_snprintf(numStr,
                sizeof(numStr) - 1,
                "%" _S32BITARG_ "",
                (SInt32) sServer->GetNumThinned());
  print_status(statusFile, stdOut, "%11s", numStr);
...
void FormattedTotalBytesBuffer(char *outBuffer,
                               int outBufferLen,
                               UInt64 totalBytes) {
  ...
  {
    displayBytes = (Float32) ((Float64) (SInt64) totalBytes
        / (Float64) (SInt64) 1073741824);
    sizeStr[0] = 'G';
    format = "%.4f%s ";
...
void RunServer() {
  ...
  CF::Thread::TaskThreadPool::RemoveThreads();
