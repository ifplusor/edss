...
//long longsize = sizeof(long); printf("size of long = %ld\n", longsize);
  ...
  s_printf("%s/%s ( Build/%s; Platform/%s; %s) Built on: %s\n",
...
CF_Error CFInit(int argc, char **argv) {
  ...
  if (thePort < 0 || thePort > 65535) {
    s_printf("Invalid port value = %d max value = 65535\n", thePort);
    exit(-1);
  }
...
//    s_printf("Streaming Server has expired\n");
//    ::exit(0);
//  }
...
#endif
#if DEBUG
      s_printf("Failed to daemonize process. Error = %d\n", OSThread::GetErrno());
#endif
      exit(-1);
    }
  }
      ...
      if (processID > 0) { // this is the parent and we have a child
        ...
        while (status == 0) { // loop on wait until status is != 0;
          ...
          if (WIFEXITED(status) && pid > 0 && status != 0) { // child exited with status -2 restart or -1 don't restart
            //s_printf("child exited with status=%d\n", exitStatus);

            if (exitStatus == -1) { // child couldn't run don't try again
              s_printf("child exited with -1 fatal error so parent is exiting too.\n");
              exit(EXIT_FAILURE);
            }
          ...
          if (WIFSIGNALED(status)) { // child exited on an unhandled signal (maybe a bus error or seg fault)
            //s_printf("child was signalled\n");
            break; // restart the child
          }

          if (pid == -1 && status == 0) { // parent woken up by a handled signal
            //s_printf("handled signal continue waiting\n");
            continue;
          }

          if (pid > 0 && status == 0) {
            //s_printf("child exited cleanly so parent is exiting\n");
            exit(EXIT_SUCCESS);
          }
...
#if DEBUG
  s_printf("setting priority to Real Time: %s\n", commandStr);
#endif
