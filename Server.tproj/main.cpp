...
int main(int argc, char *argv[]) {
  ...
  static char
      *sDefaultConfigFilePath = DEFAULTPATHS_ETC_DIR_OLD "easydarwin.conf";
  ...
  while ((ch = getopt(argc, argv, "vdfxp:DZ:c:o:S:Ih"))
      != EOF) // opt: means requires option arg
  {
    switch (ch) {
      case 'v':usage();
        ::exit(0);
      case 'd':dontFork = RunInForeground();
  ...
  if (!theXMLPrefsExist) {
    ...
    if (theConfigFilePath != NULL) {
      FilePrefsSource
          *filePrefsSource = new FilePrefsSource(true); // Allow dups
    ...
    do // fork at least once but stop on the status conditions returned by wait or if autoStart pref is false
    {
      ...
      if (processID > 0) // this is the parent and we have a child
      {
        ...
        while (status == 0) //loop on wait until status is != 0;
        {
          ...
          if (WIFEXITED(status) && pid > 0 && status
              != 0) // child exited with status -2 restart or -1 don't restart
          {
            ...
            {
              qtss_printf(
                  "child exited with -1 fatal error so parent is exiting too.\n");
              exit(EXIT_FAILURE);
            }
