...
QTSSAttrInfoDict::AttrInfo  QTSSFile::sAttributes[] =
    {   /*fields:   fAttrName, fFuncPtr, fAttrDataType, fAttrPermission */
        /* 0 */{"qtssFlObjStream", NULL, qtssAttrDataTypeQTSS_StreamRef,
                qtssAttrModeRead | qtssAttrModePreempSafe},
        /* 1 */{"qtssFlObjFileSysModuleName", NULL, qtssAttrDataTypeCharArray,
                qtssAttrModeRead | qtssAttrModePreempSafe},
        /* 2 */{"qtssFlObjLength", NULL, qtssAttrDataTypeUInt64,
                qtssAttrModeRead | qtssAttrModePreempSafe | qtssAttrModeWrite},
        /* 3 */{"qtssFlObjPosition", NULL, qtssAttrDataTypeUInt64,
                qtssAttrModeRead | qtssAttrModePreempSafe},
        /* 4 */{"qtssFlObjModDate", NULL, qtssAttrDataTypeUInt64,
                qtssAttrModeRead | qtssAttrModePreempSafe | qtssAttrModeWrite}
    };
...
QTSS_Error QTSSFile::Open(char *inPath, QTSS_OpenFileFlags inFlags) {
  ...
  CF::Thread::Task *curTask = NULL;
  QTSS_ModuleState
      *theState = (QTSS_ModuleState *) CF::Core::Thread::GetMainThreadData();
  if (CF::Core::Thread::GetCurrent() != NULL)
    theState = (QTSS_ModuleState *) CF::Core::Thread::GetCurrent()->GetThreadData();
  ...
  for (; x
             < QTSServerInterface::GetNumModulesInRole(QTSSModule::kOpenFilePreProcessRole);
         x++) {
    theErr = QTSServerInterface::GetModule(QTSSModule::kOpenFilePreProcessRole,
                                           x)->CallDispatch(
        QTSS_OpenFilePreProcess_Role,
        &theParams);
    if (theErr != QTSS_FileNotFound) {
      fModule =
          QTSServerInterface::GetModule(QTSSModule::kOpenFilePreProcessRole, x);
      break;
    }
  }
  ...
  if (theErr == QTSS_FileNotFound) {
    ...
    if (QTSServerInterface::GetNumModulesInRole(QTSSModule::kOpenFileRole)
        > 0) {
      fModule = QTSServerInterface::GetModule(QTSSModule::kOpenFileRole, 0);
      theErr = QTSServerInterface::GetModule(QTSSModule::kOpenFileRole,
                                             0)->CallDispatch(QTSS_OpenFile_Role,
                                                              &theParams);
...
QTSS_Error QTSSFile::Read(void *ioBuffer,
                          UInt32 inBufLen,
...
QTSS_Error QTSSFile::Seek(UInt64 inNewPosition) {
  ...
  (void) this->GetValuePtr(qtssFlObjLength,
                           0,
                           (void **) &theFileLength,
                           &theParamLength);
