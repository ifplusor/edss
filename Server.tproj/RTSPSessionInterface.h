...
class RTSPSessionInterface : public QTSSDictionary, public CF::Thread::Task {
 public:
  ...
  // belong.
  UInt8 GetTwoChannelNumbers(CF::StrPtrLen *inRTSPSessionID);

  //
  // Given a channel number, returns the RTSP Session ID to which this channel number refers
  CF::StrPtrLen *GetSessionIDForChannelNum(UInt8 inChannelNum);
  ...
  CF::Net::TCPSocket *GetSocket() { return &fSocket; }
 ...
 protected:
  ...
  CF::Thread::TimeoutTask fTimeoutTask;  // allows the session to be timed out
  ...
  CF::Net::TCPSocket fSocket;
  CF::Net::TCPSocket *fOutputSocketP;
  CF::Net::TCPSocket *fInputSocketP;  // <-- usually same as fSocketP, unless we're HTTP Proxying
  ...
  // For OPTIONS request
  CF::StrPtrLen fOldOutputStreamBuffer;
