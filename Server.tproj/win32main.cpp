...
int main(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, "vdxp:o:c:irsS:I"))
      != EOF) // opt: means requires option
  {
    switch (ch) {
      case 'v':
      ...
      case 'x':
        ...
        break;
      case 'i': qtss_printf("Installing the EasyDarwin service...\n");
        ::InstallService("EasyDarwin");
        qtss_printf("Starting the EasyDarwin service...\n");
        ::RunAsService("EasyDarwin");
        ::exit(0);
        break;
      case 'r': qtss_printf("Removing the EasyDarwin service...\n");
        ::RemoveService("EasyDarwin");
        ::exit(0);
      case 's': qtss_printf("Starting the EasyDarwin service...\n");
        ::RunAsService("EasyDarwin");
        ::exit(0);
      case 'I': sInitialState = qtssIdleState;
        break;
      default: break;
    }
  }
  ...
  if (sXMLParser->DoesFileExistAsDirectory()) {
    qtss_printf(
        "Directory located at location where streaming server prefs file should be.\n");
    ::exit(0);
  }
  ...
  if (!theXMLPrefsExist) {
    ...
    if (prefsErr)
      qtss_printf(
          "Could not load configuration file at %s.\n Generating a new prefs file at %s\n",
          theConfigFilePath,
          theXMLFilePath);
  ...
  if (notAService) {
    ...
    ::StartServer(sXMLParser,
                  ...
                  0,
                  kRunServerDebug_Off,
                  sAbsolutePath); // No stats update interval for now
    ::RunServer();
    ::exit(0);
  }
...
void __stdcall ServiceMain(DWORD /*argc*/, LPTSTR * argv) {
  ...
  sServiceStatusHandle =
      ::RegisterServiceCtrlHandler(theServerName, &ServiceControl);
  ...
  if (::StartServer(sXMLParser,
    ...
    ::ReportStatus(SERVICE_STOPPED,
                   ERROR_BAD_COMMAND); // I dunno... report some error
...
{
case
qtssStartingUpState: theStatusReport = SERVICE_START_PENDING;
break;
case
qtssRunningState: theStatusReport = SERVICE_RUNNING;
break;
case
qtssRefusingConnectionsState: theStatusReport = SERVICE_PAUSED;
break;
case
qtssFatalErrorState: theStatusReport = SERVICE_STOP_PENDING;
break;
case
qtssShuttingDownState: theStatusReport = SERVICE_STOP_PENDING;
break;
default:
...
sStatus.
dwCheckPoint = ++sCheckpoint;
else
sStatus.
dwCheckPoint = 0;

sStatus.
dwCurrentState = inCurrentState;
sStatus.
dwServiceSpecificExitCode = inExitCode;
...
void RunAsService(char *inServiceName) {
  ...
  if (theService) {
    ...
    bool stopped =
        ::ControlService(theService, SERVICE_CONTROL_STOP, &lpServiceStatus);
