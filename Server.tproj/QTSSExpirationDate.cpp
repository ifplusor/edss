...
void QTSSExpirationDate::PrintExpirationDate() {
  if (sIsExpirationEnabled)
    s_printf(
        "Software expires on: %s ,Get latest version on www.EasyDarwin.org\n",
        sExpirationDate);
}

void QTSSExpirationDate::sPrintExpirationDate(char *ioExpireMessage) {
  if (sIsExpirationEnabled)
    s_sprintf(ioExpireMessage, "Software expires on: %s\n", sExpirationDate);
}
...
bool QTSSExpirationDate::IsSoftwareExpired() {
  ...
  if (EOF == ::sscanf(sExpirationDate,
                      "%" _S32BITARG_ "/%" _S32BITARG_ "/%" _S32BITARG_ "",
                      &expMonth,
                      &expDay,
                      &expYear)) {
    Assert(false);
  ...
  struct tm *theLocalTime = s_localtime(&theCurrentTime, &timeResult);
