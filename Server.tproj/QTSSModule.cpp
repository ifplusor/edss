...
QTSSModule::QTSSModule(char *inName, char *inPath)
  ...
  if ((inPath != NULL) && (inPath[0] != '\0')) {
    ...
    fFragment = new CodeFragment(inPath);
...
QTSS_Error QTSSModule::LoadFromDisk(QTSS_MainEntryPointPtr *outEntrypoint) {
  ...
  CharArrayDeleter theSymbolName(new char[theFileName.Len + sMainEntrypointName.Len + 2]);
...
SInt64 QTSSModule::Run() {
  ...
  if (events & kUpdateEvent) {   // force us to update to a new idle time
