  ...
  *t = (char *) malloc(len - i);
  ...
  if (server->sin_addr.s_addr == -1) {
    ...
    if (hent != NULL) {
      ...
      s_fprintf(stderr, "Can't find address for %s\n", host);
      free(host);
      free(tag);
...
do_open(char *tool,
  ...
  if (bytag) {
    ...
    if (pstatus != NI_PARSE_OK) {
      s_fprintf(stderr, "%s: incorrect format for domain %s (%s)\n",
                   tool, name, ni_parse_error_string(pstatus));
      s_fprintf(stderr, "usage: -t <host>/<tag>\n");
      s_fprintf(stderr, "<host> can be a host name or IP address\n");
    ...
    *domain = ni_connect(&server, tag);
    if (*domain == NULL) {
      s_fprintf(stderr, "%s: can't connect to server %s\n", tool, name);
      free(tag);
      return NI_FAILED + 1;
    }
  } else {
    ...
    if (status != NI_OK) {
      s_fprintf(stderr, "%s: can't connect to server for domain %s\n",
                   tool, name);
      return status;
    }
  }
  ...
  if (status != NI_OK) {
    if (bytag)
      s_fprintf(stderr, "%s: can't connect to server %s: %s\n",
                   tool, name, ni_error(status));
    else
      s_fprintf(stderr, "%s: can't connect to server for domain %s: %s\n",
                   tool, name, ni_error(status));
    return status;
  }
...
ni_status ni2_createprop(void *domain,
                         char *pathname,
                         const ni_name key,
                         ni_namelist values) {
...
ni_status ni2_appendprop(void *domain,
                         char *pathname,
                         const ni_name key,
                         ni_namelist values) {
...
ni_status ni2_appenddirprop(void *domain,
                            ni_id *dir,
                            const ni_name key,
                            ni_namelist values) {
...
ni_status ni2_insertval(void *domain,
                        char *pathname,
                        const ni_name key,
                        const ni_name value,
                        ni_index where) {
...
ni_status ni2_insertdirval(void *domain,
                           ni_id *dir,
                           const ni_name key,
                           const ni_name value,
                           ni_index whereval) {
...
ni_status ni2_mergeprop(void *domain,
                        char *pathname,
                        const ni_name key,
                        ni_namelist values) {
...
ni_status ni2_destroyval(void *domain,
                         char *pathname,
                         const ni_name key,
                         ni_namelist values) {
...
ni_status ni2_renameprop(void *domain,
                         char *pathname,
                         const ni_name oldname,
                         const ni_name newname) {
  /* rename a property */
...
ni_status ni2_statprop(void *domain,
                       char *pathname,
                       const ni_name key,
                       ni_index *where) {
...
ni_status ni2_statpropdir(void *domain,
                          ni_id *dir,
                          const ni_name key,
                          ni_index *where) {
...
ni_status ni2_statval(void *domain,
                      char *pathname,
                      const ni_name key,
                      const ni_name value,
                      ni_index *where) {
...
ni_status ni2_copy(void *srcdomain,
                   char *path,
                   void *dstdomain,
                   bool recursive) {
...
ni_status ni2_copydir(void *srcdomain,
                      ni_id *srcdir,
                      void *dstdomain,
                      ni_id *dstdir,
                      bool recursive) {
  ...
  if (recursive) {
    ...
    for (i = 0; i < len; i++) {
      ...
      ret =
          ni2_copydirtoparentdir(srcdomain, &dir, dstdomain, dstdir, recursive);
    }
...
ni_status ni2_copydirtoparentdir(void *srcdomain,
  ...
  if (recursive) {
    ...
    for (i = 0; i < len; i++) {
      ...
      ret = ni2_copydirtoparentdir(srcdomain,
                                   &dir,
                                   dstdomain,
                                   &newdstdir,
                                   recursive);
...
ni_status ni2_lookupprop(void *domain,
                         char *pathname,
                         const ni_name key,
                         ni_namelist *values) {
  /* read a property */
...
ni_index ni_namelist_insert_sorted(ni_namelist *values,
                                   const ni_name newvalue) {
  int i, len;
