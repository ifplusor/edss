...
static char const *kMainTag = "CONFIGURATION";
static char const *kServer = "SERVER";
static char const *kModule = "MODULE";
static char const *kPref = "PREF";
static char const *kListPref = "LIST-PREF";
static char const *kEmptyObject = "EMPTY-OBJECT";
static char const *kObject = "OBJECT";
static char const *kObjectList = "LIST-OBJECT";
static char const *kValue = "VALUE";
static char const *kNameAttr = "NAME";
static char const *kTypeAttr = "TYPE";

static char const *kFileHeader[] = {
    "<?xml version =\"1.0\"?>",
...
char *XMLPrefsParser::GetPrefValueByIndex(ContainerRef container, const UInt32 inPrefsIndex, const UInt32 inValueIndex,
  ...
  if (outPrefName != nullptr) *outPrefName = nullptr;
  if (outPrefName != nullptr) *outDataType = nullptr;
  XMLTag *pref = container->GetEmbeddedTag(inPrefsIndex);
  if (pref == nullptr) return nullptr;

  return GetPrefValueByRef(pref, inValueIndex, outPrefName, outDataType);
}

char *XMLPrefsParser::
GetPrefValueByRef(ContainerRef pref, const UInt32 inValueIndex, char const **outPrefName, char const **outDataType) {
...
ContainerRef XMLPrefsParser::GetPrefRefByName(ContainerRef container, char const *inPrefName) {
  return container->GetEmbeddedTagByAttr(kNameAttr, inPrefName);
}

ContainerRef XMLPrefsParser::GetPrefRefByIndex(ContainerRef container, const UInt32 inPrefsIndex) {
  return container->GetEmbeddedTag(inPrefsIndex);
}

ContainerRef XMLPrefsParser::AddPref(ContainerRef container, char *inPrefName, char *inPrefDataType) {
  ...
  else if (strcmp(inPrefDataType, "CharArray") != 0)
...
void XMLPrefsParser::ChangePrefType(ContainerRef pref, char *inNewPrefDataType) {
  ...
  if (strcmp(inNewPrefDataType, "CharArray") != 0)
    pref->AddAttribute(kTypeAttr, inNewPrefDataType);
}

void XMLPrefsParser::SetPrefValue(ContainerRef pref, const UInt32 inValueIndex, char *inNewValue) {
