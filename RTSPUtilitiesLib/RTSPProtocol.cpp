...
QTSS_RTSPMethod RTSPProtocol::GetMethod(const StrPtrLen &inMethodStr) {
  ...
  switch (*inMethodStr.Ptr) {
    case 'S':
    case 's': theMethod = qtssSetupMethod;
      break;
    case 'D':
    case 'd': theMethod = qtssDescribeMethod;
      break;
    case 'T':
    case 't': theMethod = qtssTeardownMethod;
      break;
    case 'O':
    case 'o': theMethod = qtssOptionsMethod;
      break;
    case 'A':
    case 'a': theMethod = qtssAnnounceMethod;
      break;
  }

  if ((theMethod != qtssIllegalMethod) &&
      (inMethodStr.EqualIgnoreCase(sMethods[theMethod].Ptr,
                                   sMethods[theMethod].Len)))
    return theMethod;
...
bool RTSPProtocol::ParseRTSPURL(char const *url,
                                char *username,
                                char *password,
                                char *ip,
                                UInt16 *port,
  ...
  do {
    ...
    for (p = from; *p != '\0' && *p != '/'; ++p) {
      if (*p == ':' && colonPasswordStart == NULL) {
        ...
        if (username)
          copyUsernameOrPasswordStringFromURL(username,
                                              usernameStart,
                                              usernameLen);
        ...
        if (password)
          copyUsernameOrPasswordStringFromURL(password,
                                              passwordStart,
                                              passwordLen);
