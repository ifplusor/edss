...
class HTTPRequest {
 ...
 private:
  ...
  StrPtrLen
      fAbsoluteURI;       // If it is of the form "http://foo.bar.com/path"
  ...
  bool
      fRequestKeepAlive;              // Keep-alive information in the client request
  StrPtrLen fFieldValues[httpNumHeaders
  ];   // Array of header field values parsed from the request
  StrPtrLen
      fSvrHeader;                     // Server header set up at initialization
