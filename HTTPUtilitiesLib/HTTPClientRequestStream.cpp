  ...
  while (true) {
    ...
    if (fRequestPtr != NULL) {
      ...
      if ((fRetreatBytes > 0) && (fRequest.Len > 0))
        ::memmove(fRequest.Ptr,
                  fRequest.Ptr + fRequest.Len + fRetreatBytesRead,
                  fRetreatBytes);
...
QTSS_Error HTTPClientRequestStream::Read(void *ioBuffer,
                                         UInt32 inBufLen,
  ...
  QTSS_Error theErr = fSocket->Read(&theIoBuffer[theLengthRead],
...
QTSS_Error HTTPClientRequestStream::DecodeIncomingData(char *inSrcData,
                                                       UInt32 inSrcDataLen) {
